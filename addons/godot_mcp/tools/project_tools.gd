...
func open_in_godot(args: Dictionary) -> Dictionary:
	...
	var reload_from_disk: bool = bool(args.get(&"reload_from_disk", false))
	...
	if path.ends_with(".gd") or path.ends_with(".shader"):
		...
		if reload_from_disk and ei.get_open_scenes().has(path):
			if ei.get_unsaved_scenes().has(path):
				return {&"ok": false, &"error": "Refusing to reload an unsaved scene from disk: " + path}
	...
	return {&"ok": true, &"message": "%s %s%s" % ["Reloaded and opened" if reload_from_disk else "Opened", path, " at line %d" % line if line > 0 else ""]}
