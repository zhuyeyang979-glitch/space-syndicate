...
const DEFAULT_INITIAL_SCAN_TIMEOUT_MSEC := 90000
...
func observe_editor(
	...
	if _state == STATE_COLD:
		...
		_state = STATE_INITIAL_SCAN_RUNNING
...
func request_reload(request_id: String, path: String, now_msec: int) -> Dictionary:
	...
	if normalized_request_id == "":
		return {
			"ok": false,
			"reason_code": "filesystem_reload_request_id_required",
		}
	if _requests.has(normalized_request_id):
		...
		if str(existing_request.get("path", "")) != path.strip_edges():
			return {
				...
				"reason_code": "filesystem_reload_request_id_conflict",
				"state": _state,
			}
	...
	if _state == STATE_STOPPING:
		return {
			"ok": false,
			"request_id": normalized_request_id,
			"reason_code": "filesystem_reload_stopping",
			"state": _state,
		}
	...
	match _state:
		...
		STATE_RELOAD_RUNNING:
			...
			_reload_pending = true
		STATE_RELOAD_QUEUED, STATE_INITIAL_SCAN_RUNNING, STATE_COLD:
			if _pending_operation_id == "":
				_pending_operation_id = _create_operation("queued", now_msec)
			operation_id = _pending_operation_id
			_reload_pending = true
			if _state in [STATE_INITIAL_SCAN_RUNNING, STATE_COLD]:
			...
			if _state != STATE_INITIAL_SCAN_RUNNING and _state != STATE_COLD:
				_state = STATE_RELOAD_QUEUED
		_:
			return {
				"ok": false,
...
func _complete_initial_scan(_now_msec: int, completion_signal: bool) -> void:
	if _initial_scan_completed:
		return
	_initial_scan_completed = true
	_first_scan_completion_signal_count = 1 if completion_signal else 0
	_set_active_counts(0, 0)
	if _reload_pending:
		_state = STATE_RELOAD_QUEUED
	else:
...
func _complete_reload(operation_id: String, now_msec: int, completion_reason: String) -> void:
	...
	_operations[operation_id] = operation
	_active_operation_id = ""
	_state = STATE_RELOAD_QUEUED if _reload_pending and _pending_operation_id != "" else STATE_READY
	_set_active_counts(0, 0)
...
func _set_active_counts(scan_count: int, reload_count: int) -> void:
	_active_scan_count = clampi(scan_count, 0, 1)
	_active_reload_count = clampi(reload_count, 0, 1)
...
func _check_stop_timeout(now_msec: int) -> void:
	...
	if now_msec - _stopping_started_msec > _stop_timeout_msec and _last_error.is_empty():
		_last_error = {
			"reason_code": "filesystem_reload_stop_timeout",
			"message": "Filesystem reload owner did not stop before timeout.",
