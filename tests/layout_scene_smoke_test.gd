...
func _check_ruleset_v04_source_of_truth_component() -> void:
	...
	var card_submission_source := FileAccess.get_file_as_string("res://scripts/runtime/card_play_submission_runtime_controller.gd")
	var facility_queue_adapter_source := FileAccess.get_file_as_string("res://scripts/runtime/facility_card_queue_adapter_v06.gd")
	...
	_expect(not coordinator_source.contains("func queue_v06_facility_card_action(") and coordinator_source.contains("func resolve_queued_v06_facility_card_action(") and coordinator_source.contains("func advance_card_resolution_frame(") and infrastructure_source.contains("func apply_facility_action(") and infrastructure_source.contains("func rollback_facility_action(") and infrastructure_source.contains("func finalize_facility_action(") and not main_source.contains("func _bind_city_development_runtime_controller") and not main_source.contains("func _evaluate_city_development_request") and not main_source.contains("func _reject_legacy_direct_city_build"), "capability-bound submission bypasses no Coordinator facade while resolution reaches RegionInfrastructure apply/rollback/finalize")
	var ai_controller_source := FileAccess.get_file_as_string(AI_RUNTIME_CONTROLLER_SCRIPT)
	_expect(ai_controller_source.contains("GameActionIntentV1.ACTION_CARD_PLAY") and ai_controller_source.contains("_game_action_submission_port.ai_card_play_offer") and ai_controller_source.contains("_game_action_submission_port.submit_intent") and action_flow_source.contains("func submit_intent(") and action_flow_source.contains("request_hand_play(request)") and action_flow_source.contains("v06_receipt.get(\"queued\"") and card_submission_source.contains("func _queue_v06_facility(") and card_submission_source.contains("_facility_queue_source.submit(_facility_queue_capability"), "AI facility intent follows the typed Offer/Intent application spine into the capability-bound facility Queue")
	_expect(facility_queue_adapter_source.contains("func submit(capability: RefCounted, request: Dictionary)") and facility_queue_adapter_source.contains("facility_queue_submission_unauthorized") and facility_queue_adapter_source.contains("func resolve(entry: Dictionary)") and facility_queue_adapter_source.contains("_queue.commit_submission") and coordinator_scene_source.count("[node name=\"FacilityCardQueueAdapterV06\"") == 1 and coordinator_scene_source.count("[node name=\"TablePlayerActionApplicationFlowController\"") == 1, "GameRuntimeCoordinator composes exactly one formal Action Spine and one capability-bound facility Queue adapter")
	_expect(not main_source.contains("func _build_city_in_selected_district(") and not main_source.contains("_reject_legacy_direct_city_build") and not coordinator_source.contains("func play_runtime_card(") and not ai_controller_source.contains("play_runtime_card") and not action_flow_source.contains("play_runtime_card") and not card_submission_source.contains("play_runtime_card") and coordinator_source.contains("v06_facility_requires_game_action_spine"), "retired direct facility play and direct-build APIs stay absent while legacy runtime routing fails closed into the Action Spine")
...
func _check_game_session_save_ownership_component() -> void:
	...
	if coordinator != null and bridge != null:
		...
		_expect(int(coordinator.call("run_save_version")) == 3 and str(coordinator.call("default_run_save_path")) == "user://saves/v06/current_run.save" and str(save_snapshot.get("ruleset_id", "")) == "v0.6" and int(save_snapshot.get("currency_scale", 0)) == 100 and not bool(save_snapshot.get("explicit_path_required", true)) and bool(save_snapshot.get("production_single_slot", false)) and str(save_snapshot.get("qa_save_root", "")) == "user://test_runs/", "save v3/v0.6 exposes one fixed production slot plus isolated QA paths")
		...
		var player_path_rejected := not bool(save.call("set_qa_default_save_path_override", "user://saves/v06/current_run.save")) if save != null else false
