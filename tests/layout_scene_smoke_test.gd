...
func _run() -> void:
	...
	await _check_player_card_dock_affordance()
	_finish()
...
func _check_main_runtime_composition_scene() -> void:
	...
	if main == null:
		return
	for node_path in ["RuntimeGameScreen", "RuntimeServices", "RuntimeServices/TableAudioHost", "RuntimeServices/RuntimeControllerHost", "RuntimeServices/RuntimeControllerHost/GameRuntimeCoordinator", "RuntimeServices/RuntimeControllerHost/GameRuntimeCoordinator/CardResolutionRuntimeController", "RuntimeServices/RuntimeControllerHost/GameRuntimeCoordinator/RegionInfrastructureRuntimeController", "RuntimeServices/RuntimeControllerHost/GameRuntimeCoordinator/RegionInfrastructureWorldBridge", "RuntimeServices/RuntimeControllerHost/GameRuntimeCoordinator/ForcedDecisionRuntimeScheduler", "RuntimeServices/RuntimeFallbackHost"]:
	...
	for node_name in ["TopBar", "TopCommoditySushiTrack", "PlayerRoster", "PlanetBoard", "PlanetMapView", "PlayerBoard", "PlayerCardDock", "RegionSupplyPopup", "ContextDetailPopup", "PlayerInspectionPopup", "NonBlockingToast", "OverlayLayer", "RuntimeSurfaceLayer", "FullscreenMapOverlay", "FullscreenPlanetMapView", "CardResolutionTableBannerOverlay", "MenuModalOverlay"]:
...
func _check_core_layout_no_overlap() -> void:
	...
	await _check_core_layout_for_scene("res://scenes/ui/GameScreen.tscn", {
		...
		"horizontal": ["PlayerRoster", "PlanetBoard"],
	})
...
func _check_planet_map_sceneization_component() -> void:
	...
	_expect(main_source.contains("map_view = _embedded_runtime_planet_map_view()") and not main_source.contains("PlanetMapViewScene") and not main_source.contains("MapViewScript") and not main_source.contains("func _instantiate_planet_map_view"), "main.gd requires the statically composed PlanetMapView without a script or scene fallback")
	for retired_composition_token in ["RuntimeGameScreenScene", "FullscreenMapOverlayScene", "CardResolutionBannerScene", "BottomCountdownBarScene", "DistrictSupplyDrawerScene", "_mark_runtime_composition_fallback", "runtime_composition_fallbacks", "_make_table_audio_player"]:
...
func _check_viewmodel_contracts() -> void:
	...
	var table: Variant = table_script.new().apply_dictionary({
		...
		"contextual_detail": {
			...
			"context_kind": "region",
			"context_id": "region.fog-harbor",
			"title": "雾港区",
			"subtitle": "海陆商路交界",
			"body": "完整区域说明通过场景化详情弹窗展示。",
			"chips": [{"text": "日照开放", "tooltip": "公开区域状态", "accent": "#4ade80ff"}],
			"actions": [],
			"deep_links": [],
		},
	})
	...
	_expect(not player.to_ui_dictionary().has("hand_cards"), "PlayerBoardSnapshot leaves all card pools to the typed PlayerCardDock projection")
	_expect(str(player.to_ui_dictionary().get("hint", "")).contains("卡牌坞"), "PlayerBoardSnapshot points card interaction to the single production Card Dock")
	...
	var table_contextual_detail: Dictionary = table.to_ui_dictionary().get("contextual_detail", {}) if table.to_ui_dictionary().get("contextual_detail", {}) is Dictionary else {}
	_expect(str(table_contextual_detail.get("context_kind", "")) == "region" and str(table_contextual_detail.get("context_id", "")) == "region.fog-harbor", "TableSnapshot routes typed contextual detail without rebuilding a RightInspector payload")
...
func _check_ruleset_v04_source_of_truth_component() -> void:
	...
	if profile != null:
		...
		_expect(is_equal_approx(float(timing.get("counter_window_seconds", 0.0)), 5.0) and not timing.has("contract_window_seconds"), "Ruleset profile keeps the active counter window and omits the retired response timer")
	...
	_expect(not main_source.contains("_ruleset_timing_seconds(&\"final_countdown_seconds\")") and not main_source.contains("_ruleset_timing_seconds(&\"monster_wager_default_seconds\")") and monster_source.contains("MONSTER_WAGER_WINDOW_SECONDS := MONSTER_WAGER_SETTLEMENT_POLICY_V06.WINDOW_DURATION_SECONDS") and victory_source.contains("victory_qualification") and victory_source.contains("public_audit") and victory_source.contains("audit_failure_cooldown"), "monster owner derives the 15-second wager from the single settlement policy while VictoryControl owns qualification, audit, and cooldown timing")
...
func _check_district_purchase_settlement_and_presentation_preparation() -> void:
	...
	var drawer_legacy_absent := not main_source.contains("func _district_supply_snapshot_source") \
		and not main_source.contains("func _on_district_supply_action_requested") \
		and not main_source.contains("compose_district_supply_snapshot") \
		and ResourceLoader.exists("res://scenes/ui/table/RegionSupplyPopup.tscn") \
		and ResourceLoader.exists("res://scenes/runtime/presentation/DistrictSupplyViewerQueryPort.tscn")
	...
	var settlement_adapters_present := action_port_source.contains("func submit_intent") \
		and action_flow_source.contains("DistrictSupplyActionIntent.KIND_PURCHASE") \
		and action_flow_source.contains("district_supply_action_port")
	var legacy_settlement_absent := not main_source.contains("func _buy_card_for_player_from_district(") \
		and not main_source.contains("func _district_purchase_settlement_request(") \
		and not main_source.contains("func _record_player_card_purchase(") \
		and not main_source.contains("func _discard_card_from_player(")
	_expect(settlement_adapters_present and legacy_settlement_absent and settlement_service_source.contains("after_player[\"cash\"] =") and settlement_service_source.contains("after_player[\"card_purchase_count\"] ="), "typed DistrictSupplyActionPort and Application Flow reach the settlement owner with no Main purchase adapter")
	var audit_script := load(SCENEIZATION_AUDIT_REGISTRY_SCRIPT) as Script
	var audit: RefCounted = audit_script.new() if audit_script != null else null
	if audit != null:
...
func _check_legacy_player_surface_retirement_component() -> void:
	...
	if bench_packed != null:
		...
		var expected_cases := ["real_main_scene_loads", "sceneized_player_board_present", "sceneized_player_card_dock_present", "sceneized_action_dock_present", "sceneized_bid_board_present", "legacy_player_refresh_absent", "legacy_seat_renderer_absent", "legacy_hand_renderer_absent", "legacy_action_tray_absent", "legacy_district_renderer_absent", "legacy_first_summon_renderer_absent", "legacy_tableau_renderer_absent", "legacy_contract_ui_wrapper_absent", "runtime_table_snapshot_pure_data", "typed_card_submission_bridge_present", "application_flow_bridge_present", "privacy_boundary_preserved", "card_presentation_service_composition", "table_viewmodel_service_composition", "card_presentation_runtime_source", "player_card_dock_viewmodel_owned_by_service", "contextual_detail_targets_scene_owned", "public_track_viewmodel_privacy", "coordinator_pure_data_routes", "presentation_rule_boundary_preserved", "legacy_presentation_and_snapshot_owners_absent", "retired_function_set_absent"]
		var cases: Array = bench.call("retirement_cases")
...
func _check_monster_runtime_characterization_component() -> void:
	...
	var controller := controller_packed.instantiate() if controller_packed != null else null
	if controller != null:
		_expect(controller.has_method("configure") and controller.has_method("reset_state") and controller.has_method("tick_wager_decisions_realtime") and controller.has_method("tick_battle_lifecycles") and controller.has_method("tick_motion") and controller.has_method("tick_lifecycle") and controller.has_method("resolve_targeted_skill") and controller.has_method("roster_snapshot") and controller.has_method("to_save_data") and controller.has_method("apply_save_data") and controller.has_method("debug_snapshot"), "MonsterRuntimeController exposes roster, lifecycle, targeting, wager, and save APIs")
...
func _check_weather_runtime_characterization_component() -> void:
	...
	if packed != null:
		...
		var cases: Array = bench.call("characterization_cases")
		var required_cases := ["weather_call_graph_complete", "six_data_driven_weather_types_exist", "weather_control_dispatch_exists", "runtime_state_shape", "initial_forecast_created", "forecast_lead_data_driven_30_to_60", "active_duration_45_to_90_and_fade_10", "single_region_event_and_max_two_unended", "destroyed_districts_excluded", "single_region_anchor_selection", "seeded_fallback_selection", "natural_selection_deterministic_without_order_coupling", "explicit_card_weather_control_schedules", "forced_forecast_keeps_public_warning", "invalid_anchor_rejects_atomically", "unknown_type_falls_back_safely", "sequence_increments_once", "activation_occurs_at_starts_at", "activation_sets_started_and_ends_at", "activation_clears_forecast", "activation_defers_to_generation_clock", "queued_weather_respects_max_two_unended", "expiration_removes_only_expired", "expiration_refreshes_world_once", "production_multiplier_applies", "transport_multiplier_applies", "consumption_multiplier_applies", "gravity_tide_ocean_slowdown", "overlapping_multipliers_compose", "city_network_refresh_routes_once", "product_market_refresh_routes_once", "normal_realtime_tick_continues", "monster_wager_freezes_weather", "readonly_pause_freezes_weather", "ai_weather_intent_uses_same_route", "card_resolution_weather_dispatch", "current_save_shape", "legacy_save_defaults", "public_forecast_privacy", "sprint49_deletion_candidates_complete", "controller_scene_composition", "controller_api_contract", "coordinator_static_composition", "state_owner_cutover", "shared_rng_owner_cutover", "lifecycle_owner_cutover", "multiplier_owner_cutover", "card_rewrite_owner_cutover", "save_owner_cutover", "ai_controller_binding", "pure_debug_snapshot", "main_legacy_weather_absent", "no_parallel_weather_owner"]
...
func _check_industry_capacity_card_group_runtime_component() -> void:
	...
	var coordinator_packed := load(GAME_RUNTIME_COORDINATOR_SCENE) as PackedScene
	var coordinator := coordinator_packed.instantiate() if coordinator_packed != null else null
	var player_mana := coordinator.find_child("PlayerManaRuntimeController", true, false) if coordinator != null else null
	var commodity_flow := coordinator.find_child("CommodityFlowRuntimeController", true, false) if coordinator != null else null
	var commodity_bridge := coordinator.find_child("CommodityFlowWorldBridge", true, false) if coordinator != null else null
	var infrastructure := coordinator.find_child("RegionInfrastructureRuntimeController", true, false) if coordinator != null else null
	var eligibility := coordinator.find_child("CardPlayEligibilityRuntimeService", true, false) if coordinator != null else null
	var queue := coordinator.find_child("CardResolutionQueueRuntimeService", true, false) if coordinator != null else null
	var mana_public: Dictionary = player_mana.call("public_snapshot") if player_mana != null else {}
	var mana_debug: Dictionary = player_mana.call("debug_snapshot") if player_mana != null else {}
	var flow_debug: Dictionary = commodity_flow.call("debug_snapshot") if commodity_flow != null else {}
	var bridge_debug: Dictionary = commodity_bridge.call("debug_snapshot") if commodity_bridge != null else {}
	var infrastructure_debug: Dictionary = infrastructure.call("debug_snapshot") if infrastructure != null else {}
	var eligibility_debug: Dictionary = eligibility.call("debug_snapshot") if eligibility != null else {}
	var queue_debug: Dictionary = queue.call("debug_snapshot") if queue != null else {}
	var current_owner_gates_load := load(MAIN_RUNTIME_COMPOSITION_TEST) is Script and load("res://tests/card_player_state_production_adapter_v06_test.gd") is Script and load("res://tests/card_resolution_queue_cadence_v06_test.gd") is Script and load("res://tests/commodity_flow_local_baseline_demand_v06_test.gd") is Script
	var current_nodes_composed := player_mana != null and player_mana.scene_file_path == "res://scenes/runtime/PlayerManaRuntimeController.tscn" and commodity_flow != null and commodity_flow.scene_file_path == COMMODITY_FLOW_RUNTIME_CONTROLLER_SCENE and commodity_bridge != null and commodity_bridge.scene_file_path == COMMODITY_FLOW_WORLD_BRIDGE_SCENE and infrastructure != null and infrastructure.scene_file_path == REGION_INFRASTRUCTURE_RUNTIME_CONTROLLER_SCENE and eligibility != null and eligibility.scene_file_path == CARD_PLAY_ELIGIBILITY_RUNTIME_SERVICE_SCENE and queue != null and queue.scene_file_path == CARD_RESOLUTION_QUEUE_RUNTIME_SERVICE_SCENE
	var retired_nodes_absent := coordinator != null and coordinator.find_child("IndustryCapacityRuntimeService", true, false) == null and coordinator.find_child("IndustryCapacityWorldBridge", true, false) == null
	var owner_boundaries_ok := bool(mana_debug.get("asset_balance_authority", false)) and not bool(mana_debug.get("commodity_flow_authority", true)) and not bool(mana_debug.get("queue_authority", true)) and not bool(mana_debug.get("legacy_industry_capacity_fallback_used", true)) and bool(flow_debug.get("owns_capacity_allocation", false)) and bool(flow_debug.get("owns_fixed_point_flow", false)) and bool(flow_debug.get("owns_sale_receipt_ledger", false)) and not bool(flow_debug.get("owns_cash_state", true)) and str(bridge_debug.get("runtime_owner", "")) == "none" and not bool(bridge_debug.get("owns_flow_rules", true)) and not bool(bridge_debug.get("owns_sale_receipts", true)) and str(infrastructure_debug.get("runtime_owner", "")) == "RegionInfrastructureRuntimeController" and str(infrastructure_debug.get("ruleset_id", "")) == "v0.6" and bool(eligibility_debug.get("owns_asset_requirement_interpretation", false)) and bool(eligibility_debug.get("industry_capacity_requirement_retired", false)) and not bool(eligibility_debug.get("queue_authority", true)) and not bool(queue_debug.get("asset_reservation_authority", true)) and not bool(queue_debug.get("priority_bid_authority", true)) and not bool(queue_debug.get("cash_authority", true))
	var public_boundary_ok := bool(mana_public.get("asset_balances_private", false)) and bool(mana_public.get("reservation_count_hidden", false)) and not mana_public.has("pools_by_player") and not mana_public.has("reservations") and not _variant_contains_callable(mana_public) and not _variant_contains_object(mana_public) and not _variant_contains_callable(mana_debug) and not _variant_contains_object(mana_debug) and not _variant_contains_callable(flow_debug) and not _variant_contains_object(flow_debug) and not _variant_contains_callable(bridge_debug) and not _variant_contains_object(bridge_debug) and not _variant_contains_callable(infrastructure_debug) and not _variant_contains_object(infrastructure_debug) and not _variant_contains_callable(eligibility_debug) and not _variant_contains_object(eligibility_debug) and not _variant_contains_callable(queue_debug) and not _variant_contains_object(queue_debug)
	_expect(current_nodes_composed and retired_nodes_absent and owner_boundaries_ok and public_boundary_ok and current_owner_gates_load, "GameRuntimeCoordinator composes the v0.6 asset/recovery chain while the retired SS05-05 capacity owner and bridge stay absent")
	if coordinator != null:
		coordinator.free()
...
func _check_player_card_dock_affordance() -> void:
	var packed := load("res://scenes/ui/table/PlayerCardDock.tscn") as PackedScene
	_expect(packed != null, "PlayerCardDock scene loads for three-pool affordance")
	if packed == null:
		return
	var dock := packed.instantiate() as SpaceSyndicatePlayerCardDock
	root.add_child(dock)
	await process_frame
	dock.bind_viewer(0, 1)
	var bound_host := dock.find_child("BoundActionCards", true, false)
	var normal_host := dock.find_child("NormalHandCards", true, false)
	var commodity_host := dock.find_child("CommodityCards", true, false)
	var debug := dock.debug_snapshot()
	_expect(bound_host != null and normal_host != null and commodity_host != null, "PlayerCardDock owns one host for each authorized card pool")
	_expect(_node_tree_text(dock).contains("绑定行动 0") and _node_tree_text(dock).contains("普通牌 0") and _node_tree_text(dock).contains("商品牌 0"), "empty Card Dock keeps all three pool identities visible without fabricating cards")
	_expect(_node_tree_text(dock).contains("V0.6 共享容量") and _node_tree_text(dock).contains("0 / 5"), "empty Card Dock truthfully presents the current shared V0.6 capacity")
	_expect(int(debug.get("visible_card_count", -1)) == 0 and not bool(debug.get("mutates_gameplay", true)) and not bool(debug.get("reads_world_state", true)), "empty Card Dock is a read-only typed target with zero action entries")
...
func _check_main_player_panel_refresh_contract() -> void:
	...
	var player_card_dock_source := FileAccess.get_file_as_string("res://scripts/ui/table/player_card_dock.gd")
	...
	_expect(not main_source.contains("func _runtime_player_board_quick_actions") and presentation_query_source.contains("\"quick_actions\"") and player_card_dock_source.contains("game_action_offer_requested.emit") and player_card_dock_source.contains("OFFER.validation_report"), "scene-owned PlayerCardDock emits typed offers while the presentation query keeps non-card quick actions")
	...
	var drag_body := _function_source(game_screen_source, "_on_card_drag_released")
	_expect(not main_source.contains("_on_runtime_game_screen_card_drop_requested") and drag_body.contains("_district_at_map_position(screen_position)") and drag_body.contains("{\"region_id\": region_id}") and action_flow_source.contains("request[\"selected_district\"] = district_index"), "GameScreen converts drag hit testing to a stable region ID and the typed flow revalidates it outside Main")
	_expect(not player_board_source.contains("HandRack") and not player_board_source.contains("hand_cards"), "PlayerBoard has physically relinquished ordinary-card rendering to PlayerCardDock")
	_expect(player_card_dock_source.contains("func _render_pool") and player_card_dock_source.contains("_card_node_by_identity") and player_card_dock_source.contains("func _card_identity"), "PlayerCardDock performs same-identity synchronization for all three typed pools")
	_expect(player_card_dock_source.contains("signal card_unselected") and player_card_dock_source.contains("_selected_identity") and player_card_dock_source.contains("_sync_selection_visuals"), "PlayerCardDock owns stable selected-card focus without Main or PlayerBoard")
	_expect(player_card_dock_source.contains("func submit_target_selection") and player_card_dock_source.contains("OFFER.target_ids") and not player_card_dock_source.contains("_use_skill"), "PlayerCardDock binds stable region targets and emits offers without executing gameplay rules")
