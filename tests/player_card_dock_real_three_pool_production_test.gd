...
func _purchase_real_rack_card(
	...
	var popup := screen.get_node_or_null("RegionSupplyPopup") as SpaceSyndicateRegionSupplyPopup
	if world == null or query == null or presentation == null or popup == null:
		return false
	...
	popup.bind_viewer(-1, 0)
	popup.bind_viewer(0, authorization_revision)
	var quote_surface := query.snapshot_for_viewer(0)
	if not popup.apply_presentation(quote_surface, 0, authorization_revision):
		return false
	popup.call("_on_card_purchase_requested", card_id, "alpha04_real_quote")
	var purchase_surface := query.snapshot_for_viewer(0)
	if not popup.apply_presentation(purchase_surface, 0, authorization_revision):
		return false
	popup.call("_on_card_purchase_requested", card_id, "alpha04_real_purchase")
	await process_frame
