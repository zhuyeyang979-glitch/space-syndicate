...
func _run() -> void:
	...
	_expect(main_source.contains("var max_chips := 2 if compact else 5") and main_source.contains("if compact:\n\t\treturn") and main_source.contains("var art_height := 42 if compact and is_hand_card else"), "compact hand cards keep status reasons in hover instead of adding another text row")
	...
	_expect(main_source.contains("牌轨") and main_source.contains("CardResolutionTtaMarketPanel") and main_source.contains("CardResolutionTtaOfferRailFrame") and main_source.contains("CardResolutionTtaOfferRailLegend") and main_source.contains("公共牌槽") and main_source.contains("拖看｜悬停") and main_source.contains("CardResolutionTtaSlotMarketMat") and main_source.contains("CardResolutionTtaAgeMarketRuler") and main_source.contains("CardResolutionTtaCostBandRail") and main_source.contains("CardResolutionTtaCostCell") and main_source.contains("CardResolutionTtaSlotGrooveRail") and main_source.contains("CardResolutionTtaScrollShell") and main_source.contains("CardResolutionTtaScrollCue") and main_source.contains("CardResolutionTtaGhostSlot") and main_source.contains("固定公共牌槽") and main_source.contains("CardResolutionTtaMiniCard") and main_source.contains("CARD_TRACK_SLOT_HEIGHT := 22") and main_source.contains("CardResolutionCostPipRail") and main_source.contains("CARD_TRACK_MANUAL_SCROLL_HOLD_MSEC") and main_source.contains("_mark_card_resolution_track_manual_scroll") and main_source.contains("_maybe_follow_card_resolution_track") and main_source.contains("_connect_card_resolution_track_hover"), "anonymous card track uses a thin Through-the-Ages-style public offer rail instead of prose cards")
