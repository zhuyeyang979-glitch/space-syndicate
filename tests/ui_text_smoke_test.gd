...
func _run() -> void:
	...
	_expect(main_source.contains("var max_chips := 2 if compact else 5") and main_source.contains("if compact:\n\t\treturn") and main_source.contains("var art_height := 34 if compact_hand_card else"), "compact hand cards keep status reasons in hover and fit as first-screen mini-cards")
	...
	_expect(main_source.contains("DistrictSupplyShelfBoard") and main_source.contains("district_supply_chip_row") and main_source.contains("DistrictSupplyMarketStatusRail") and main_source.contains("DistrictSupplyMarketSummaryChip") and main_source.contains("价格已锁") and main_source.contains("单窗口") and main_source.contains("手牌 %d/%d") and main_source.contains("\"可买\"") and main_source.contains("\"弃牌\"") and main_source.contains("\"仅看\""), "district card rack uses board-game shelf chips and short locked-window affordances")
	_expect(main_source.contains("市场牌架｜悬停看｜双击买") and main_source.contains("DistrictSupplySideDrawer") and main_source.contains("DistrictSupplyMarketGrid") and main_source.contains("DistrictSupplyMarketCardPanel") and main_source.contains("DistrictSupplyMarketCardChipRail") and main_source.contains("DistrictSupplyMarketCardRank") and main_source.contains("DistrictSupplyMarketCardStateBand") and main_source.contains("DistrictSupplyMarketCardStateSignal") and main_source.contains("DistrictSupplySelectedPreview") and main_source.contains("DistrictSupplyPurchaseVerdictRail") and main_source.contains("DistrictSupplyPurchaseVerdictSignal"), "district card rack uses a compact side-drawer market presentation with board-game market cards")
