...
func _init() -> void:
	...
	_expect(submission.contains("func _queue_v06_facility(") \
			and submission.contains("_facility_queue_source.submit(") \
			and submission.contains("_facility_queue_capability,"), "v0.6 facilities have one capability-bound queue submission adapter path")
	...
	_expect(flow.contains("result = card_play.request_hand_play(request)") \
			and flow.contains("result = card_play.retry_hand_play(request)") \
			and flow.contains("bool(v06_receipt.get(\"queued\", false))"), "human and AI GameAction intents share the typed card submission path and identical failed delivery uses only the closed recovery port")
