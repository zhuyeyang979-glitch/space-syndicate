...
const BENCH_SCENE := preload("res://scenes/tools/WeatherRegionDamageV1Bench.tscn")
...
func _run() -> void:
	...
	print("WEATHER_REGION_DAMAGE_V1_TEST|status=%s|checks=%d|failures=%d" % [
		"PASS" if passed else "FAIL",
		int(result.get("check_count", 0)),
		int(result.get("failure_count", 0)),
	])
	if not passed:
		push_error("Weather region damage v1 failures: %s" % str(result.get("failed_cases", [])))
	bench.queue_free()
	await process_frame
