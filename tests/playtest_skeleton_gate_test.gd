...
func _check_card_skeleton() -> void:
	...
	var district_preview_scene := FileAccess.get_file_as_string("res://scenes/ui/DistrictSupplyPreviewCard.tscn")
	var district_preview_script := FileAccess.get_file_as_string("res://scripts/ui/district_supply_preview_card.gd")
	var district_snapshot_service := FileAccess.get_file_as_string("res://scripts/runtime/district_supply_snapshot_service.gd")
	...
	_expect(_contains_all(district_preview_scene, ["DistrictSupplyDecisionStrip", "DistrictSupplyPreviewBodyLabel", "max_lines_visible = 3"]) and _contains_all(district_preview_script, ["decision_strip", "DistrictSupplyDecisionChip"]) and _contains_all(district_snapshot_service, ["func _decision_chips", "\"用途:%s\"", "\"买入:%s\"", "\"打出:免门槛\""]), "district supply Snapshot Service keeps a scan-first decision strip for use, buy state, play gate, and target before long text")
...
func _check_table_skeleton() -> void:
	...
	_expect(_contains_all(player_board, ["PlayerResourceTableau", "PlayerHandTableau", "PlayerCommandTableau", "HandRack"]) and not player_board.contains("PlayerBidBoard"), "PlayerBoard has resource, hand, command, and hand-rack skeletons without a permanent bid region")
	...
	_expect(_contains_all(overlay_scene, ["SideDrawerLayer", "ModalLayer", "DragPreviewLayer", "PublicBidDecisionPanel"]) and overlay.contains("_dock_confirm_to_planet_side_lane"), "OverlayLayer owns drawer/modal/drag layers and the transient public-bid surface at the table edge")
