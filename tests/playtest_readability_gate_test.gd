...
func _check_player_facing_source_guards() -> void:
	...
	var district_supply_snapshot_service := FileAccess.get_file_as_string("res://scripts/runtime/district_supply_snapshot_service.gd")
	...
	_expect(main_source.contains("第一局：首召、开牌架、发展项目、读结算。"), "tutorial menu opens with one-line v0.4 first-game guidance")
	...
	_expect(district_supply_preview_scene.contains("DistrictSupplyPreviewScanGrid") and district_supply_preview_script.contains("SCAN_SECTION_BODY_LIMIT := 34") and district_supply_preview_script.contains("_render_scan_sections") and district_supply_preview_script.contains("body_label.visible = body_label.text != \"\" and not has_scan_sections") and district_supply_preview_script.contains("facts_label.visible = facts_label.text != \"\" and not has_scan_sections") and district_supply_preview_script.contains("status_label.visible = status_label.text != \"\" and not has_scan_sections") and district_supply_snapshot_service.contains("func _preview_scan_sections") and district_supply_snapshot_service.contains("\"title\": \"用途\"") and district_supply_snapshot_service.contains("\"title\": \"买入\"") and district_supply_snapshot_service.contains("\"title\": \"打出\"") and district_supply_snapshot_service.contains("\"title\": \"目标\""), "district supply Snapshot Service supplies four compact scan sections instead of main.gd or always-visible dense prose")
