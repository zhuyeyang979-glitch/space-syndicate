...
func _run() -> void:
	...
	_expect(driver_source.contains('main_instance.set("configured_player_count", RECOMMENDED_PLAYER_COUNT)') and driver_source.contains("_confirm_start_new_run_from_setup"), "driver starts a normal four-seat session")
