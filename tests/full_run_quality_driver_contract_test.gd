...
func _run() -> void:
	...
	_expect(driver_source.contains('int(public_progress.get("top_k_gdp_per_minute", 0)) <= 0'), "the driver waits for the first authoritative GDP receipt before spending the source revision on expansion navigation")
