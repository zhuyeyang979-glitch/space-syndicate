...
func _run() -> void:
	...
	_expect(driver_source.contains('["district_supply_wait", "facility_play_wait", "gdp_accumulation_wait"]'), "only explicit no-action rack, facility-play, and GDP waits use the higher acceleration tier")
	...
	_expect(driver_source.contains('facility_chain_incomplete := int(public_progress.get("owned_facility_count", 0)) < 4'), "the scripted player derives its opening facility-chain goal from the public progress projection")
	_expect(driver_source.contains('reason_facility_not_visible') and driver_source.contains('not bool(wait_facts.get("has_visible_facility", true))'), "a visible rack without a facility rotates immediately instead of buying unrelated cards or inspecting future rack order")
	...
	_expect(DriverScript.ACTION_ENGINE_TIME_SCALE == 1.0 and DriverScript.SUPPLY_WAIT_ENGINE_TIME_SCALE == 8.0 and DriverScript.GDP_WAIT_ENGINE_TIME_SCALE == 8.0 and driver_source.contains("Engine.time_scale = 1.0"), "typed UI actions remain human-paced while explicit solar/GDP waits accelerate and cleanup always restores the global clock")
