...
func _test_game_screen_typed_adapters() -> void:
	...
	_expect(intents.size() == 1 and int(gameplay_intents[0]) == 0, "card-resolution track selection emits one typed selection intent and zero gameplay intents")
	_expect(intents.back().selection_kind == TableSelectionIntent.KIND_SELECT_CARD_RESOLUTION and intents.back().target_card_resolution_id == 51 and intents.back().source_surface == &"card_resolution_track", "typed track adapter carries only stable public resolution identity")
