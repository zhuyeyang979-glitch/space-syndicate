...
func _run() -> void:
	...
	_expect(int(evidence.get("production_required_sections", 0)) == 17 and int(evidence.get("production_transactional_sections", 0)) == 5 and int(evidence.get("production_unsupported_sections", 0)) == 12, "production registry exposes the audited 5/12 capability boundary")
...
func _finish() -> void:
	print("V06_SAVE_OWNER_REGISTRY_TEST|status=%s|checks=7|failures=%d" % ["PASS" if _failures.is_empty() else "FAIL", _failures.size()])
