...
func _run() -> void:
	...
	_expect(main_source.contains("player_panel_scroll.custom_minimum_size = Vector2(0, 318)") and main_source.contains("首屏优先看到自己的手牌"), "bottom table-edge rack is tall enough to show the player's hand on the first screen")
	...
	var resource_index := main_source.find("_add_player_resource_cube_rail(hand_column")
	...
	_expect(resource_index > hand_index, "resource cube rail stays below the hand rack so cards are visible first")
	_expect(goal_index > hand_index, "next-step goal prompt stays below the hand rack so cards remain visible first")
	_expect(first_summon_index > tray_index and first_summon_index < district_action_index, "starter-summon prompt is the first action-tray decision for new testers")
