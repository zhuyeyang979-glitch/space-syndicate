...
func _run() -> void:
	...
	_expect(main_source.contains("card_minimum_size = Vector2(160, 168)") and main_source.contains("var art_height := 42 if compact and is_hand_card"), "compact hand cards stay small enough for a bottom rack while preserving hover detail")
	_expect(main_source.contains("CardFaceChipRail") and main_source.contains("card_face_chip_rail") and main_source.contains("_add_card_face_chip_rail") and main_source.contains("HandCardPlayLamp") and main_source.contains("HandCardPlayLampStatus") and main_source.contains("HandCardPlayStateRail") and main_source.contains("HandCardPlayReason") and main_source.contains("_short_card_text(_card_rules_text(skill_name, skill, true).replace") and main_source.contains("action_button.custom_minimum_size = Vector2(0, 24)"), "card faces use compact chip rails, a status lamp, and clipped hand-card effect text for costs and hand-play status")
	...
	_expect(main_source.contains("hand_scroll.custom_minimum_size = Vector2(0, 168)") and main_source.contains("PlayerHandRackPanel") and main_source.contains("PlayerHandRackChipRail") and main_source.contains("PlayerHandEmptySlot"), "hand rack reserves enough height and reads as a board-game card rack")
