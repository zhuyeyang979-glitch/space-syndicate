...
func _init() -> void:
	...
	_check(not main_source.contains("advance_table_presentation(delta)"), "Main no longer owns or forwards the presentation frame")
	...
	var presentation_phase_source := FileAccess.get_file_as_string("res://scripts/runtime/runtime_presentation_schedule_coordinator.gd")
	_check(runtime_loop_source.contains("_phase_coordinator.advance_frame(real_delta)") and presentation_phase_source.contains("_presentation.advance_table_presentation(context.real_delta)"), "phase coordination delegates presentation through the typed presentation port without RuntimeLoop consuming receipts")
