...
func _run() -> void:
	...
	_expect(str(snapshot.get("summary_text", "")).contains("情报换钱") and str(snapshot.get("summary_text", "")).contains("当前不揭示正误"), "summary explains settlement without revealing truth")
	...
	_expect(action_ids.has("track_return_42") and action_ids.has("track_guess_42") and action_ids.has("track_open_轨道融资1"), "existing focused track action ids stay intact")
