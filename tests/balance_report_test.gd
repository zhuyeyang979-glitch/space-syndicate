...
func _run() -> void:
	...
	var scenario_curve: Array = result.get("scenario_power_curve", []) if result.get("scenario_power_curve", []) is Array else []
	var scenario_recommendations: Dictionary = result.get("scenario_recommendations", {}) if result.get("scenario_recommendations", {}) is Dictionary else {}
	...
	_expect(scenario_curve.size() >= 4, "balance report emits a price/power curve for each vertical-slice scenario")
	for scenario_id in ["first_table", "monster_pressure", "public_track_intro", "bid_practice"]:
		var picks: Array = scenario_recommendations.get(scenario_id, []) if scenario_recommendations.get(scenario_id, []) is Array else []
		_expect(picks.size() >= 1, "balance report emits recommendations for %s" % scenario_id)
	...
	_expect(report.contains("价格过低 Top 20") and report.contains("价格过高 Top 20") and report.contains("Rank I-IV 梯度异常") and report.contains("剧本价格/强度曲线") and report.contains("首局剧本推荐卡") and report.contains("怪兽压迫推荐卡") and report.contains("公开牌轨推荐卡") and report.contains("竞价练习推荐卡") and report.contains("不适合首局剧本的复杂卡"), "generated balance_report contains required sections")
	var checked_in_report := FileAccess.get_file_as_string("res://docs/balance_report.md")
	_expect(checked_in_report.contains("价格过低 Top 20") and checked_in_report.contains("剧本价格/强度曲线") and checked_in_report.contains("首局剧本推荐卡") and checked_in_report.contains("怪兽压迫推荐卡") and checked_in_report.contains("公开牌轨推荐卡") and checked_in_report.contains("竞价练习推荐卡"), "checked-in balance_report is present with required player-facing sections")
