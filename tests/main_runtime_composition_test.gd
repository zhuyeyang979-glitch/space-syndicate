...
func _check_static_composition(main: Control) -> void:
	...
	for node_name in ["TopBar", "PlayerRosterPanel", "PlanetBoard", "PlanetMapView", "PlayerBoard", "CompactCurrentActionSurface", "PlayerCardDock", "ContextDetailDrawer", "OverlayLayer"]:
	...
	for node_name in ["FullscreenMapOverlay", "FullscreenPlanetMapView", "PlanetMapControlToolbar", "CardResolutionTableBannerOverlay", "BottomCountdownOverlay", "RegionSupplyPopup", "MenuModalOverlay"]:
	...
	var region_supply_popup := screen.find_child("RegionSupplyPopup", true, false) as Control if screen != null else null
	_expect(region_supply_popup != null and region_supply_popup.scene_file_path == "res://scenes/ui/table/RegionSupplyPopup.tscn", "OverlayLayer owns the typed RegionSupplyPopup scene instance")
	_expect(region_supply_popup != null and region_supply_popup.has_method("bind_viewer") and region_supply_popup.has_method("apply_projection") and region_supply_popup.has_method("clear_projection") and region_supply_popup.has_method("debug_snapshot") and region_supply_popup.has_signal("supply_action_requested") and region_supply_popup.has_signal("navigation_intent_requested"), "RegionSupplyPopup owns a typed projection and aggregate action boundary")
	_expect(region_supply_popup != null and region_supply_popup.find_child("ProductionDistrictSupplyDrawer", true, false) != null, "typed RegionSupplyPopup embeds one production drawer renderer")
	...
	var retired_presentation_functions := ["_card_theme_color", "_card_category_icon", "_card_use_case_text_for_skill", "_card_rules_text", "_card_rule_facts", "_card_detail_tooltip", "_runtime_hand_card_snapshots", "_runtime_card_track_snapshot_source", "_runtime_card_resolution_track_snapshot_source", "_card_resolution_track_badge_texts", "_card_resolution_track_badge_color", "_card_resolution_overlay_badge_texts", "_card_resolution_contract_badge_text"]
	var presentation_cutover_checked := not main_source.contains("TableSnapshotScript") and card_presentation_source.contains("func compose_card(") and card_presentation_source.contains("func compose_hand_card(") and table_viewmodel_source.contains("func compose_table(") and table_viewmodel_source.contains("\"owns_contextual_surface_sources\": true") and table_viewmodel_source.contains("func _compose_track(") and table_snapshot_source.contains("current_action_context") and table_snapshot_source.contains("context_detail")
	...
	_expect(presentation_cutover_checked, "Sprint 42 removes parallel card presentation, hand/track/context-detail, and TableSnapshot owners from main.gd")
