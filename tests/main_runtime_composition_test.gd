...
func _check_static_composition(main: Control) -> void:
	...
	for node_name in ["TopBar", "PlayerRoster", "PlanetBoard", "PlanetMapView", "PlayerBoard", "PlayerCardDock", "RegionSupplyPopup", "ContextDetailPopup", "PlayerInspectionPopup", "NonBlockingToast", "OverlayLayer"]:
	...
	for node_name in ["FullscreenMapOverlay", "FullscreenPlanetMapView", "PlanetMapControlToolbar", "CardResolutionTableBannerOverlay", "MenuModalOverlay"]:
	...
	var district_supply_drawer := screen.find_child("RegionSupplyPopup", true, false) as Control if screen != null else null
	_expect(district_supply_drawer != null and district_supply_drawer.scene_file_path == "res://scenes/ui/table/RegionSupplyPopup.tscn", "GameScreen owns the typed RegionSupplyPopup scene instance")
	_expect(district_supply_drawer != null and district_supply_drawer.has_method("apply_presentation") and district_supply_drawer.has_method("clear_supply") and district_supply_drawer.has_method("presentation_target_snapshot") and district_supply_drawer.has_signal("game_action_offer_requested"), "RegionSupplyPopup owns its viewer-safe presentation target and typed GameActionOffer boundary")
	for drawer_node_name in ["DistrictPurchaseWindowStatus", "DistrictSupplyShelfChipRail", "DistrictSupplyMarketStatusRail", "DistrictSupplyPrivacyHint", "DistrictSupplyMarketGrid", "DistrictSupplyMarketEmptyState", "DistrictSupplyPreviewBox", "DistrictSupplyPreviewEmptyState"]:
		_expect(district_supply_drawer != null and district_supply_drawer.find_child(drawer_node_name, true, false) != null, "RegionSupplyPopup statically owns %s" % drawer_node_name)
	for retired_name in ["RightInspector", "DistrictSupplySideDrawerOverlay", "HandRack", "PlayerHandTableau", "RoleSeatLayerHost", "BackSeatLayer", "FrontSeatLayer"]:
		_expect(screen == null or screen.find_child(retired_name, true, false) == null, "production GameScreen retires %s" % retired_name)
	...
	_expect(table_viewmodel != null and table_viewmodel.scene_file_path == GAME_TABLE_VIEWMODEL_RUNTIME_SERVICE and table_viewmodel.has_method("compose_table") and table_viewmodel.has_method("compose_card_surfaces") and not table_viewmodel.has_method("compose_resolution_overlay_badges") and table_viewmodel.has_method("debug_snapshot"), "GameRuntimeCoordinator owns the authoritative GameTableViewModelRuntimeService scene and retires the legacy overlay-badge API")
	...
	var retired_presentation_functions := ["_card_theme_color", "_card_category_icon", "_card_use_case_text_for_skill", "_card_rules_text", "_card_rule_facts", "_card_detail_tooltip", "_runtime_hand_card_snapshots", "_runtime_card_track_snapshot_source", "_runtime_right_inspector_snapshot_source", "_runtime_card_resolution_track_snapshot_source", "_card_resolution_track_badge_texts", "_card_resolution_track_badge_color", "_card_resolution_overlay_badge_texts", "_card_resolution_contract_badge_text", "_build_card_resolution_overlay", "_show_card_batch_lobby_overlay", "_show_card_resolution_overlay", "_hide_card_resolution_overlay", "_refresh_card_resolution_overlay_badges", "_runtime_card_resolution_overlay_badge_source", "_card_resolution_overlay_compact_body_text", "_card_resolution_overlay_detail_text", "_update_card_resolution_timer_bar"]
	var presentation_cutover_checked := not main_source.contains("TableSnapshotScript") and card_presentation_source.contains("func compose_card(") and card_presentation_source.contains("func compose_hand_card(") and table_viewmodel_source.contains("func compose_table(") and table_viewmodel_source.contains("func _compose_contextual_detail(") and table_viewmodel_source.contains("func _compose_track(")
	...
	presentation_cutover_checked = presentation_cutover_checked \
		and not main_source.contains("CardResolutionTableBannerOverlay") \
		and not main_source.contains("BottomCountdownOverlay") \
		and not main_source.contains("PlanetRightSpaceRail") \
		and not main_source.contains("compose_game_resolution_overlay_badges")
	_expect(presentation_cutover_checked, "presentation composition owns typed hand/track/context data outside main.gd")
...
func _check_runtime_snapshot(main: Control, phase: String) -> void:
	...
	_expect(bool(table_viewmodel_snapshot.get("service_ready", false)) and bool(table_viewmodel_snapshot.get("service_authoritative", false)) and bool(table_viewmodel_snapshot.get("owns_table_snapshot_normalization", false)) and bool(table_viewmodel_snapshot.get("owns_contextual_detail_assembly", false)) and bool(table_viewmodel_snapshot.get("owns_public_track_viewmodels", false)) and bool(table_viewmodel_snapshot.get("owns_typed_resolution_overlay_projection", false)) and not bool(table_viewmodel_snapshot.get("calculates_play_legality", true)) and not bool(table_viewmodel_snapshot.get("mutates_game_state", true)), "%s configures scene-owned TableSnapshot, public track, typed resolution overlay, and contextual-detail assembly" % phase)
