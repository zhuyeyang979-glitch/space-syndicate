...
func _check_first_ten_minute_action_chain() -> void:
	...
	var owned_facilities: Array = project_content.get("owned_facilities", []) if project_content.get("owned_facilities", []) is Array else []
	_expect(bool(progress_after_play.get("has_played_card", false)), "first ten-minute path marks the real development card play")
	_expect(project_resolved and bool(project_content.get("city_present", false)) and not owned_facilities.is_empty(), "first ten-minute path creates a real public facility owned by the local player")
	...
	_expect(str(main.call("_first_run_coach_stage", progress_after_economy)) == "buy_followup", "first ten-minute path moves from first project income to a second current-rack purchase")
	if main.has_method("_close_menu"):
		main.call("_close_menu")
	await _wait_frames(4)
	var followup_purchase_before := _local_card_purchase_count(main)
	_expect(bool(main.call("_activate_first_run_coach_action", "coach_buy_card")), "first ten-minute path can buy a second currently public regional listing")
