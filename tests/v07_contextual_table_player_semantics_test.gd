...
func _run() -> void:
	...
	_expect(bool(initial.get("planet_map_connected", false)), "surface uses the real production PlanetMapView")
	...
	_expect(map_view != null, "production map input remains available in the Bench surface")
	if map_view != null:
