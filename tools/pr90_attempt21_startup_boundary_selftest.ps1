...
function New-CompleteFacts {
    return @{
        ...
        endpoint_owner_match=$true; connection_file_exists=$true; session_id_match=$true; first_request_sent=$true;
...
$f = New-CompleteFacts; $f.endpoint_owner_match=$false; Add-BoundaryCase 'endpoint_owner_wrong' $f 'M5' $false 'STARTUP_M5_ENDPOINT_OWNER_MISMATCH'
...
$result = [ordered]@{
    schema='SpaceSyndicateStartupBoundarySelfTestV1'
    status=if($passCount -eq $cases.Count -and $cases.Count -ge 28){'PASS'}else{'FAIL'}
