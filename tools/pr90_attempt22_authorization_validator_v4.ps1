...
if([string]$sealObject.schema-cne'Pr90ListenerParityV2ToolingSealV1'-or[string]$sealObject.status-cne'SEALED'-or[string]$sealObject.tooling_head_sha-cne$ExpectedToolingHead-or[string]$sealObject.tooling_tree_sha-cne$ExpectedToolingTree-or[int]$sealObject.listener_parity_contract_version-ne2-or[string]$sealObject.listener_forensics_sha256-cne[string]$manifest.listener_forensics_sha256-or[string]$sealObject.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $sealObject)){$errors.Add('TOOLING_SEAL_CONTRACT_MISMATCH')}
if([string]$toolingManifestObject.schema-cne'Pr90ListenerParityV2ToolingManifestV1'-or[string]$toolingManifestObject.status-cne'READY'-or-not[bool]$toolingManifestObject.startup_probe_b_authorization_eligible-or[string]$toolingManifestObject.tooling_head_sha-cne$ExpectedToolingHead-or[string]$toolingManifestObject.tooling_tree_sha-cne$ExpectedToolingTree-or
   ...
   [string]$toolingManifestObject.tooling_file_hash_inventory_sha256-cne[string]$manifest.tooling_file_hash_inventory_sha256-or[string]$toolingManifestObject.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $toolingManifestObject)){$errors.Add('TOOLING_MANIFEST_CONTRACT_MISMATCH')}
$toolingFieldRows=@(
    ...
    @('probe_b_controller_sha256','tools/pr90_exact_clone_probe_b_controller_v1.ps1'),@('probe_b_result_builder_sha256','tools/pr90_exact_clone_probe_b_result_builder_v1.ps1'),@('probe_b_attestation_builder_sha256','tools/pr90_exact_clone_probe_b_attestation_builder_v1.ps1'),
...
$pairs=@(
    ...
    @('probe_b_result_path','probe_b_result_sha256'),@('probe_b_attestation_path','probe_b_attestation_sha256'),@('probe_b_finalizer_result_path','probe_b_finalizer_result_sha256'),@('preformal_dry_run_path','preformal_dry_run_sha256'),@('listener_forensics_path','listener_forensics_sha256'),
...
$probeB=$null;$probeBA=$null;$preformal=$null
...
$evidenceContracts=Test-Pr90Attempt22EvidenceContractsV1 -ProbeB $probeB -ProbeBAttestation $probeBA -Preformal $preformal -ExpectedProbeBResultSha256 ([string]$manifest.probe_b_result_sha256) `
    ...
    -ExpectedToolingSealSha256 ([string]$manifest.tooling_seal_sha256) -ExpectedGodotGuiSha256 ([string]$manifest.godot_executable_sha256) -ExpectedGodotConsoleSha256 ([string]$manifest.godot_console_sha256)
...
$result=[pscustomobject][ordered]@{schema='Pr90Attempt22AuthorizationValidationV4';status=if($errors.Count-eq0){'PASS'}else{'BLOCKED'};created_at_utc=[DateTimeOffset]::UtcNow.ToString('o');manifest_path=[IO.Path]::GetFullPath($ManifestPath);manifest_sha256=$observed;product_head_sha=$ExpectedProductHead;product_tree_sha=$ExpectedProductTree;tooling_head_sha=$ExpectedToolingHead;tooling_tree_sha=$ExpectedToolingTree;declared_field_count=$declaredFields.Count;validated_field_count=$validatedFieldCount;field_mismatch_count=$errors.Count;missing_field_count=@($errors|Where-Object{$_-like'MISSING_FIELD:*'}).Count;unexpected_field_count=@($errors|Where-Object{$_-like'UNEXPECTED_FIELD:*'}).Count;attempt19_compatibility_status=[string]$compatibility.status;authorization_consumed=$false;errors=@($errors);canonical_payload_sha256=''}
