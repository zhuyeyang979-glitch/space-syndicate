...
$baseHead='91e6e19d93262dbf910156ddee1c7b176c53acd4';$baseTree='d4230637387e7d57a46f48ed06261daad9a9ff45'
$sealedBaseManifestSha256='06055e13a4787973fccb351dc0c876d89ec6b8680b1d6fd26d6c70a6fca80e8d';$sealedBaseSealSha256='458da471587b8a4f562fa45b64c37f689b7f1b0ce9b1d318c9a06e1dac691947';$sealedBaseSelfTestSha256='14ed331e00380a77efeaa631161fcc59a322c044d2a8f8b4984d1f7a0c52ac79';$sealedFrozenInputSha256='3a064504628f4168d74b2558fa7473fbe3f00288e79deec5362184ce71f6e2d5'
if($ExpectedBaseToolingManifestSha256-cne$sealedBaseManifestSha256-or$ExpectedBaseToolingSealSha256-cne$sealedBaseSealSha256-or$ExpectedBaseSelfTestSha256-cne$sealedBaseSelfTestSha256-or$ExpectedFrozenInputInventorySha256-cne$sealedFrozenInputSha256){throw 'Frozen 91e6 authority hash arguments are not exact.'}
...
if([string]$baseManifest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingManifestV1'-or[string]$baseManifest.status-cne'READY'-or[string]$baseManifest.tooling_head_sha-cne$baseHead-or[string]$baseManifest.tooling_tree_sha-cne$baseTree-or[string]$baseManifest.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseManifest)-or
   [string]$baseSeal.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSealV1'-or[string]$baseSeal.status-cne'SEALED'-or[string]$baseSeal.tooling_head_sha-cne$baseHead-or[string]$baseSeal.tooling_tree_sha-cne$baseTree-or[string]$baseSeal.manifest_sha256-cne$ExpectedBaseToolingManifestSha256-or[string]$baseSeal.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseSeal)){throw 'Frozen 91e6 Tooling authority contract mismatch.'}
if([string]$baseSelfTest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSelfTestV1'-or[string]$baseSelfTest.selftest_revision-cne'PR90_ATTEMPT22_FORMAL_AUTHORITY_REPAIR_V1'-or[string]$baseSelfTest.status-cne'PASS'-or[int]$baseSelfTest.new_selftest_case_count-ne125-or[int]$baseSelfTest.total_tooling_selftest_pass_count-ne451-or[int]$baseSelfTest.total_tooling_selftest_failure_count-ne0-or
   (Get-Pr90ProbeBSha256 $BaseSelfTestPath)-cne[string]$baseManifest.new_selftest_sha256){throw 'Frozen 91e6 Tooling self-test is not the exact 125-case V1 receipt.'}
if(-not(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest)-or[int]$newSelfTest.new_selftest_case_count-ne152-or
   [string]$newSelfTest.selftest_case_name_inventory_sha256-cne'b55910a6d3429acbbda25eb3fd768c4b39dcda9ca1470cdb62e556f7c9139eaa'){throw 'Attempt22 baseline identity split self-test version or exact case inventory mismatch.'}
...
if([string]$frozenInput.schema-cne'Pr90ProbeBV2FrozenInputInventoryV1'-or[string]$frozenInput.status-cne'FROZEN'-or[string]$frozenInput.tooling_head_sha-cne[string]$baseManifest.base_tooling_head_sha-or[string]$frozenInput.tooling_tree_sha-cne[string]$baseManifest.base_tooling_tree_sha-or
...
$authorizedRuntimeChanges=@('tools/pr90_probe_b_attempt22_contract_v1.psm1')
...
$eligible=($scopeErrors.Count-eq0-and$worktreeBlobMismatch-eq0-and$runtimeMismatch.Count-eq0-and$authorizedRuntimeChangeCount-eq1-and$productCodeChangeCount-eq0-and$productTestChangeCount-eq0-and[int]$newSelfTest.new_selftest_case_count-eq152-and[string]$newSelfTest.selftest_case_name_inventory_sha256-ceq'b55910a6d3429acbbda25eb3fd768c4b39dcda9ca1470cdb62e556f7c9139eaa'-and(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest))
