...
if([string]$baseManifest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingManifestV1'-or[string]$baseManifest.status-cne'READY'-or[string]$baseManifest.tooling_head_sha-cne$baseHead-or[string]$baseManifest.tooling_tree_sha-cne$baseTree-or[string]$baseManifest.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseManifest)-or
   [string]$baseSeal.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSealV1'-or[string]$baseSeal.status-cne'SEALED'-or[string]$baseSeal.tooling_head_sha-cne$baseHead-or[string]$baseSeal.tooling_tree_sha-cne$baseTree-or[string]$baseSeal.manifest_sha256-cne$ExpectedBaseToolingManifestSha256-or[string]$baseSeal.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseSeal)){throw 'Frozen 70ccb5c0 Tooling authority contract mismatch.'}
if([string]$baseSelfTest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSelfTestV1'-or[string]$baseSelfTest.selftest_revision-cne'PR90_ATTEMPT22_MAIN_RUNTIME_STREAM_REBIND_REPAIR_V5'-or[string]$baseSelfTest.status-cne'PASS'-or[int]$baseSelfTest.new_selftest_case_count-ne199-or[int]$baseSelfTest.total_tooling_selftest_pass_count-ne525-or[int]$baseSelfTest.total_tooling_selftest_failure_count-ne0-or
   (Get-Pr90ProbeBSha256 $BaseSelfTestPath)-cne[string]$baseManifest.new_selftest_sha256){throw 'Frozen 70ccb5c0 Tooling self-test is not the exact 199-case V5 receipt.'}
if(-not(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest)-or[int]$newSelfTest.new_selftest_case_count-ne220-or
   [string]$newSelfTest.selftest_case_name_inventory_sha256-cne'fe6c1d82c5bff6378783226718915eb2d8f42996f69dcf44948399755ea219f6'){throw 'Attempt22 canonical import V2 owner repair self-test version or exact case inventory mismatch.'}
...
$added=@('tools/release_harness/pr90_canonical_import_authority_v3.ps1')
...
$purposeByPath=@{
 ...
 'tools/pr90_probe_b_attempt22_contract_v1.psm1'='current Attempt22 schemas, canonical import V2 owner identity, and fail-closed predicates'
 'tools/pr90_probe_b_attempt22_selftest_v1.ps1'='versioned canonical import owner self-test and stale-report rejection'
 'tools/pr90_probe_b_attempt22_tooling_manifest_builder_v1.ps1'='exact 4M/1A Tooling inventory authority'
 'tools/pr90_probe_b_attempt22_tooling_seal_builder_v1.ps1'='post-commit Tooling seal'
 'tools/release_harness/pr90_canonical_import_authority_v3.ps1'='single canonical import harness reusing sealed launcher, V2 endpoint owner identity, and scoped cleanup'
}
...
$eligible=($scopeErrors.Count-eq0-and$worktreeBlobMismatch-eq0-and$runtimeMismatch.Count-eq0-and$authorizedRuntimeChangeCount-eq2-and$productCodeChangeCount-eq0-and$productTestChangeCount-eq0-and[int]$newSelfTest.new_selftest_case_count-eq220-and[string]$newSelfTest.selftest_case_name_inventory_sha256-ceq'fe6c1d82c5bff6378783226718915eb2d8f42996f69dcf44948399755ea219f6'-and(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest))
