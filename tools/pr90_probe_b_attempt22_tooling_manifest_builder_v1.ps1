...
if([string]$baseManifest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingManifestV1'-or[string]$baseManifest.status-cne'READY'-or[string]$baseManifest.tooling_head_sha-cne$baseHead-or[string]$baseManifest.tooling_tree_sha-cne$baseTree-or[string]$baseManifest.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseManifest)-or
   [string]$baseSeal.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSealV1'-or[string]$baseSeal.status-cne'SEALED'-or[string]$baseSeal.tooling_head_sha-cne$baseHead-or[string]$baseSeal.tooling_tree_sha-cne$baseTree-or[string]$baseSeal.manifest_sha256-cne$ExpectedBaseToolingManifestSha256-or[string]$baseSeal.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseSeal)){throw 'Frozen b3fb0ad0 Tooling authority contract mismatch.'}
if([string]$baseSelfTest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSelfTestV1'-or[string]$baseSelfTest.selftest_revision-cne'PR90_ATTEMPT22_CANONICAL_IMPORT_PIPELINE_LIFECYCLE_REPAIR_V8'-or[string]$baseSelfTest.status-cne'PASS'-or[int]$baseSelfTest.new_selftest_case_count-ne226-or[int]$baseSelfTest.total_tooling_selftest_pass_count-ne552-or[int]$baseSelfTest.total_tooling_selftest_failure_count-ne0-or
   (Get-Pr90ProbeBSha256 $BaseSelfTestPath)-cne[string]$baseManifest.new_selftest_sha256){throw 'Frozen b3fb0ad0 Tooling self-test is not the exact 226-case V8 receipt.'}
if(-not(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest)-or[int]$newSelfTest.new_selftest_case_count-ne228-or
   [string]$newSelfTest.selftest_case_name_inventory_sha256-cne'7b410b0d081ba72d888642a1bb7b536e1242b6fdea3a5034c1b5af126f56eae4'){throw 'Attempt22 canonical import process-start identity repair self-test version or exact case inventory mismatch.'}
...
$modified=@(
 ...
 'tools/release_harness/pr90_canonical_import_authority_v3.ps1'
)
$added=@()
...
$purposeByPath=@{
 ...
 'tools/pr90_probe_b_attempt22_contract_v1.psm1'='current Attempt22 schemas, canonical import typed process-start identity, and fail-closed predicates'
 'tools/pr90_probe_b_attempt22_selftest_v1.ps1'='versioned canonical import process-start identity self-test and stale-report rejection'
 ...
 'tools/release_harness/pr90_canonical_import_authority_v3.ps1'='single canonical import harness with typed process-start identity, exact launcher-PID wait, V2 endpoint owner identity, and scoped cleanup'
}
...
$authorizedRuntimeChanges=@('tools/pr90_probe_b_attempt22_contract_v1.psm1','tools/release_harness/pr90_canonical_import_authority_v3.ps1')
...
$eligible=($scopeErrors.Count-eq0-and$worktreeBlobMismatch-eq0-and$runtimeMismatch.Count-eq0-and$authorizedRuntimeChangeCount-eq2-and$productCodeChangeCount-eq0-and$productTestChangeCount-eq0-and[int]$newSelfTest.new_selftest_case_count-eq228-and[string]$newSelfTest.selftest_case_name_inventory_sha256-ceq'7b410b0d081ba72d888642a1bb7b536e1242b6fdea3a5034c1b5af126f56eae4'-and(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest))
