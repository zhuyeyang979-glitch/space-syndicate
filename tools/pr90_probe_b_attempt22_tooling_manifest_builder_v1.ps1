...
if([string]$baseManifest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingManifestV1'-or[string]$baseManifest.status-cne'READY'-or[string]$baseManifest.tooling_head_sha-cne$baseHead-or[string]$baseManifest.tooling_tree_sha-cne$baseTree-or[string]$baseManifest.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseManifest)-or
   [string]$baseSeal.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSealV1'-or[string]$baseSeal.status-cne'SEALED'-or[string]$baseSeal.tooling_head_sha-cne$baseHead-or[string]$baseSeal.tooling_tree_sha-cne$baseTree-or[string]$baseSeal.manifest_sha256-cne$ExpectedBaseToolingManifestSha256-or[string]$baseSeal.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseSeal)){throw 'Frozen c1d868df Tooling authority contract mismatch.'}
if([string]$baseSelfTest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSelfTestV1'-or[string]$baseSelfTest.selftest_revision-cne'PR90_ATTEMPT22_FORMAL_TAP_CENTER_VIEWPORT_REPAIR_V18'-or[string]$baseSelfTest.status-cne'PASS'-or[int]$baseSelfTest.new_selftest_case_count-ne264-or[int]$baseSelfTest.total_tooling_selftest_pass_count-ne590-or[int]$baseSelfTest.total_tooling_selftest_failure_count-ne0-or
   (Get-Pr90ProbeBSha256 $BaseSelfTestPath)-cne[string]$baseManifest.new_selftest_sha256){throw 'Frozen c1d868df Tooling self-test is not the exact 264-case V18 receipt.'}
if(-not(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest)-or[int]$newSelfTest.new_selftest_case_count-ne264-or
   [string]$newSelfTest.selftest_case_name_inventory_sha256-cne'a5c5dcca0b334e2ba77a3a2a5fa7e1dab048bbafd0cfdd548b2c79bfbfc93d60'){throw 'Attempt22 formal ephemeral UI rebind repair self-test version or exact case inventory mismatch.'}
...
$purposeByPath=@{
 ...
 'tools/pr90_attempt21_cursor_aware_exact_mcp_v5.ps1'='formal orchestration with stable-parent ephemeral UI rebinding, bounded viewport navigation, public UI combat selection, scoped cleanup, and terminal evidence'
 ...
 'tools/pr90_probe_b_attempt22_contract_v1.psm1'='current Attempt22 schemas, exact transient-node classifier, stable-parent UI rebind ledger, combat evidence binding, and fail-closed validation'
 'tools/pr90_probe_b_attempt22_selftest_v1.ps1'='versioned ephemeral UI churn and exact missing-response self-test with combat-chain and stale-report rejection'
