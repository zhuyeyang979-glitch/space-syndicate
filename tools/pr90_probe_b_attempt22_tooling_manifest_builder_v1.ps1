...
foreach($sidecarPath in @($BaseToolingManifestPath,$BaseToolingSealPath,$BaseSelfTestPath,$NewSelfTestPath,$FrozenInputInventoryPath)){if(-not(Test-Pr90ProbeBShaSidecar $sidecarPath "$sidecarPath.sha256")){throw "Authority evidence sidecar mismatch: $sidecarPath"}}
...
if([string]$baseManifest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingManifestV1'-or[string]$baseManifest.status-cne'READY'-or[string]$baseManifest.tooling_head_sha-cne$baseHead-or[string]$baseManifest.tooling_tree_sha-cne$baseTree-or[string]$baseManifest.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseManifest)-or
   [string]$baseSeal.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSealV1'-or[string]$baseSeal.status-cne'SEALED'-or[string]$baseSeal.tooling_head_sha-cne$baseHead-or[string]$baseSeal.tooling_tree_sha-cne$baseTree-or[string]$baseSeal.manifest_sha256-cne$ExpectedBaseToolingManifestSha256-or[string]$baseSeal.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseSeal)){throw 'Frozen c28dec0a Tooling authority contract mismatch.'}
if([string]$baseSelfTest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSelfTestV1'-or[string]$baseSelfTest.selftest_revision-cne'PR90_ATTEMPT22_CANONICAL_IMPORT_PROCESS_START_IDENTITY_REPAIR_V9'-or[string]$baseSelfTest.status-cne'PASS'-or[int]$baseSelfTest.new_selftest_case_count-ne228-or[int]$baseSelfTest.total_tooling_selftest_pass_count-ne554-or[int]$baseSelfTest.total_tooling_selftest_failure_count-ne0-or
   (Get-Pr90ProbeBSha256 $BaseSelfTestPath)-cne[string]$baseManifest.new_selftest_sha256){throw 'Frozen c28dec0a Tooling self-test is not the exact 228-case V9 receipt.'}
if(-not(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest)-or[int]$newSelfTest.new_selftest_case_count-ne232-or
   [string]$newSelfTest.selftest_case_name_inventory_sha256-cne'e725a7f9f92e2471efccc74b9faee9ff5097acaafc2011a5ebf207d75bce4f4c'){throw 'Attempt22 Godot GUI/console identity repair self-test version or exact case inventory mismatch.'}
...
$added=@()
$formalReachable=@('tools/invoke_role_godot_mcp.ps1','tools/pr90_attempt21_cursor_aware_exact_mcp_v5.ps1','tools/pr90_attempt21_mcp_startup_contract.psm1','tools/pr90_mcp_startup_state_machine_v1.psm1','tools/pr90_probe_b_attempt22_contract_v1.psm1','tools/release_harness/pr90_canonical_import_authority_v3.ps1','tools/role_godot_mcp_process_identity.psm1')
$purposeByPath=@{
 'tools/invoke_role_godot_mcp.ps1'='V2 control and GUI endpoint identity-bound MCP invocation'
 'tools/pr90_attempt21_cursor_aware_exact_mcp_v5.ps1'='formal orchestration with explicit GUI/console seal identities, main-runtime stream rebind, request evidence, scoped cleanup, finalizer, and terminal evidence'
 'tools/pr90_attempt21_mcp_startup_contract.psm1'='startup snapshots, canonical state, and explicit process identity'
 'tools/pr90_attempt22_authorization_manifest_builder_v4.ps1'='Attempt22 current and historical authority projection with distinct GUI and console identities'
 'tools/pr90_attempt22_authorization_seal_builder_v4.ps1'='Attempt22 immutable authorization seal with distinct GUI and console identities'
 'tools/pr90_attempt22_authorization_validator_v4.ps1'='production-isomorphic cross-evidence validation for distinct GUI and console identities'
 ...
 'tools/pr90_probe_b_attempt22_contract_v1.psm1'='current Attempt22 schemas, distinct Godot GUI and console identity, and fail-closed predicates'
 'tools/pr90_probe_b_attempt22_selftest_v1.ps1'='versioned Godot GUI/console identity self-test and stale-report rejection'
 'tools/pr90_probe_b_attempt22_tooling_manifest_builder_v1.ps1'='exact 8M/0A Tooling inventory authority'
 'tools/pr90_probe_b_attempt22_tooling_seal_builder_v1.ps1'='post-commit Tooling seal'
 'tools/release_harness/pr90_canonical_import_authority_v3.ps1'='unchanged inherited canonical import harness with typed process-start identity, exact launcher-PID wait, V2 endpoint owner identity, and scoped cleanup'
}
...
$runtimeReachable=@('tools/launch_role_godot_mcp.ps1','tools/stop_role_godot_mcp.ps1','tools/invoke_role_godot_mcp.ps1','tools/pr90_attempt19_import_controller_v3.ps1','tools/pr90_attempt19_import_runner_v3.ps1','tools/pr90_attempt19_import_finalizer_dry_run.ps1','tools/pr90_attempt21_mcp_startup_probe.ps1','tools/pr90_attempt21_mcp_startup_watchdog.ps1','tools/pr90_mcp_startup_state_machine_v1.psm1','tools/pr90_attempt21_mcp_startup_contract.psm1','tools/pr90_getnettcp_listener_adapter_v1.psm1','tools/pr90_netstat_listener_adapter_v1.psm1','tools/pr90_endpoint_listener_core_v2.psm1','tools/pr90_listener_bracketed_cohort_v2.psm1','tools/pr90_listener_process_identity_reader_v1.psm1','tools/pr90_mcp_endpoint_ownership_v2.psm1','tools/pr90_m5_listener_parity_v2_contract.psm1','tools/pr90_attempt21_cursor_aware_exact_mcp_v5.ps1','tools/pr90_probe_b_import_finalizer_binding_v1.ps1','tools/pr90_probe_b_attempt22_contract_v1.psm1','tools/release_harness/pr90_canonical_import_authority_v3.ps1','tools/role_godot_mcp_process_identity.psm1')
...
foreach($relative in $runtimeReachable){$baseRow=@($baseManifest.tooling_files|Where-Object{([string]$_.relative_path).Replace('\','/')-ceq$relative});$newRow=@($rows|Where-Object{[string]$_.relative_path-ceq$relative});if($newRow.Count-ne1){$runtimeMismatch.Add($relative);continue};if($baseRow.Count-eq0-and$added-ccontains$relative-and$authorizedRuntimeChanges-ccontains$relative){$authorizedRuntimeChangeCount+=1;continue};if($baseRow.Count-ne1){$runtimeMismatch.Add($relative);continue};$changed=([string]$baseRow[0].sha256-cne[string]$newRow[0].sha256);if($authorizedRuntimeChanges-ccontains$relative){if($changed){$authorizedRuntimeChangeCount+=1}else{$runtimeMismatch.Add("AUTHORIZED_RUNTIME_NOT_CHANGED:$relative")}}elseif($changed){$runtimeMismatch.Add($relative)}}
...
$eligible=($scopeErrors.Count-eq0-and$worktreeBlobMismatch-eq0-and$runtimeMismatch.Count-eq0-and$authorizedRuntimeChangeCount-eq2-and$productCodeChangeCount-eq0-and$productTestChangeCount-eq0-and[int]$newSelfTest.new_selftest_case_count-eq232-and[string]$newSelfTest.selftest_case_name_inventory_sha256-ceq'e725a7f9f92e2471efccc74b9faee9ff5097acaafc2011a5ebf207d75bce4f4c'-and(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest))
...
$manifest=[pscustomobject][ordered]@{
 ...
 frozen_input_inventory_path=[IO.Path]::GetFullPath($FrozenInputInventoryPath);frozen_input_inventory_sha256=Get-Pr90ProbeBSha256 $FrozenInputInventoryPath;frozen_input_tooling_head_sha=[string]$frozenInput.tooling_head_sha;frozen_input_tooling_tree_sha=[string]$frozenInput.tooling_tree_sha;frozen_input_count=[int]$frozenInput.input_count;frozen_input_hash_inventory_sha256=[string]$frozenInput.input_inventory_sha256
