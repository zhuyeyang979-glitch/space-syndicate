...
if([string]$baseManifest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingManifestV1'-or[string]$baseManifest.status-cne'READY'-or[string]$baseManifest.tooling_head_sha-cne$baseHead-or[string]$baseManifest.tooling_tree_sha-cne$baseTree-or[string]$baseManifest.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseManifest)-or
   [string]$baseSeal.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSealV1'-or[string]$baseSeal.status-cne'SEALED'-or[string]$baseSeal.tooling_head_sha-cne$baseHead-or[string]$baseSeal.tooling_tree_sha-cne$baseTree-or[string]$baseSeal.manifest_sha256-cne$ExpectedBaseToolingManifestSha256-or[string]$baseSeal.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseSeal)){throw 'Frozen 803e4fc5 Tooling authority contract mismatch.'}
if([string]$baseSelfTest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSelfTestV1'-or[string]$baseSelfTest.selftest_revision-cne'PR90_ATTEMPT22_CANONICAL_IMPORT_MODULE_BINDING_REPAIR_V7'-or[string]$baseSelfTest.status-cne'PASS'-or[int]$baseSelfTest.new_selftest_case_count-ne224-or[int]$baseSelfTest.total_tooling_selftest_pass_count-ne550-or[int]$baseSelfTest.total_tooling_selftest_failure_count-ne0-or
   (Get-Pr90ProbeBSha256 $BaseSelfTestPath)-cne[string]$baseManifest.new_selftest_sha256){throw 'Frozen 803e4fc5 Tooling self-test is not the exact 224-case V7 receipt.'}
if(-not(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest)-or[int]$newSelfTest.new_selftest_case_count-ne226-or
   [string]$newSelfTest.selftest_case_name_inventory_sha256-cne'30c4cfa67808fbc7c12c2862ad32abdbe53643603677b17bf19058550b700d44'){throw 'Attempt22 canonical import pipeline-lifecycle repair self-test version or exact case inventory mismatch.'}
...
$added=@()
$formalReachable=@('tools/invoke_role_godot_mcp.ps1','tools/pr90_attempt21_cursor_aware_exact_mcp_v5.ps1','tools/pr90_attempt21_mcp_startup_contract.psm1','tools/pr90_mcp_startup_state_machine_v1.psm1','tools/pr90_probe_b_attempt22_contract_v1.psm1','tools/release_harness/pr90_canonical_import_authority_v3.ps1')
$purposeByPath=@{
 ...
 'tools/pr90_probe_b_attempt22_contract_v1.psm1'='current Attempt22 schemas, canonical import V2 owner identity, bounded launcher lifecycle, and fail-closed predicates'
 'tools/pr90_probe_b_attempt22_selftest_v1.ps1'='versioned canonical import pipeline-lifecycle self-test and stale-report rejection'
 ...
 'tools/release_harness/pr90_canonical_import_authority_v3.ps1'='single canonical import harness with exact launcher-PID wait, V2 endpoint owner identity, and scoped cleanup'
}
...
$runtimeReachable=@('tools/launch_role_godot_mcp.ps1','tools/stop_role_godot_mcp.ps1','tools/invoke_role_godot_mcp.ps1','tools/pr90_attempt19_import_controller_v3.ps1','tools/pr90_attempt19_import_runner_v3.ps1','tools/pr90_attempt19_import_finalizer_dry_run.ps1','tools/pr90_attempt21_mcp_startup_probe.ps1','tools/pr90_attempt21_mcp_startup_watchdog.ps1','tools/pr90_mcp_startup_state_machine_v1.psm1','tools/pr90_attempt21_mcp_startup_contract.psm1','tools/pr90_getnettcp_listener_adapter_v1.psm1','tools/pr90_netstat_listener_adapter_v1.psm1','tools/pr90_endpoint_listener_core_v2.psm1','tools/pr90_listener_bracketed_cohort_v2.psm1','tools/pr90_listener_process_identity_reader_v1.psm1','tools/pr90_mcp_endpoint_ownership_v2.psm1','tools/pr90_m5_listener_parity_v2_contract.psm1','tools/pr90_attempt21_cursor_aware_exact_mcp_v5.ps1','tools/pr90_probe_b_import_finalizer_binding_v1.ps1','tools/pr90_probe_b_attempt22_contract_v1.psm1','tools/release_harness/pr90_canonical_import_authority_v3.ps1')
...
$eligible=($scopeErrors.Count-eq0-and$worktreeBlobMismatch-eq0-and$runtimeMismatch.Count-eq0-and$authorizedRuntimeChangeCount-eq2-and$productCodeChangeCount-eq0-and$productTestChangeCount-eq0-and[int]$newSelfTest.new_selftest_case_count-eq226-and[string]$newSelfTest.selftest_case_name_inventory_sha256-ceq'30c4cfa67808fbc7c12c2862ad32abdbe53643603677b17bf19058550b700d44'-and(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest))
