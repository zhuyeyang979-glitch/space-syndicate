...
if([string]$baseManifest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingManifestV1'-or[string]$baseManifest.status-cne'READY'-or[string]$baseManifest.tooling_head_sha-cne$baseHead-or[string]$baseManifest.tooling_tree_sha-cne$baseTree-or[string]$baseManifest.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseManifest)-or
   [string]$baseSeal.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSealV1'-or[string]$baseSeal.status-cne'SEALED'-or[string]$baseSeal.tooling_head_sha-cne$baseHead-or[string]$baseSeal.tooling_tree_sha-cne$baseTree-or[string]$baseSeal.manifest_sha256-cne$ExpectedBaseToolingManifestSha256-or[string]$baseSeal.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseSeal)){throw 'Frozen 913196d9 Tooling authority contract mismatch.'}
if([string]$baseSelfTest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSelfTestV1'-or[string]$baseSelfTest.selftest_revision-cne'PR90_ATTEMPT22_GODOT_GUI_CONSOLE_IDENTITY_REPAIR_V10'-or[string]$baseSelfTest.status-cne'PASS'-or[int]$baseSelfTest.new_selftest_case_count-ne232-or[int]$baseSelfTest.total_tooling_selftest_pass_count-ne558-or[int]$baseSelfTest.total_tooling_selftest_failure_count-ne0-or
   (Get-Pr90ProbeBSha256 $BaseSelfTestPath)-cne[string]$baseManifest.new_selftest_sha256){throw 'Frozen 913196d9 Tooling self-test is not the exact 232-case V10 receipt.'}
if(-not(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest)-or[int]$newSelfTest.new_selftest_case_count-ne233-or
   [string]$newSelfTest.selftest_case_name_inventory_sha256-cne'809972131a00b9aa2e2fefb524f6bcf350999e09103906cf97e746b417454409'){throw 'Attempt22 finalizer console identity repair self-test version or exact case inventory mismatch.'}
...
$purposeByPath=@{
 ...
 'tools/pr90_attempt21_cursor_aware_exact_mcp_v5.ps1'='formal orchestration with explicit GUI/console seal identities, console-bound finalizer, main-runtime stream rebind, request evidence, scoped cleanup, and terminal evidence'
 ...
 'tools/pr90_probe_b_attempt22_contract_v1.psm1'='current Attempt22 schemas, finalizer console identity, and fail-closed predicates'
 'tools/pr90_probe_b_attempt22_selftest_v1.ps1'='versioned finalizer console identity self-test and stale-report rejection'
...
$eligible=($scopeErrors.Count-eq0-and$worktreeBlobMismatch-eq0-and$runtimeMismatch.Count-eq0-and$authorizedRuntimeChangeCount-eq2-and$productCodeChangeCount-eq0-and$productTestChangeCount-eq0-and[int]$newSelfTest.new_selftest_case_count-eq233-and[string]$newSelfTest.selftest_case_name_inventory_sha256-ceq'809972131a00b9aa2e2fefb524f6bcf350999e09103906cf97e746b417454409'-and(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest))
