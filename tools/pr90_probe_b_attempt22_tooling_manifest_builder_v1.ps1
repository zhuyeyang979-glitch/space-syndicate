...
if([string]$baseManifest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingManifestV1'-or[string]$baseManifest.status-cne'READY'-or[string]$baseManifest.tooling_head_sha-cne$baseHead-or[string]$baseManifest.tooling_tree_sha-cne$baseTree-or[string]$baseManifest.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseManifest)-or
   [string]$baseSeal.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSealV1'-or[string]$baseSeal.status-cne'SEALED'-or[string]$baseSeal.tooling_head_sha-cne$baseHead-or[string]$baseSeal.tooling_tree_sha-cne$baseTree-or[string]$baseSeal.manifest_sha256-cne$ExpectedBaseToolingManifestSha256-or[string]$baseSeal.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseSeal)){throw 'Frozen 74244df3 Tooling authority contract mismatch.'}
if([string]$baseSelfTest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSelfTestV1'-or[string]$baseSelfTest.selftest_revision-cne'PR90_ATTEMPT22_FORMAL_BASELINE_IDENTITY_SPLIT_V2'-or[string]$baseSelfTest.status-cne'PASS'-or[int]$baseSelfTest.new_selftest_case_count-ne154-or[int]$baseSelfTest.total_tooling_selftest_pass_count-ne480-or[int]$baseSelfTest.total_tooling_selftest_failure_count-ne0-or
   (Get-Pr90ProbeBSha256 $BaseSelfTestPath)-cne[string]$baseManifest.new_selftest_sha256){throw 'Frozen 74244df3 Tooling self-test is not the exact 154-case V2 receipt.'}
if(-not(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest)-or[int]$newSelfTest.new_selftest_case_count-ne164-or
   [string]$newSelfTest.selftest_case_name_inventory_sha256-cne'10021f9a1f6c3195b49b1951a9d1ed5acc92f027b7134aa6577cf2f296519e84'){throw 'Attempt22 watchdog classifier self-test version or exact case inventory mismatch.'}
...
$eligible=($scopeErrors.Count-eq0-and$worktreeBlobMismatch-eq0-and$runtimeMismatch.Count-eq0-and$authorizedRuntimeChangeCount-eq2-and$productCodeChangeCount-eq0-and$productTestChangeCount-eq0-and[int]$newSelfTest.new_selftest_case_count-eq164-and[string]$newSelfTest.selftest_case_name_inventory_sha256-ceq'10021f9a1f6c3195b49b1951a9d1ed5acc92f027b7134aa6577cf2f296519e84'-and(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest))
