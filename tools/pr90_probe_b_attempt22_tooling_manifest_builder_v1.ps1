...
if([string]$baseManifest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingManifestV1'-or[string]$baseManifest.status-cne'READY'-or[string]$baseManifest.tooling_head_sha-cne$baseHead-or[string]$baseManifest.tooling_tree_sha-cne$baseTree-or[string]$baseManifest.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseManifest)-or
   [string]$baseSeal.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSealV1'-or[string]$baseSeal.status-cne'SEALED'-or[string]$baseSeal.tooling_head_sha-cne$baseHead-or[string]$baseSeal.tooling_tree_sha-cne$baseTree-or[string]$baseSeal.manifest_sha256-cne$ExpectedBaseToolingManifestSha256-or[string]$baseSeal.canonical_payload_sha256-cne(Get-Pr90ProbeBCanonicalSha256 $baseSeal)){throw 'Frozen 70c459f5 Tooling authority contract mismatch.'}
if([string]$baseSelfTest.schema-cne'Pr90ProbeBV2ResultRecoveryToolingSelfTestV1'-or[string]$baseSelfTest.selftest_revision-cne'PR90_ATTEMPT22_COMMAND_STATE_HEARTBEAT_REPAIR_V13'-or[string]$baseSelfTest.status-cne'PASS'-or[int]$baseSelfTest.new_selftest_case_count-ne244-or[int]$baseSelfTest.total_tooling_selftest_pass_count-ne570-or[int]$baseSelfTest.total_tooling_selftest_failure_count-ne0-or
   (Get-Pr90ProbeBSha256 $BaseSelfTestPath)-cne[string]$baseManifest.new_selftest_sha256){throw 'Frozen 70c459f5 Tooling self-test is not the exact 244-case V13 receipt.'}
if(-not(Test-Pr90Attempt22FormalRepairSelfTestReceiptV1 $newSelfTest)-or[int]$newSelfTest.new_selftest_case_count-ne260-or
   [string]$newSelfTest.selftest_case_name_inventory_sha256-cne'8041c92e4a0bf96b5ba7d4618bbeb6e3a3fc6ae645a5e24dd04bd0234093888a'){throw 'Attempt22 formal combat scenario coverage repair self-test version or exact case inventory mismatch.'}
...
$purposeByPath=@{
 ...
 'tools/pr90_attempt21_cursor_aware_exact_mcp_v5.ps1'='formal orchestration with explicit GUI/console seal identities, real UI combat scenario coverage, exact presentation receipts, scoped cleanup, and terminal evidence'
 ...
 'tools/pr90_probe_b_attempt22_contract_v1.psm1'='current Attempt22 schemas, combat scenario coverage classification, and fail-closed predicates'
 'tools/pr90_probe_b_attempt22_selftest_v1.ps1'='versioned combat scenario and presentation-chain self-test with stale-report rejection'
