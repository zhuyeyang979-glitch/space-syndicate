...
$terminal = [ordered]@{
    ...
    forced_stop=if($null-ne$summary){[bool]$summary.forced_stop}else{$false};
    forced_stop_process_ids=if($null-ne$summary){@($summary.forced_stop_process_ids)}else{@()};
    unrelated_process_termination_count=if($null-ne$summary){[int]$summary.unrelated_process_termination_count}else{0};
    stopped_cleanly=($taskOwnedRows.Count-eq0-and$terminalGodot.Count-eq0-and$terminalListeners.Count-eq0-and$null-ne$summary-and[bool]$summary.stops_cleanly);
    canonical_payload_sha256=''
}
