...
public static class Generation9MemoryStatus {
...
$unexpectedUntrackedBefore = @($untrackedBefore | Where-Object {
    ...
    $path -notlike "*.uid" -and
        $path -notlike "reports/reuse/generation9_platform_qualification/canonical_import_compatibility_correction_001/*"
})
...
if ($PassId -eq "pass-001") {
    ...
    $pass1Report = Join-Path $root "reports/reuse/generation9_platform_qualification/canonical_import_compatibility_correction_001/pass-001/canonical_import_report.json"
...
if ($godotVersion -notmatch '^4\.7\.') {
    throw "Generation 9 is pinned to Godot 4.7.x; observed: $godotVersion"
}

$externalRoot = Join-Path "C:\Users\Administrator\Documents\Codex\2026-08-20\qu\outputs\v076-generation9-platform-qualification\canonical-import-compatibility-correction-001" $PassId
...
$roamingRoot = Join-Path $roleLocalRoot "canonical-import-compatibility-correction-001-$PassId-appdata-roaming"
$localAppDataRoot = Join-Path $roleLocalRoot "canonical-import-compatibility-correction-001-$PassId-appdata-local"
...
$startCapacity = [Generation9MemoryStatus+MEMORYSTATUSEX]::new()
if (-not [Generation9MemoryStatus]::GlobalMemoryStatusEx($startCapacity)) {
...
$process = Start-Process -FilePath $godotPath -ArgumentList $arguments -Environment @{
    APPDATA = $roamingRoot
...
while (-not $process.HasExited) {
    ...
    $memory = [Generation9MemoryStatus+MEMORYSTATUSEX]::new()
    if (-not [Generation9MemoryStatus]::GlobalMemoryStatusEx($memory)) {
...
$report = [ordered]@{
    schema_version = "space_syndicate.v076.generation9_canonical_import_compatibility_pass.v2"
    authorization_id = "USER_AUTHORIZATION_V076_PLATFORM_QUALIFIED_GENERATION9_20260901"
    pass_id = $PassId
    ...
    correction_id = "canonical-import-headless-mode-correction-001"
...
$repoOutputRoot = Join-Path $root "reports/reuse/generation9_platform_qualification/canonical_import_compatibility_correction_001/$PassId"
