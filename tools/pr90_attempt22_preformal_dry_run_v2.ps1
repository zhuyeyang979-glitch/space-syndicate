...
if($toolSealGreen){$seal=Get-Content -Raw -LiteralPath $config.tooling_seal_path|ConvertFrom-Json -Depth 100;$toolSealGreen=[string]$seal.schema-ceq'Pr90ProbeBV2ResultRecoveryToolingSealV1'-and[string]$seal.status-ceq'SEALED'-and[string]$seal.tooling_head_sha-ceq$toolingHead-and[string]$seal.tooling_tree_sha-ceq$toolingTree-and[string]$seal.manifest_sha256-ceq[string]$config.tooling_manifest_sha256}
...
if($toolManifestGreen){$tm=Get-Content -Raw -LiteralPath $config.tooling_manifest_path|ConvertFrom-Json -Depth 100;$toolManifestGreen=[string]$tm.schema-ceq'Pr90ProbeBV2ResultRecoveryToolingManifestV1'-and[string]$tm.status-ceq'READY'-and[string]$tm.tooling_head_sha-ceq$toolingHead-and[string]$tm.tooling_tree_sha-ceq$toolingTree-and[bool]$tm.preformal_authorization_eligible-and-not[bool]$tm.startup_probe_b_authorization_eligible-and[int]$tm.tooling_file_hash_mismatch_count-eq0-and[int]$tm.runtime_reachable_tooling_hash_mismatch_count-eq0-and-not[string]::IsNullOrWhiteSpace([string]$tm.probe_b_recovery_controller_sha256)-and-not[string]::IsNullOrWhiteSpace([string]$tm.probe_b_recovery_contract_module_sha256)}
...
$recoveryReceiptGreen=Test-HashBinding $config.probe_b_recovery_receipt_path $config.probe_b_recovery_receipt_sha256
if($recoveryReceiptGreen){$recoveryReceipt=Get-Content -Raw -LiteralPath $config.probe_b_recovery_receipt_path|ConvertFrom-Json -Depth 100;$recoveryReceiptGreen=[string]$recoveryReceipt.schema-ceq'Pr90ProbeBV2ResultRecoveryReceiptV1'-and[string]$recoveryReceipt.status-ceq'PASS'-and[string]$recoveryReceipt.result_sha256-ceq[string]$config.probe_b_result_sha256-and[string]$recoveryReceipt.attestation_sha256-ceq[string]$config.probe_b_attestation_sha256-and
    [string]$recoveryReceipt.probe_execution_tooling_head_sha-ceq[string]$config.probe_b_execution_tooling_head_sha-and[string]$recoveryReceipt.probe_execution_tooling_tree_sha-ceq[string]$config.probe_b_execution_tooling_tree_sha-and[string]$recoveryReceipt.probe_execution_tooling_seal_sha256-ceq[string]$config.probe_b_execution_tooling_seal_sha256-and
    [string]$recoveryReceipt.result_recovery_tooling_head_sha-ceq[string]$config.probe_b_recovery_tooling_head_sha-and[string]$recoveryReceipt.result_recovery_tooling_tree_sha-ceq[string]$config.probe_b_recovery_tooling_tree_sha-and[string]$recoveryReceipt.result_recovery_tooling_manifest_sha256-ceq[string]$config.probe_b_recovery_tooling_manifest_sha256-and[string]$recoveryReceipt.result_recovery_tooling_seal_sha256-ceq[string]$config.probe_b_recovery_tooling_seal_sha256-and
    [string]$recoveryReceipt.recovery_controller_sha256-ceq[string]$config.probe_b_recovery_controller_sha256-and[string]$recoveryReceipt.recovery_contract_module_sha256-ceq[string]$config.probe_b_recovery_contract_module_sha256-and[string]$recoveryReceipt.frozen_input_inventory_sha256-ceq[string]$config.probe_b_frozen_input_inventory_sha256-and
    [int]$recoveryReceipt.runtime_reachable_tooling_hash_mismatch_count-eq0-and[int]$recoveryReceipt.frozen_probe_modification_count-eq0-and[int]$recoveryReceipt.probe_execution_count_delta-eq0-and[int]$recoveryReceipt.godot_process_start_count-eq0-and[int]$recoveryReceipt.mcp_process_start_count-eq0-and
...
$importGreen=(Test-HashBinding $config.import_controller_path $config.import_controller_sha256)-and(Test-HashBinding $config.import_finalizer_path $config.import_finalizer_sha256)-and(Test-HashBinding $config.import_runner_path $config.import_runner_sha256)-and(Test-HashBinding $attempt22ContractPath $config.attempt22_contract_module_sha256)-and(Test-PowerShellLoad $config.import_controller_path)-and(Test-PowerShellLoad $config.import_finalizer_path)-and(Test-PowerShellLoad $attempt22ContractPath)
...
$finalizerPlan=[pscustomobject][ordered]@{controller=[IO.Path]::GetFullPath([string]$config.import_finalizer_path);controller_sha256=[string]$config.import_finalizer_sha256;import_runner=[IO.Path]::GetFullPath([string]$config.import_runner_path);import_runner_sha256=[string]$config.import_runner_sha256;mode='FinalizeSnapshot';baseline_sha256=[string]$config.sealed_baseline_sha256;class_cache_sha256=[string]$config.class_cache_sha256;prelaunch_inventory_path=[IO.Path]::GetFullPath([string]$config.formal_prelaunch_ignored_inventory_path);terminal_manifest_path=[IO.Path]::GetFullPath([string]$config.formal_terminal_manifest_path);finalizer_result_path=[IO.Path]::GetFullPath([string]$config.formal_finalizer_result_path);normal_stop_required=$true;terminal_zero_required=$true;result_written_after_finalizer=$true}
$plan=[pscustomobject][ordered]@{executable=Join-Path $PSHOME 'pwsh.exe';script=[IO.Path]::GetFullPath([string]$config.cursor_runbook_path);script_sha256=[string]$config.cursor_runbook_sha256;command_parameters=$commandParameters;phase_plan=$phasePlan;evidence_plan=$evidencePlan;accounting_plan=$accountingPlan;finalizer_plan=$finalizerPlan;deferred_binding_count=2;deferred_bindings=@('ExpectedFormalAuthorizationValidationReceiptSha256','ExpectedFormalAuthorizationSealSha256');formal_evidence_root=[IO.Path]::GetFullPath([string]$config.formal_evidence_root)}
...
$pairedParameters=@(
    ...
    @('Attempt22ContractScriptPath','ExpectedAttempt22ContractSha256'),@('ImportFinalizerBindingPath','ExpectedImportFinalizerBindingSha256'),
    @('ImportRunnerPath','ExpectedImportRunnerSha256'),@('ClassCachePath','ExpectedClassCacheSha256'),@('GodotGuiPath','ExpectedGodotGuiSha256')
)
...
$formalEvidenceFull=[IO.Path]::GetFullPath([string]$config.formal_evidence_root)
$plannedPathsGreen=([IO.Path]::GetFullPath([string]$config.formal_terminal_manifest_path)-ceq[IO.Path]::GetFullPath((Join-Path $formalEvidenceFull 'terminal-process-port-manifest.json'))-and[IO.Path]::GetFullPath([string]$config.formal_finalizer_result_path)-ceq[IO.Path]::GetFullPath((Join-Path $formalEvidenceFull 'formal-import-finalizer-result.json')))
$planGreen=$missingRunbookTokens.Count-eq0-and$missingCommandParameters.Count-eq0-and$missingMandatoryParameters.Count-eq0-and$duplicateCommandParameterCount-eq0-and$pairedParameterFailureCount-eq0-and$hashParametersGreen-and$commandParameters.Count-eq44-and$phasePlan.Count-eq11-and$evidencePlan.Count-eq10-and$accountingPlan.Count-eq6-and[string]$finalizerPlan.mode-ceq'FinalizeSnapshot'-and$plannedPathsGreen
...
$futurePaths=@($config.formal_evidence_root,$config.future_authorization_validation_receipt_path,$config.future_authorization_seal_path,$config.formal_authorization_consumption_receipt_path,$config.formal_prelaunch_ignored_inventory_path)
$boundaryGreen=$processesBefore.Count-eq0-and$mcpProcessesBefore.Count-eq0-and$listenersBefore.Count-eq0-and$processesAfter.Count-eq0-and$mcpProcessesAfter.Count-eq0-and$listenersAfter.Count-eq0-and@($futurePaths|Where-Object{Test-Path -LiteralPath ([string]$_)}).Count-eq0
