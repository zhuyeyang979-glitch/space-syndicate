#!/usr/bin/env python3
"""Fail-closed Generation 10 STEP11 receipt schema and committed-chain validator."""
...
SCHEMA_VERSION = "space_syndicate.v076.generation10_step11_receipt.v1"
RUNTIME_SCHEMA_VERSION = "space_syndicate.v076.generation10_step11_mcp_runtime_evidence.v1"
EVIDENCE_MANIFEST_SCHEMA_VERSION = "space_syndicate.v076.generation10_step11_evidence_manifest.v1"
AUTHORIZATION_SCHEMA_VERSION = "space_syndicate.v076.generation10_authorization_manifest.v1"
ENVIRONMENT_SEAL_SCHEMA_VERSION = "space_syndicate.v076.generation10_environment_seal.v1"
TOOLING_SEAL_SCHEMA_VERSION = "space_syndicate.v076.generation10_receipt_tooling_seal.v1"
EXECUTION_START_SCHEMA_VERSION = "space_syndicate.v076.generation10_execution_start.v1"
PROGRESS_SCHEMA_VERSION = "space_syndicate.v076.generation10_progress.v1"
SUMMARY_SCHEMA_VERSION = "space_syndicate.v076.generation10_summary.v1"

AUTHORIZATION_ID = "USER_AUTHORIZATION_V076_GENERATION10_REPAIRED_RUNNER_FORMAL_20260902"
SUPPLEMENTAL_AUTHORIZATION_ID = "USER_EXPLICIT_GENERATION10_AUTHORIZATION_20260902"
GENERATION_ID = 10
PARENT_GENERATION_ID = 9
PARENT_EVIDENCE_ID = 9695
EVIDENCE_ID = 9696
SEED = 917592522
PLAYER_COUNT = 4
...
TOOLING_SEAL_PATH = "reports/reuse/full_convergence/generation10/generation10_receipt_tooling_repair_seal_012.json"
SUPERSEDED_AUTHORIZATION_PATH = "reports/reuse/full_convergence/generation10/generation10_authorization_manifest_repair_011.json"
AUTHORIZATION_PATH = "reports/reuse/full_convergence/generation10/generation10_authorization_manifest_repair_012.json"
AUTHORIZATION_SIDECAR_PATH = AUTHORIZATION_PATH + ".sha256"
ENVIRONMENT_SEAL_PATH = "reports/reuse/full_convergence/generation10/generation10_environment_seal_repair_012.json"
    ...
    def finish(self) -> dict[str, Any]:
        ...
        payload: dict[str, Any] = {
            ...
            **{name: sorted(set(rows)) for name, rows in self.failures.items()},
            ...
            "generation9_step11_receipt_status": "NOT_EMITTED",
            "generation9_formal_status": "FORMAL_PLATFORM_FAIL",
...
def _derive_runtime_components(result: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if result.get("execution_class") != "FORMAL":
        raise ValueError("FORMAL_RESULT_EXECUTION_CLASS_MISMATCH")
...
def _validate_authorization(project: GitProject, authorization_head: str, tooling_head: str, report: Report) -> tuple[dict[str, Any] | None, bytes | None]:
    ...
    if value is None or raw is None or not _exact_keys(report, value, AUTHORIZATION_FIELDS, "authorization"):
        return None, raw
    superseded_head = str(project._git("log", "-1", "--format=%H", tooling_head, "--", SUPERSEDED_AUTHORIZATION_PATH))
    expected = {
        ...
        "tooling_repair_reason_code": "BOUNDED_READ_ONLY_RUNTIME_QUERY_30_SECONDS_NO_COMMAND_RETRY",
    }
...
def _validate_evidence_manifest(project: GitProject, artifact_head: str, receipt: Mapping[str, Any], report: Report) -> set[str]:
    ...
    if value is None or raw is None or not _exact_keys(report, value, MANIFEST_FIELDS, "evidence_manifest"):
        return set()
    expected = {
        "schema_version": EVIDENCE_MANIFEST_SCHEMA_VERSION, "manifest_id": "v076-generation10-step11-evidence-manifest-001",
...
def _validate_receipt(project: GitProject, artifact_head: str, execution_head: str, execution_tree: str, tooling_head: str, tooling_tree: str, authorization: Mapping[str, Any], authorization_raw: bytes, environment_raw: bytes, report: Report) -> tuple[dict[str, Any] | None, bytes | None]:
    ...
    expected = {
        "schema_version": SCHEMA_VERSION, "receipt_id": "v076-current-subject-step11-generation10-formal-attempt-001",
        ...
        "previous_receipt_path": PARENT_RECEIPT_PATH, "previous_receipt_status": "FORMAL_PLATFORM_FAIL",
...
def validate_repository(project_root: Path, expected_consumer_head: str) -> dict[str, Any]:
    ...
    if len(authorization_additions) != 1 or len(environment_additions) != 1 or len(receipt_additions) != 1:
        report.add("identity_mismatches", "GENERATION10_CHAIN_ADDITION_NOT_UNIQUE", f"auth={authorization_additions}:env={environment_additions}:receipt={receipt_additions}")
        return report.finish()
    ...
    try:
        ...
        report.add("identity_mismatches", "GENERATION10_COMMIT_CHAIN_INVALID", str(exc))
        return report.finish()
    ...
    for path in sorted(formal_paths | {AUTHORIZATION_PATH, AUTHORIZATION_SIDECAR_PATH, ENVIRONMENT_SEAL_PATH, ENVIRONMENT_SIDECAR_PATH}):
        try:
            if project.read(artifact_head, path) != project.read(evaluated_head, path):
                report.add("hash_mismatches", "GENERATION10_ARTIFACT_DRIFT_AFTER_APPEND", path)
        except Exception as exc:
            report.add("path_failures", "GENERATION10_ARTIFACT_UNAVAILABLE", f"{path}:{exc}")
...
def _package_formal(project_root: Path, evidence_root: Path) -> dict[str, Any]:
    ...
    if preflight.get("schema_version") != "space_syndicate.v076.generation10_execution_preflight.v1" or preflight.get("execution_class") != "FORMAL":
        raise ValueError("FORMAL_PREFLIGHT_CLASS_INVALID")
    ...
    confirmation_checks = checks.get("confirmation")
    if not isinstance(confirmation_checks, dict) or any(value is not True for value in confirmation_checks.values()):
        raise ValueError("FORMAL_PREFLIGHT_CONFIRMATION_CHECK_FAILED")
    ...
    confirmation_receipt_raw = (evidence_root / "source-nonformal-confirmation-receipt.json").read_bytes()
    confirmation_result_raw = (evidence_root / "source-nonformal-execution-result.json").read_bytes()
    confirmation_receipt = json.loads(confirmation_receipt_raw.decode("utf-8"))
    confirmation_result = json.loads(confirmation_result_raw.decode("utf-8"))
    if confirmation_receipt.get("schema_version") != "space_syndicate.v076.generation10_nonformal_confirmation_receipt.v1" or confirmation_receipt.get("status") != "PASS":
        raise ValueError("NONFORMAL_CONFIRMATION_RECEIPT_INVALID")
    if confirmation_receipt.get("execution_head_sha") != execution_head or confirmation_receipt.get("execution_tree_sha") != execution_tree:
        raise ValueError("NONFORMAL_CONFIRMATION_HEAD_MISMATCH")
    if confirmation_receipt.get("runner_sha256") != authorization.get("formal_runner_sha256") or confirmation_receipt.get("environment_seal_sha256") != sha256_bytes(environment_raw):
        raise ValueError("NONFORMAL_CONFIRMATION_INPUT_BINDING_MISMATCH")
    if confirmation_receipt.get("execution_result_sha256") != sha256_bytes(confirmation_result_raw):
        raise ValueError("NONFORMAL_CONFIRMATION_RESULT_HASH_MISMATCH")
    if confirmation_receipt.get("formal_execution_count") != 0 or confirmation_receipt.get("automatic_retry_count") != 0:
        raise ValueError("NONFORMAL_CONFIRMATION_CONSUMED_FORMAL_BUDGET")
    if confirmation_result.get("status") != "PASS" or confirmation_result.get("execution_class") != "NONFORMAL_CONFIRMATION" or confirmation_result.get("formal_execution_count") != 0:
        raise ValueError("NONFORMAL_CONFIRMATION_RESULT_INVALID")
    if confirmation_result.get("execution_head_sha") != execution_head or confirmation_result.get("execution_tree_sha") != execution_tree:
        raise ValueError("NONFORMAL_CONFIRMATION_RESULT_HEAD_MISMATCH")
    _derive_runtime_components({**confirmation_result, "formal_execution_count": 1, "execution_class": "FORMAL"})
    ...
    try:
        ...
        runtime = {
            ...
            "mcp_session_id": f"generation10-formal-attempt-001-editor-{connection['pid']}-port-{connection.get('port', 23207)}",
        ...
        evidence_manifest = {
            ...
            "manifest_id": "v076-generation10-step11-evidence-manifest-001",
            "generation_id": GENERATION_ID,
            "resume_evidence_id": EVIDENCE_ID,
            "receipt_id": "v076-current-subject-step11-generation10-formal-attempt-001",
        ...
        receipt = {
            ...
            "previous_receipt_status": "FORMAL_PLATFORM_FAIL",
        ...
        summary = {
            ...
            "generation9_step11_receipt_status": "NOT_EMITTED", "generation9_formal_status": "FORMAL_PLATFORM_FAIL",
            "generation7_modification_count": 0, "generation8_modification_count": 0, "generation9_modification_count": 0,
            "generation7_rerun_count": 0, "generation8_rerun_count": 0, "generation9_rerun_count": 0,
