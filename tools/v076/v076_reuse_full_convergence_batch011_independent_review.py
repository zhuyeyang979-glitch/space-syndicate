"""Independent read-only audit and external B receipt for Batch-011."""
...
import v076_reuse_full_convergence_batch011_materializer as batch011
...
def audit(root: Path, membership_stage: Path, proposal_path: Path) -> dict[str, Any]:
    root = root.resolve()
    membership = batch011.validate_membership(root, membership_stage)
    ...
    if (
        set(proposal) != batch011.PROPOSAL_FIELDS
        or proposal.get("schema_version") != batch011.PROPOSAL_SCHEMA
        or proposal.get("candidate_kind") != batch011.PROPOSAL_KIND
        or proposal.get("evaluated_head_sha") != head
        or proposal.get("evaluated_tree_sha") != tree
        or proposal.get("failure_count") != batch011.MEMBERSHIP_COUNT
        or proposal.get("failure_fingerprint_set_sha256")
        != batch011.MEMBERSHIP_SET_SHA256
    ...
    for fingerprint in proposal["failure_fingerprints"]:
        ...
        if not isinstance(row, dict) or set(row) != batch011.PROPOSAL_ROW_FIELDS:
            raise IndependentReviewError(f"INDEPENDENT_ROW_INVALID:{fingerprint}")
        ...
        if (
            ...
            != identities.source_identity(root, batch011.registry_projection.SOURCE, path)
        ):
            raise IndependentReviewError(f"INDEPENDENT_BINDING_SOURCE_INVALID:{fingerprint}")
        ...
        if (
            ...
            or binding.get("subject_projection_sha256") != base.sha(base.canonical(projection))
            or row.get("expected_registry_rows")
            != base._canonical_registry_rows(projection)
        ):
            raise IndependentReviewError(f"INDEPENDENT_EXACT_PROJECTION_INVALID:{fingerprint}")
        ...
        if failures:
            raise IndependentReviewError(f"INDEPENDENT_AUTHORITY_INVALID:{fingerprint}:{failures[0]}")
        disposition = str(binding.get("recommended_disposition", ""))
        counts[disposition] = counts.get(disposition, 0) + 1
        if path in seen_paths or component in seen_components:
            raise IndependentReviewError(f"INDEPENDENT_IDENTITY_COLLISION:{fingerprint}")
        seen_paths.add(path)
    ...
    if counts != {
        ...
        raise IndependentReviewError("INDEPENDENT_CLASSIFICATION_COUNT_INVALID")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        ...
        review = {
            "schema_version": batch011.REVIEW_SCHEMA,
            "batch_id": batch011.BATCH_ID,
            ...
            "failure_fingerprint_set_sha256": batch011.MEMBERSHIP_SET_SHA256,
            "review_id": "B",
            "reviewer_authority_id": batch011.REVIEWERS["B"],
            "findings": [],
            "p0_count": 0,
            "p1_count": 0,
            "status": "GO",
        ...
        path = batch011._write_external_document(args.root, args.output, review)
        print(json.dumps({"status": "PASS", "review_id": "B", "path": path.as_posix(), "sha256": base.sha(path.read_bytes())}, sort_keys=True))
        return 0
    except (
        IndependentReviewError,
        batch011.Batch011Error,
        base.MaterializerError,
        membership_builder.BuilderError,
    ) as exc:
        print(json.dumps({"status": "FAIL", "batch_id": batch011.BATCH_ID, "error": str(exc)}, sort_keys=True))
        return 2
