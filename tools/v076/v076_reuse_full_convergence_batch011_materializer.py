"""Build and materialize the exact Batch-011 historical identity correction.

The frozen membership and Registry projection already exist as append-only
evidence.  This successor binds them to one committed Head, requires two
distinct external review receipts, and writes only a fresh external stage.
"""
...
import v076_reuse_full_convergence_batch011_registry_projection_builder as registry_projection
...
MEMBERSHIP_SEAL_PAYLOAD_SHA256 = (
    "95faf90ddd52b5e004ced0880fe1f3967723dcfb3b529dfe2f3e239b5c556e05"
)
REGISTRY_PROJECTION_REL = Path(
    "reports/reuse/full_convergence/generation10/"
    "batch011_registry_projection_001/registry-candidate.json"
)
MEMBERSHIP_ROOT_REL = Path(
    "reports/reuse/full_convergence/generation10/"
    "historical_identity_batch011_membership_001"
)
...
PREDECESSOR_REL = BATCH_ROOT / "batch-010" / "batch-010-manifest.json"
PREDECESSOR_SHA256 = (
...
PROPOSAL_SCHEMA = (
    ...
    "batch011_exact_projection_candidate.v1"
)
REVIEW_SCHEMA = (
    "space_syndicate.v076.reuse_full_convergence."
    "batch011_exact_projection_review.v1"
)
...
SUPPORTED_GROUPS = (
    ...
    (
        ...
        "transition_46b33bba77b3_e584cd4d8b0c_diagnostic-only.json",
        "DIAGNOSTIC_ONLY",
    ),
)
BASE_OUTPUTS = frozenset(
    {
        "batch-011/batch-011-manifest.json",
        "batch-011/batch_inventory.json",
        "batch-011/batch_classification.json",
        "batch-011/batch_correction_records.json",
        "batch-011/batch_negative_checks.json",
        "batch-011/batch_review_A.json",
        "batch-011/batch_review_B.json",
    }
)
RECORD_OUTPUTS = {
    disposition: f"records/batch-011/{filename}"
    for disposition, filename, _suffix in SUPPORTED_GROUPS
}
OUTPUT_ALLOWLIST = frozenset({*BASE_OUTPUTS, *RECORD_OUTPUTS.values()})


class Batch011Error(ValueError):
    """Fail-closed Batch-011 trust-boundary error."""
...
def _strict_committed_file(root: Path, head: str, relative: Path) -> bytes:
    ...
    if not path.is_file() or path.read_bytes() != payload:
        raise Batch011Error(f"COMMITTED_WORKTREE_PARITY_INVALID:{relative.as_posix()}")
    return payload
...
def _require_helper_parity(root: Path, head: str) -> None:
    helpers = (
        ...
        root / "tools/v076/v076_reuse_full_convergence_batch011_independent_review.py",
    )
    for helper in helpers:
        relative = helper.relative_to(root).as_posix()
        if not helper.is_file() or helper.read_bytes() != base.committed(root, head, relative):
            raise Batch011Error(f"EXECUTION_HELPER_NOT_COMMITTED:{relative}")


def validate_membership(root: Path, stage: Path) -> dict[str, Any]:
    root = root.resolve()
    stage = base._require_external_stage(root, stage, "BATCH011_MEMBERSHIP_STAGE")
    expected = {
    ...
    if (
        ...
        raise Batch011Error("MEMBERSHIP_STAGE_FILE_SET_INVALID")
    head, _tree = _head_tree(root)
    if not convergence._is_ancestor(
        root, registry_projection.MEMBERSHIP_HEAD, head
    ):
        raise Batch011Error("CURRENT_HEAD_NOT_MEMBERSHIP_DESCENDANT")
    ...
    for filename in sorted(expected):
        ...
        if raw != trusted:
            raise Batch011Error(f"MEMBERSHIP_EXTERNAL_COMMITTED_DRIFT:{filename}")
    ...
    if documents["membership-candidate.json"] != candidate:
        raise Batch011Error("MEMBERSHIP_CANDIDATE_OBJECT_DRIFT")
    ...
    if (
        ...
        raise Batch011Error("MEMBERSHIP_SEAL_INVALID")
    declared_candidate = Path(str(seal.get("candidate_path", ""))).resolve()
    if declared_candidate != (stage / "membership-candidate.json").resolve():
        raise Batch011Error("MEMBERSHIP_SEAL_CANDIDATE_PATH_INVALID")
    reviews = {
        "INDEPENDENT": "membership-review-independent.json",
        "PRIMARY": "membership-review-primary.json",
    }
    refs = seal.get("review_receipts")
    if not isinstance(refs, list) or [row.get("review_id") for row in refs] != [
        "INDEPENDENT",
        "PRIMARY",
    ]:
        raise Batch011Error("MEMBERSHIP_REVIEW_REFERENCE_SET_INVALID")
    for reference in refs:
        review_id = str(reference.get("review_id", ""))
        filename = reviews.get(review_id)
        if (
            not filename
            ...
            raise Batch011Error("MEMBERSHIP_REVIEW_REFERENCE_INVALID")
        ...
        if (
            ...
            raise Batch011Error(f"MEMBERSHIP_REVIEW_INVALID:{review_id}")
...
def _projection_evidence(root: Path, head: str) -> dict[str, Any]:
    ...
    document = base.strict_json_bytes(raw, "BATCH011_REGISTRY_PROJECTION")
    if (
        document.get("batch_id") != BATCH_ID
        or document.get("failure_count") != MEMBERSHIP_COUNT
        or document.get("failure_fingerprint_set_sha256") != MEMBERSHIP_SET_SHA256
        ...
        raise Batch011Error("REGISTRY_PROJECTION_EVIDENCE_INVALID")
    target = document.get("target_registry")
    if not isinstance(target, dict):
        raise Batch011Error("REGISTRY_PROJECTION_TARGET_INVALID")
    try:
        target_bytes = base64.b64decode(target.get("target_bytes_base64", ""), validate=True)
    except Exception as exc:
        raise Batch011Error("REGISTRY_PROJECTION_TARGET_BASE64_INVALID") from exc
    ...
    if (
        ...
        raise Batch011Error("REGISTRY_PROJECTION_TARGET_NOT_CURRENT_AUTHORITY")
    rows = document.get("rows")
    if not isinstance(rows, list) or len(rows) != MEMBERSHIP_COUNT:
        raise Batch011Error("REGISTRY_PROJECTION_ROW_SET_INVALID")
    by_fingerprint = {
    ...
    if len(by_fingerprint) != MEMBERSHIP_COUNT:
        raise Batch011Error("REGISTRY_PROJECTION_ROW_IDENTITY_INVALID")
    for binding in document.get("source_graph_bindings", []):
        ...
        if base.git(root, "rev-parse", f"{head}:{path}") != binding.get(
            ...
            raise Batch011Error(f"REGISTRY_PROJECTION_SOURCE_GRAPH_DRIFT:{path}")
    return {"document": document, "rows": by_fingerprint}


def _reachability(disposition: str) -> str:
    return {
        "HISTORICAL_TEST_ONLY": "TEST_ONLY",
        "HISTORICAL_ACTIVE_LINEAGE_REGISTERED": "PRODUCTION_REACHABLE",
...
def _binding(
    ...
    disposition = str(proof.get("recommended_disposition", ""))
    if not isinstance(row, dict) or disposition not in {item[0] for item in SUPPORTED_GROUPS}:
        raise Batch011Error(f"PROJECTION_PROOF_CLASSIFICATION_INVALID:{fingerprint}")
    ...
    if (
        ...
        raise Batch011Error(f"PROJECTION_PROOF_SOURCE_IDENTITY_DRIFT:{fingerprint}")
    ...
    reachability = _reachability(disposition)
    ...
    binding = {
        ...
        "historical_production_reachability": reachability,
    ...
    if set(binding) != set(convergence.IDENTITY_BINDING_FIELDS):
        raise Batch011Error(f"GENERATED_BINDING_FIELD_SET_INVALID:{fingerprint}")
    ...
    if failures:
        raise Batch011Error(f"GENERATED_BINDING_INVALID:{fingerprint}:{failures[0]}")
...
def build_proposal(root: Path, membership_stage: Path) -> dict[str, Any]:
    ...
    if set(evidence["rows"]) != set(fingerprints):
        raise Batch011Error("REGISTRY_PROJECTION_MEMBERSHIP_SET_MISMATCH")
    ...
    authorized_identities, _primary, _legacy = membership_builder._load_authority(
        root, head
    )
    ...
    for fingerprint in fingerprints:
        ...
        if proof.get("component_row") != direct_rows[fingerprint]:
            raise Batch011Error(f"DIRECT_REGISTRY_ROW_PROOF_DRIFT:{fingerprint}")
        binding, source = _binding(
            ...
            path,
            proof,
            authorized_identities[fingerprint],
        )
    ...
    proposal = {
        ...
        "batch_id": BATCH_ID,
        "membership_candidate_payload_sha256": MEMBERSHIP_CANDIDATE_PAYLOAD_SHA256,
...
def validate_proposal_document(
    ...
    if (
        ...
        or proposal.get("failure_fingerprint_set_sha256") != MEMBERSHIP_SET_SHA256
        ...
        raise Batch011Error("PROPOSAL_CONTRACT_INVALID")
    rows = proposal.get("rows")
    fingerprints = list(candidate["failure_fingerprints"])
    if not isinstance(rows, dict) or set(rows) != set(fingerprints):
        raise Batch011Error("PROPOSAL_ROW_SET_INVALID")
    authorized_identities, _primary, _legacy = membership_builder._load_authority(
        root, head
    )
    ...
    for fingerprint in fingerprints:
        ...
        if (
            ...
            raise Batch011Error(f"PROPOSAL_ROW_INVALID:{fingerprint}")
        binding = proposal_row.get("identity_binding")
        path = str(candidate["rows"][fingerprint]["subject_value"])
        source = identities.source_identity(root, registry_projection.SOURCE, path)
        if (
            not isinstance(binding, dict)
            or set(binding) != set(convergence.IDENTITY_BINDING_FIELDS)
            or proposal_row.get("source_identity") != source
        ):
            raise Batch011Error(f"PROPOSAL_BINDING_OR_SOURCE_INVALID:{fingerprint}")
        selector = binding["authority_selectors"]
        ...
        if (
            ...
            raise Batch011Error(f"PROPOSAL_PROJECTION_INVALID:{fingerprint}")
        failures = convergence._authorized_identity_binding_failures(
            root,
            fingerprint,
            binding,
            authorized_identities.get(fingerprint),
            record_rule_ids=["HISTORY_UNCLASSIFIED_PRODUCT_COMPONENT"],
        )
        if failures:
            raise Batch011Error(f"PROPOSAL_BINDING_INVALID:{fingerprint}:{failures[0]}")
...
def _external_document(root: Path, path: Path, label: str) -> tuple[dict[str, Any], bytes, Path]:
    try:
        return base._read_external_json(root, path, label=label)
    except base.MaterializerError as exc:
        raise Batch011Error(str(exc)) from exc


def _write_external_document(root: Path, path: Path, document: dict[str, Any]) -> Path:
    root = root.resolve()
    ...
    try:
        ...
        raise Batch011Error("EXTERNAL_OUTPUT_INSIDE_REPOSITORY")
    if resolved.exists():
        raise Batch011Error("APPEND_ONLY_EXTERNAL_OUTPUT_EXISTS")
    ...
    try:
        ...
        raise Batch011Error("APPEND_ONLY_EXTERNAL_OUTPUT_EXISTS") from exc
    return resolved


def review_a(root: Path, membership_stage: Path, proposal_path: Path, output: Path) -> dict[str, Any]:
    membership = validate_membership(root, membership_stage)
    proposal, _raw, _resolved = _external_document(root, proposal_path, "PROPOSAL_A")
    validate_proposal_document(root, membership, proposal)
    review = {
    ...
    return {"status": "PASS", "review_id": "A", "path": path.as_posix(), "sha256": _sha(path.read_bytes())}
...
def _review(
    ...
    review, _raw, resolved = _external_document(root, path, f"REVIEW_{expected_id}")
    if (
        set(review) != REVIEW_FIELDS
        or review.get("schema_version") != REVIEW_SCHEMA
        or review.get("batch_id") != BATCH_ID
        or review.get("proposal_payload_sha256") != proposal.get("proposal_payload_sha256")
        ...
        or review.get("failure_fingerprint_set_sha256") != MEMBERSHIP_SET_SHA256
        ...
        raise Batch011Error(f"REVIEW_INVALID:{expected_id}")
    return review, resolved


def _partition(bindings: dict[str, dict[str, Any]], fingerprints: list[str]) -> dict[str, list[str]]:
    groups = {disposition: [] for disposition, _filename, _suffix in SUPPORTED_GROUPS}
    for fingerprint in fingerprints:
        disposition = str(bindings[fingerprint].get("recommended_disposition", ""))
        if disposition not in groups:
            raise Batch011Error(f"UNSUPPORTED_DISPOSITION:{fingerprint}:{disposition}")
    ...
    if {key: len(value) for key, value in groups.items()} != {
        ...
        raise Batch011Error("DISPOSITION_CARDINALITY_INVALID")
    return groups
...
def build_documents(
    ...
    authorized_identities = validated["authorized_identities"]
    head = validated["head"]
    tree = validated["tree"]
    groups = _partition(bindings, fingerprints)
    ...
    predecessor = base.strict_json_bytes(predecessor_bytes, "BATCH010_PREDECESSOR")
    if (
        _sha(predecessor_bytes) != PREDECESSOR_SHA256
        or predecessor.get("batch_id") != "batch-010"
        or predecessor.get("record_chain_terminal_sha256")
        != PREDECESSOR_TERMINAL_SHA256
    ):
        raise Batch011Error("BATCH010_PREDECESSOR_INVALID")
    authority_hashes = base._authority_source_hashes(root, head)
    supplement_sha256 = _sha(
        base.committed(root, head, convergence.DESCENDANT_HISTORY_V3_SUPPLEMENT_REL)
    )
    ...
    for index, (disposition, filename, suffix) in enumerate(SUPPORTED_GROUPS, 1):
        ...
        selected_bindings = {fingerprint: bindings[fingerprint] for fingerprint in selected}
        record = base._record_document(
            ...
            creator="V076ReuseFullConvergenceBatch011Materializer",
        )
        failures = convergence.validate_extension_record_document(record)
        if failures:
            raise Batch011Error(f"GENERATED_RECORD_INVALID:{filename}:{failures[0]}")
        ...
        if failures:
            raise Batch011Error(f"GENERATED_RECORD_REPO_INVALID:{filename}:{failures[0]}")
        relative = f"records/batch-011/{filename}"
        ...
        summaries.append(
            {
                ...
                "path": (RECORD_ROOT / "batch-011" / filename).as_posix(),
    ...
    correction_records = {
        ...
        "authoritative_binding_source": "batch-011-manifest.json#record_bindings",
        "batch_id": BATCH_ID,
        "correction_record_count": len(summaries),
        "failure_count": MEMBERSHIP_COUNT,
        "failure_fingerprint_set_sha256": MEMBERSHIP_SET_SHA256,
        "records": summaries,
    }
    ...
    if failures:
        raise Batch011Error(f"GENERATED_MANIFEST_INVALID:{failures[0]}")
    for field, (_filename, schema, kind) in convergence.BATCH_ARTIFACT_SPECS.items():
        ...
        if failures:
            raise Batch011Error(f"GENERATED_ARTIFACT_INVALID:{field}:{failures[0]}")
    documents = {
        "batch-011/batch-011-manifest.json": manifest,
        "batch-011/batch_inventory.json": artifacts["inventory"],
        "batch-011/batch_classification.json": artifacts["classification"],
        "batch-011/batch_correction_records.json": correction_records,
        "batch-011/batch_negative_checks.json": artifacts["negative_checks"],
        "batch-011/batch_review_A.json": artifacts["review_a"],
        "batch-011/batch_review_B.json": artifacts["review_b"],
        **records,
    }
    if set(documents) != OUTPUT_ALLOWLIST:
        raise Batch011Error("OUTPUT_DOCUMENT_SET_INVALID")
    return documents


def write_stage(root: Path, stage: Path, documents: dict[str, dict[str, Any]]) -> Path:
    root = root.resolve()
    stage = base._require_external_stage(root, stage, "BATCH011_OUTPUT_STAGE")
    if stage.exists():
        raise Batch011Error("OUTPUT_STAGE_MUST_BE_FRESH")
    stage.mkdir(parents=True, exist_ok=False)
    for relative in sorted(documents):
        if relative not in OUTPUT_ALLOWLIST:
            raise Batch011Error(f"OUTPUT_PATH_NOT_ALLOWLISTED:{relative}")
        path = stage / relative
    ...
    if actual != OUTPUT_ALLOWLIST:
        raise Batch011Error("OUTPUT_POSTWRITE_SET_INVALID")
    for relative, document in documents.items():
        path = stage / relative
        if path.read_bytes() != _canonical(document):
            raise Batch011Error(f"OUTPUT_POSTWRITE_PARITY_INVALID:{relative}")
    return stage
...
def materialize(
    ...
    root = root.resolve()
    membership = validate_membership(root, membership_stage)
    proposal, _raw, proposal_resolved = _external_document(root, proposal_path, "PROPOSAL")
    validated = validate_proposal_document(root, membership, proposal)
    review_a_document, review_a_resolved = _review(root, review_a_path, proposal, "A")
    review_b_document, review_b_resolved = _review(root, review_b_path, proposal, "B")
    if len({proposal_resolved, review_a_resolved, review_b_resolved}) != 3:
        raise Batch011Error("PROPOSAL_REVIEW_FILE_ALIAS")
...
def main(argv: list[str] | None = None) -> int:
    ...
    try:
        if args.command == "proposal":
            ...
            result = {
                "status": "PASS",
                "batch_id": BATCH_ID,
                "proposal_payload_sha256": document["proposal_payload_sha256"],
                "path": path.as_posix(),
                "sha256": _sha(path.read_bytes()),
            }
        elif args.command == "review-a":
            result = review_a(
                args.root, args.membership_stage, args.proposal, args.output
            )
        else:
            result = materialize(
                args.root,
    ...
    except (Batch011Error, base.MaterializerError, membership_builder.BuilderError) as exc:
        print(json.dumps({"status": "FAIL", "batch_id": BATCH_ID, "error": str(exc)}, sort_keys=True))
        return 2
