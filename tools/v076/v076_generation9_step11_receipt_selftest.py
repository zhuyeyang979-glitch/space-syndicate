#!/usr/bin/env python3
"""In-memory negative contract tests for the Generation 9 receipt validator."""
...
import v076_generation9_step11_receipt as contract  # noqa: E402
...
def runtime_result() -> dict[str, object]:
    ...
    return {
        "status": "PASS", "formal_execution_count": 1, "automatic_retry_count": 0,
        "generation_id": 9, "new_evidence_id": 9695,
...
check("formal_document_field_sets_are_exact", len(contract.EXECUTION_START_FIELDS) == 23 and len(contract.PROGRESS_FIELDS) == 14 and len(contract.SUMMARY_FIELDS) == 26)
...
check("schema_generation_is_9", schema["required_generation_id"] == 9)
check("schema_evidence_is_9695", schema["required_resume_evidence_id"] == 9695)
...
check("report_validation_mode", report["validation_mode"] == "GENERATION9_SUCCESSOR")
check("report_history_preserved", report["generation7_step11_receipt_status"] == "BLOCKED" and report["generation8_step11_receipt_status"] == "BLOCKED")
...
result = {
    "schema_version": "space_syndicate.v076.generation9_step11_receipt_selftest.v1",
