...
$probeId='pr90-m5-passive-characterization-v6-001'
...
if(-not[bool]$samplingContract.sampling_budget_sufficient-or-not[bool]$samplingContract.short_time_bounded){throw 'Passive sampling contract is not sufficient and bounded.'}
...
try{
    ...
    $m0Green=([string]$manifest.schema-ceq'SpaceSyndicatePr90ListenerObserverToolingManifestV5'-and[string]$manifest.status-ceq'READY_FOR_ONE_M0_M5_CHARACTERIZATION_ONLY'-and[bool]$manifest.characterization_eligible-and-not[bool]$manifest.formal_authorization_eligible-and[string]$manifest.authorized_characterization_probe_id-ceq$probeId-and[string]$seal.schema-ceq'SpaceSyndicatePr90ListenerObserverToolingSealV5'-and[string]$seal.status-ceq'SEALED'-and[bool]$seal.characterization_eligible-and[string]$seal.authorized_characterization_probe_id-ceq$probeId-and[string]$selftest.schema-ceq'SpaceSyndicatePr90ListenerObserverSelfTestV5'-and[string]$selftest.status-ceq'PASS'-and[int]$selftest.case_count-ge79-and[int]$selftest.pass_count-eq[int]$selftest.case_count-and[bool]$selftest.ancestor_chain_flat_cardinality_green-and[int]$selftest.ancestor_chain_nested_array_count-eq0-and[int]$selftest.ancestor_chain_pid_conversion_failure_count-eq0-and[int]$selftest.passive_probe_required_sample_count-ge5-and[bool]$selftest.passive_probe_sample_budget_sufficient-and[int]$selftest.passive_probe_sample_budget_false_green_count-eq0-and[int]$selftest.inline_if_parameter_expression_count_after-eq0-and[int]$selftest.powershell_parse_error_count-eq0-and[int]$selftest.powershell_parameter_binding_exception_count-eq0-and[int]$manifest.passive_probe_required_sample_count-eq[int]$samplingContract.required_total_sample_count-and[int]$manifest.passive_probe_sample_interval_ms-eq[int]$samplingContract.sample_interval_ms-and[int]$manifest.passive_probe_sampling_budget_ms-eq[int]$samplingContract.sampling_budget_ms-and[bool]$manifest.passive_probe_sample_budget_sufficient-and[int]$manifest.passive_probe_sample_budget_false_green_count-eq0-and[string]$manifest.tooling_head_sha-ceq$toolingHead-and[string]$manifest.tooling_tree_sha-ceq$toolingTree-and$gitClean-and$fileMismatch-eq0)
    ...
    Write-Milestone M4 PASS '' ([ordered]@{port=$Port;getnet_count=@($net.records).Count;netstat_count=@($stat.records).Count;sampling_contract=$samplingContract;first_jsonrpc_request_sent=$false})

    $sampleStartedUtc=[DateTimeOffset]::UtcNow;$sampleDeadline=$sampleStartedUtc.AddMilliseconds([int]$samplingContract.sampling_budget_ms);$sampleIndex=0
    do{
        ...
        if($samples.Count-ge[int]$samplingContract.required_total_sample_count){$stable=Test-EndpointOwnerStableWindowV1 @($samples);if($stable.green){break}}
    ...
    $m5Green=($samples.Count-ge[int]$samplingContract.required_total_sample_count-and$stable.green-and[int]$stable.stable_sample_count-ge[int]$samplingContract.required_consecutive_parity_sample_count-and[double]$stable.stable_window_ms-ge[double]$samplingContract.required_stable_window_ms-and$paritySamples.Count-ge[int]$samplingContract.required_consecutive_parity_sample_count-and@($samples|Where-Object{-not[bool]$_.parity.parity}).Count-eq0-and$ownerPids.Count-eq1-and$ownerKeys.Count-eq1-and$lineageKeys.Count-eq1-and$null-ne$ownerIdentity-and[bool]$lastGood[0].owner_is_gui_engine-and[bool]$lastGood[0].owner_descendant_of_wrapper-and[bool]$lastGood[0].command_line_fixture_match-and[bool]$lastGood[0].windows_session_match-and[bool]$lastGood[0].user_sid_match-and[bool]$lastGood[0].created_after_launch_epoch)
    $samplesEvidence=[ordered]@{schema='SpaceSyndicatePr90EndpointListenerSamplesV3';probe_id=$probeId;sample_count=$samples.Count;consecutive_parity_sample_count=[int]$stable.stable_sample_count;sampling_started_utc=$sampleStartedUtc.ToString('o');sampling_deadline_utc=$sampleDeadline.ToString('o');sampling_contract=$samplingContract;samples=@($samples);first_jsonrpc_request_count=0;endpoint_application_request_count=0;canonical_payload_sha256=''};Write-EvidenceJson (Join-Path $evidence 'endpoint-listener-samples-v3.json') $samplesEvidence
    ...
    $characterization=[ordered]@{schema='SpaceSyndicatePr90EndpointOwnerCharacterizationV3';status=if($m5Green){'PASS'}else{'BLOCKED'};probe_id=$probeId;total_listener_sample_count=$samples.Count;consecutive_parity_sample_count=[int]$stable.stable_sample_count;sampling_contract=$samplingContract;endpoint_owner_pid=$ownerPid;endpoint_owner_process_role=if($null-ne$ownerIdentity-and$ownerPid-ne$wrapperPid){'GUI_ENGINE'}else{'UNKNOWN'};endpoint_owner_is_gui_engine=if($null-ne$lastGood-and$lastGood.Count){[bool]$lastGood[0].owner_is_gui_engine}else{$false};endpoint_owner_is_console_wrapper=($ownerPid-eq$wrapperPid);endpoint_owner_is_descendant_of_launcher=if($null-ne$lastGood-and$lastGood.Count){[bool]$lastGood[0].owner_descendant_of_wrapper-and[int]$wrapperIdentity.parent_pid-eq[int]$launcher.Id}else{$false};endpoint_owner_command_line_fixture_match=if($lastGood.Count){[bool]$lastGood[0].command_line_fixture_match}else{$false};endpoint_owner_windows_session_match=if($lastGood.Count){[bool]$lastGood[0].windows_session_match}else{$false};endpoint_owner_user_sid_match=if($lastGood.Count){[bool]$lastGood[0].user_sid_match}else{$false};endpoint_owner_created_after_launch_epoch=if($lastGood.Count){[bool]$lastGood[0].created_after_launch_epoch}else{$false};stable_sample_count=$stable.stable_sample_count;stable_window_ms=$stable.stable_window_ms;pid_changed_count=[math]::Max(0,$ownerPids.Count-1);creation_identity_changed_count=[math]::Max(0,$ownerKeys.Count-1);process_lineage_changed_count=[math]::Max(0,$lineageKeys.Count-1);prelaunch_protected_port_listener_count=$preCount;multiple_active_endpoint_owner_count=if($samples.Count){[math]::Max(0,[int]$samples[-1].parity.matched_count-1)}else{0};m5_root_cause_class=if($m5Green){'D_ENDPOINT_ARCHITECTURE_CONTRACT_DRIFT'}else{'G_UNRESOLVED'};m5_root_cause_name=if($m5Green){'ENDPOINT_ARCHITECTURE_CONTRACT_DRIFT'}else{'UNRESOLVED'};m5_root_cause_formally_attested=$m5Green;endpoint_ownership_contract_v2_implemented=$false;first_jsonrpc_request_count=0;endpoint_application_request_count=0;m6_to_m11_count=0;play_main_scene_count=0;product_match_count=0;formal_mcp_execution_count=0;authorized_run_count_consumed=0;canonical_payload_sha256=''};Write-EvidenceJson (Join-Path $evidence 'endpoint-owner-characterization-v3.json') $characterization
    $m5MilestoneParameters=Resolve-Pr90M5MilestoneParametersV1 -M5Green $m5Green
    $m5MilestoneStatus=[string]$m5MilestoneParameters.status
    $m5MilestoneFailureClass=[string]$m5MilestoneParameters.failure_class
    if(-not$m5Green){$milestoneFailure=$m5MilestoneFailureClass}
    Write-Milestone M5 $m5MilestoneStatus $m5MilestoneFailureClass ([ordered]@{characterization_sha256=Get-Pr90Sha256 (Join-Path $evidence 'endpoint-owner-characterization-v3.json');total_listener_sample_count=$samples.Count;consecutive_parity_sample_count=$stable.stable_sample_count;stable_window_ms=$stable.stable_window_ms;owner_pid=$ownerPid;root_cause_class=$characterization.m5_root_cause_class})
}catch{
...
$result=[ordered]@{
    schema='SpaceSyndicatePr90M5EndpointOwnerCharacterizationV3Result';status=$status;probe_id=$probeId;execution_mode='NONFORMAL_PASSIVE_M0_M5_CHARACTERIZATION_ONLY'
    product_head_sha=$productHead;product_tree_sha=$productTree;tooling_head_sha=if($null-ne$manifest){[string]$manifest.tooling_head_sha}else{''};tooling_tree_sha=if($null-ne$manifest){[string]$manifest.tooling_tree_sha}else{''};fixture_head_sha=$expectedFixtureHead;fixture_tree_sha=$expectedFixtureTree
    characterization_probe_execution_count=1;v6_probe_execution_count=1;automatic_retry_allowed=$false;second_v6_probe_created=$false
    listener_observer_selftest_status=if($null-ne$selftest){[string]$selftest.status}else{'UNKNOWN'};total_listener_sample_count=$samples.Count;consecutive_parity_sample_count=if($null-ne$stable){[int]$stable.stable_sample_count}else{0};passive_sampling_contract=$samplingContract
    endpoint_listener_observer_source_count=2;endpoint_listener_observer_parity=if($samples.Count){@($samples|Where-Object{-not[bool]$_.parity.parity}).Count-eq0}else{$false};endpoint_listener_a_only_count=if($samples.Count){[int]$samples[-1].parity.a_only_count}else{0};endpoint_listener_b_only_count=if($samples.Count){[int]$samples[-1].parity.b_only_count}else{0}
    endpoint_owner_stable_sample_count=if($null-ne$stable){[int]$stable.stable_sample_count}else{0};endpoint_owner_stable_window_ms=if($null-ne$stable){[double]$stable.stable_window_ms}else{0};endpoint_owner_pid=if($ownerPid-gt0){$ownerPid}else{$null};endpoint_owner_process_role=if($m5Green){'GUI_ENGINE'}else{'UNKNOWN'}
    endpoint_owner_is_gui_engine=$m5Green;endpoint_owner_is_console_wrapper=if($ownerPid-gt0){$ownerPid-eq$wrapperPid}else{$false};endpoint_owner_is_descendant_of_launcher=$m5Green;endpoint_owner_command_line_fixture_match=$m5Green;endpoint_owner_windows_session_match=$m5Green;endpoint_owner_user_sid_match=$m5Green
    endpoint_owner_pid_changed_count=[math]::Max(0,$ownerPids.Count-1);endpoint_owner_creation_identity_changed_count=[math]::Max(0,$ownerKeys.Count-1);endpoint_owner_process_lineage_changed_count=[math]::Max(0,$lineageKeys.Count-1);prelaunch_protected_port_listener_count=$preCount;multiple_active_endpoint_owner_count=if($samples.Count){[math]::Max(0,[int]$samples[-1].parity.matched_count-1)}else{0}
    m5_root_cause_class=if($m5Green){'D_ENDPOINT_ARCHITECTURE_CONTRACT_DRIFT'}else{'G_UNRESOLVED'};m5_root_cause_formally_attested=$m5Green
    characterization_stopped_cleanly=[bool]$terminal.stopped_cleanly;forced_stop=[bool]$terminal.forced_stop;godot_process_count_after=[int]$terminal.godot_process_count_after;port_7576_count_after=[int]$terminal.port_7576_count_after;port_7586_count_after=[int]$terminal.port_7586_count_after;unrelated_process_termination_count=[int]$terminal.unrelated_process_termination_count
    failure_class=if($status-ceq'PASS'){''}else{$milestoneFailure};failure_detail=$characterizationFailure;first_jsonrpc_request_sent=$false;endpoint_application_request_count=0;post_repair_m0_m11_probe_execution_count=0;m6_to_m11_execution_count=0;play_main_scene_count=0;product_match_count=0;formal_mcp_execution_count=0;authorized_run_count_consumed=0;exact_sha_mcp_status='NOT_STARTED'
    ready_for_endpoint_ownership_v2_repair_authorization=($status-ceq'PASS');ready_for_post_repair_m0_m11_probe=$false;ready_for_pr90_startup_probe_b=$false;ready_for_new_exact_sha_mcp_authorization=$false;canonical_payload_sha256=''
}
...
- Total/stable samples: $($result.total_listener_sample_count) / $($result.consecutive_parity_sample_count)
- Stable window: $($result.endpoint_owner_stable_window_ms) ms
- Sampling budget: $($samplingContract.sampling_budget_ms) ms
...
$attestation=[ordered]@{schema='SpaceSyndicatePr90M5EndpointOwnerCharacterizationV3Attestation';status=if($status-ceq'PASS'){'SEALED'}else{'BLOCKED_EVIDENCE_SEALED'};probe_id=$probeId;product_head_sha=$productHead;product_tree_sha=$productTree;tooling_head_sha=$result.tooling_head_sha;tooling_tree_sha=$result.tooling_tree_sha;tooling_manifest_sha256=Get-Pr90Sha256 $manifestPath;tooling_seal_sha256=Get-Pr90Sha256 $sealPath;selftest_sha256=Get-Pr90Sha256 $selfTestPath;tooling_files=if($null-ne$manifest){$manifest.tooling_files}else{@()};passive_sampling_contract=$samplingContract;total_listener_sample_count=$result.total_listener_sample_count;consecutive_parity_sample_count=$result.consecutive_parity_sample_count;endpoint_owner_stable_window_ms=$result.endpoint_owner_stable_window_ms;m5_root_cause_class=$result.m5_root_cause_class;m5_root_cause_formally_attested=$result.m5_root_cause_formally_attested;bound_evidence=$boundEvidence;result_path=$resultPath;result_sha256=Get-Pr90Sha256 $resultPath;markdown_path=$mdPath;markdown_sha256=Get-Pr90Sha256 $mdPath;terminal_manifest_sha256=Get-Pr90Sha256 (Join-Path $evidence 'terminal-process-port-manifest-v3.json');characterization_attestation_sealed=($status-ceq'PASS');characterization_tooling_bytes_changed_after_seal=$false;endpoint_ownership_contract_v2_implemented=$false;post_repair_m0_m11_probe_execution_count=0;first_jsonrpc_request_sent=$false;endpoint_application_request_count=0;m6_to_m11_execution_count=0;play_main_scene_count=0;product_match_count=0;formal_mcp_execution_count=0;authorized_run_count_consumed=0;ready_for_endpoint_ownership_v2_repair_authorization=($status-ceq'PASS');ready_for_post_repair_m0_m11_probe=$false;ready_for_pr90_startup_probe_b=$false;ready_for_new_exact_sha_mcp_authorization=$false;canonical_payload_sha256=''}
