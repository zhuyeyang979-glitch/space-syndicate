...
$probeId='pr90-m5-endpoint-owner-characterization-v2-001'
...
function Write-EvidenceJson([string]$Path,[object]$Value){
    if($Value.PSObject.Properties.Name-contains'canonical_payload_sha256'){$Value.canonical_payload_sha256=Get-Pr90CanonicalSha256 $Value}
...
try{
    ...
    $m1=[ordered]@{schema='SpaceSyndicatePr90M5CharacterizationExecutionStartV2';probe_id=$probeId;started_utc=[DateTimeOffset]::UtcNow.ToString('o');fixture_head=$fixtureHead;fixture_tree=$fixtureTree;fixture_root=$fixture;fixture_clean=$fixtureClean;prelaunch_7576_listener_count=0;prelaunch_7586_listener_count=0;prelaunch_godot_process_count=0;first_jsonrpc_request_sent=$false;m6_to_m11_execution_count=0;formal_mcp_execution_count=0;authorized_run_count_consumed=0;canonical_payload_sha256=''}
    Write-EvidenceJson (Join-Path $evidence 'execution-start-v2.json') $m1;Write-Milestone M1 PASS '' ([ordered]@{execution_start_sha256=Get-Pr90Sha256(Join-Path $evidence 'execution-start-v2.json');fixture_head=$fixtureHead;fixture_tree=$fixtureTree})

    $launchScript=Join-Path $tooling 'tools\launch_role_godot_mcp.ps1';$launchReceiptPath=Join-Path $evidence 'launch-receipt-v2.json';$launchOut=Join-Path $evidence 'launcher.stdout.log';$launchErr=Join-Path $evidence 'launcher.stderr.log';$sessionId=[guid]::NewGuid().ToString('N')
...
"@
$mdPath=Join-Path $root 'pr90_m5_endpoint_owner_characterization_v2_result.md';Write-Pr90ImmutableText -Path $mdPath -Text $md -WriteSha256Sidecar|Out-Null
...
$attestation=[ordered]@{schema='SpaceSyndicatePr90M5EndpointOwnerCharacterizationV2Attestation';status=if($status-ceq'PASS'){'SEALED'}else{'BLOCKED_EVIDENCE_SEALED'};probe_id=$probeId;product_head_sha=$productHead;product_tree_sha=$productTree;tooling_head_sha=$result.tooling_head_sha;tooling_tree_sha=$result.tooling_tree_sha;tooling_manifest_sha256=Get-Pr90Sha256 $manifestPath;tooling_seal_sha256=Get-Pr90Sha256 $sealPath;selftest_sha256=Get-Pr90Sha256 $selfTestPath;tooling_files=if($null-ne$manifest){$manifest.tooling_files}else{@()};bound_evidence=$boundEvidence;result_path=$resultPath;result_sha256=Get-Pr90Sha256 $resultPath;markdown_path=$mdPath;markdown_sha256=Get-Pr90Sha256 $mdPath;terminal_manifest_sha256=Get-Pr90Sha256(Join-Path $evidence 'terminal-process-port-manifest-v2.json');characterization_attestation_sealed=($status-ceq'PASS');characterization_tooling_bytes_changed_after_seal=$false;endpoint_ownership_contract_v2_implemented=$false;post_repair_m0_m11_probe_execution_count=0;first_jsonrpc_request_sent=$false;endpoint_application_request_count=0;m6_to_m11_execution_count=0;formal_mcp_execution_count=0;authorized_run_count_consumed=0;canonical_payload_sha256=''}
$attestationPath=Join-Path $root 'pr90_m5_endpoint_owner_characterization_v2_attestation.json';Write-EvidenceJson $attestationPath $attestation
