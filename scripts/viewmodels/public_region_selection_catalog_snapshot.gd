extends RefCounted
class_name PublicRegionSelectionCatalogSnapshot

const ENTRY_SCRIPT := preload("res://scripts/viewmodels/public_region_selection_catalog_entry.gd")
const SCHEMA_VERSION := 1
const CATALOG_KIND := "region"
const SOURCE_OWNER_ID := "WorldSessionState"
...
) -> PublicRegionSelectionCatalogSnapshot:
...
func apply_dictionary(source: Dictionary) -> PublicRegionSelectionCatalogSnapshot:
	_valid = false
	entries.clear()
	...
	for source_variant in source_entries:
		if not (source_variant is Dictionary):
			return self
		var entry: ENTRY_SCRIPT = ENTRY_SCRIPT.new().apply_dictionary(source_variant as Dictionary)
		if entry == null or not entry.is_valid() or seen_ids.has(entry.region_id):
			return self
		seen_ids[entry.region_id] = true
...
func entry_by_id(region_id: String) -> ENTRY_SCRIPT:
	for entry in entries:
...
static func ordering_revision_for(entry_rows: Array) -> String:
	return JSON.stringify(["public_region_selection_order_revision_v1", _ordering_rows(entry_rows)]).sha256_text()


static func ordering_fingerprint_for(entry_rows: Array) -> String:
	return JSON.stringify(["public_region_selection_order_fingerprint_v1", _ordering_rows(entry_rows)]).sha256_text()
...
static func data_revision_for(entry_rows: Array) -> String:
	...
	for entry_variant in entry_rows:
		...
		rows.append([
			...
			str(entry.get("public_status", "")),
			bool(entry.get("selectable", false)),
			str(entry.get("disabled_reason", "")),
			str(entry.get("public_terrain", "")),
		])
	return JSON.stringify(["public_region_selection_data_revision_v1", rows]).sha256_text()
