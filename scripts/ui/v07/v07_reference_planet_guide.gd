@tool
extends Control
class_name V07ReferencePlanetGuide
...
func debug_snapshot() -> Dictionary:
	var lines: Array = _payload.get("local_grid_lines", []) \
		if _payload.get("local_grid_lines", []) is Array else []
	return {
		"kind": "v07_reference_planet_guide",
		"mode": str(_payload.get("mode", "")),
		"grid_line_count": lines.size(),
		"sceneized": true,
		"radial_spoke_count": 0,
		"positional_marker_count": 0,
	}
...
func _draw() -> void:
	...
	_draw_latitude_guides(center, radius, globe_blend)
	_draw_local_grid(globe_blend)


func _draw_orbit_rings(center: Vector2, radius: float) -> void:
	var rings: Array = _payload.get("orbit_rings", []) \
		if _payload.get("orbit_rings", []) is Array else []
	for ring_variant in rings:
		...
		draw_arc(
			_as_vector2(ring.get("center", center)),
			maxf(2.0, float(ring.get("radius", radius))),
			0.0,
			TAU,
			96,
			color,
...
func _draw_latitude_guides(center: Vector2, radius: float, globe_blend: float) -> void:
	var color := Color("#67e8f9")
	color.a = 0.045 + globe_blend * 0.075
	for ratio in [0.28, 0.55, 0.78]:
		draw_arc(center, radius * float(ratio), 0.0, TAU, 96, color, 0.65, true)
...
func _draw_local_grid(globe_blend: float) -> void:
	...
	for line_variant in lines:
		...
		draw_line(
			_as_vector2(line.get("from", Vector2.ZERO)),
			_as_vector2(line.get("to", Vector2.ZERO)),
			color,
			1.0,
			true
		)
