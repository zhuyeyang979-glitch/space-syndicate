...
class_name SpaceSyndicateBestiaryMonsterKpiCard

@onready var title_label: Label = %BestiaryMonsterKpiTitle
@onready var value_label: Label = %BestiaryMonsterKpiValue
@onready var meta_label: Label = %BestiaryMonsterKpiMeta


func set_kpi(data: Dictionary) -> void:
	var accent := _dictionary_color(data, "accent", Color("#fb923c"))
	...
	value_label.text = _short_text(value_text, 38)
