...
class_name SpaceSyndicateProductCodexBrowser
...
const THUMBNAIL_CARD_SCENE := preload("res://scenes/ui/codex/ProductCodexThumbnailCard.tscn")

@onready var previous_button: Button = %ProductBrowserPreviousButton
@onready var next_button: Button = %ProductBrowserNextButton
@onready var page_label: Label = %ProductBrowserPageLabel
@onready var overview_host: HFlowContainer = %ProductBrowserOverviewHost
@onready var thumbnail_grid: GridContainer = %ProductBrowserThumbnailGrid
@onready var preview_detail: Control = %ProductBrowserPreviewDetail
...
func set_browser(data: Dictionary) -> void:
	...
	if preview_detail != null and preview_detail.has_method("set_product") and preview_variant is Dictionary:
		preview_detail.call("set_product", preview_variant as Dictionary)
...
func _render_entries(entries: Array) -> void:
	...
	for entry_variant: Variant in entries:
		...
		card.name = "ProductThumbnail_%d" % catalog_index
