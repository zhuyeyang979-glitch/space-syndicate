...
func _render_badge(badge_variant: Variant, fallback_accent: Color, fallback_secondary: Color) -> void:
	...
	var profile_line := _label(str(badge.get("profile", "商品｜商业线")), 10, Color("#fde68a"))
	...
	var terrain_line := _label(str(badge.get("terrain", "地形:通用")), 10, Color("#bfdbfe"))
	...
	var weather := _label(_short_text(weather_text, 76), 10, Color("#fde68a"))
	...
	var use_line := _label(_short_text(use_text, 72), 10, Color("#dcfce7"))
...
func _add_strategy(entry: Dictionary) -> void:
	...
	var body := _label(_short_text(body_text, 86), 10, Color("#e5e7eb"))
