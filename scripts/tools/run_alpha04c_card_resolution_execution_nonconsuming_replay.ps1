...
$AuthorizationId = "alpha04c-card-resolution-execution-save-v4-replay-v1"
$RunId = "alpha04c-card-resolution-execution-replay-v1-attempt-1"
...
$evidenceFull = Join-Path $projectFull "reports/handoffs/alpha04c_card_resolution_execution_save_v4_replay_v1.json"
$parentFull = Join-Path $projectFull "reports/handoffs/alpha04c_card_resolution_execution_save_v4_replay_v1_parent_attestation.json"
$authorizationPath = Join-Path $projectFull "scripts/tools/card_resolution_execution_owner_replay_authorization_v1.json"

Assert-Condition (Test-Path -LiteralPath $GodotPath -PathType Leaf) "godot_console_missing"
Assert-Condition (Test-Path -LiteralPath $authorizationPath -PathType Leaf) "execution_replay_authorization_missing"
Assert-Condition (-not (Test-Path -LiteralPath $evidenceFull) -and -not (Test-Path -LiteralPath $parentFull)) "execution_replay_evidence_already_exists"
...
Assert-Condition (
    (Test-ExactPropertySet -Value $authorization -Expected @(
        ...
        -and [string]$authorization.contract_id -ceq "CardResolutionExecutionOwnerReplayAuthorizationV1" `
        -and [string]$authorization.authorization_id -ceq $AuthorizationId `
        -and [string]$authorization.run_id_prefix -ceq "alpha04c-card-resolution-execution-replay-v1" `
...
) "execution_replay_authorization_invalid"
...
Assert-Condition ($LASTEXITCODE -eq 0 -and $diffCheck.Count -eq 0) "execution_replay_git_diff_check_failed"
Assert-Condition ($dirtyPaths.Count -eq 0) "execution_replay_tree_not_clean"
Assert-Condition ($branch -cmatch "^codex/alpha04c-card-resolution-execution-save-v4-[0-9a-f]{7,12}$") "execution_replay_branch_invalid"
$remoteHead = (& git -C $projectFull rev-parse "refs/remotes/origin/$branch" 2>$null).Trim().ToLowerInvariant()
Assert-Condition ($LASTEXITCODE -eq 0 -and $remoteHead -ceq $repositoryHead) "execution_replay_remote_checkpoint_mismatch"
...
Assert-Condition (-not (Test-Path -LiteralPath $v8Root)) "v8_root_created_before_execution_replay"
...
$replayRoot = Join-Path $gitCommon "codex/cold_restore_v3/non-official-alpha04c-card-resolution-execution-replay-v1"
...
Assert-Condition (-not (Test-Path -LiteralPath $replayRoot)) "execution_replay_attempt_already_claimed"
$claim = [ordered]@{
    schema_version = 1
    claim_id = "CardResolutionExecutionOwnerReplayAttemptClaimV1"
...
$admission = [ordered]@{
    schema_version = 1
    admission_id = "CardResolutionExecutionOwnerReplayChildAdmissionV1"
...
$tempRoot = Join-Path $tempBase ("space-syndicate-alpha04c-execution-replay-v1-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    foreach ($argument in @(
        ...
        "res://scripts/tools/alpha04c_card_resolution_execution_nonconsuming_replay.gd",
    ...
    Assert-Condition ($process.Start()) "execution_replay_godot_launch_failed"
    $stdoutTask = $process.StandardOutput.ReadToEndAsync()
    $stderrTask = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(120000)) {
        $process.Kill($true)
        $process.WaitForExit()
        throw "execution_replay_timeout"
    }
    $stdout = $stdoutTask.GetAwaiter().GetResult()
    $stderr = $stderrTask.GetAwaiter().GetResult()
    $childExitCode = $process.ExitCode
    Assert-Condition (Test-Path -LiteralPath $evidenceFull -PathType Leaf) "execution_replay_child_evidence_missing"
    ...
    $childGreen = $childExitCode -eq 0 `
        ...
        -and [bool]$childResult.execution_replay_scenario_identity_green `
        -and [bool]$childResult.execution_replay_registry_binding_green `
        -and [bool]$childResult.execution_save_v4_replay_green `
        -and [bool]$childResult.execution_payload_closed `
        -and [bool]$childResult.execution_restore_parity `
        -and [bool]$childResult.execution_timer_bits_parity `
        -and [bool]$childResult.execution_cadence_bits_parity `
        -and [bool]$childResult.inflight_transaction_parity `
        -and [bool]$childResult.pending_settlement_parity `
        -and [bool]$childResult.transition_lineage_parity `
        -and [int]$childResult.execution_capture_mutation_count -eq 0 `
    ...
    Assert-Condition ([bool]$parent.parent_attestation_green) "execution_replay_parent_attestation_failed"
    [Console]::Out.WriteLine("ALPHA04C_CARD_RESOLUTION_EXECUTION_REPLAY_V1_PARENT|" + (ConvertTo-Json -InputObject $parent -Compress -Depth 8))
}
finally {
    if ($null -ne $process) {
        try {
    ...
    if ($resolvedTempRoot.StartsWith($tempBase, [StringComparison]::OrdinalIgnoreCase) `
            -and (Split-Path -Leaf $resolvedTempRoot).StartsWith("space-syndicate-alpha04c-execution-replay-v1-", [StringComparison]::Ordinal)) {
