extends RefCounted
class_name RemainingOwnerClosedDataPreflightAttemptV2

const AUTHORIZATION_PATH := "res://scripts/tools/remaining_owner_closed_data_preflight_attempt_v2_authorization.json"
const ATTEMPT_ROOT_RELATIVE_PATH := "codex/cold_restore_v3/non-official-alpha04c-remaining-owner-preflight-index14-18-attempt-v2"
const CONTRACT_ID := "RemainingOwnerClosedDataPreflightAttemptV2"
const ATTEMPT_ID := "alpha04c-remaining-owner-preflight-index14-18-attempt-v2"
...
static func authorization() -> Dictionary:
	...
	if not _has_exact_fields(value, AUTHORIZATION_FIELDS) \
			...
			or str(value.get("contract_id", "")) != "RemainingOwnerClosedDataPreflightAttemptV2Authorization" \
...
static func consume_child_admission(
	...
	if attempt_root.is_empty() or claim_path != expected_claim \
		...
		return _rejected("remaining_owner_preflight_attempt_v2_admission_path_invalid")
	...
	if claim_text.is_empty() or claim_text.sha256_text().to_lower() != claim_sha256:
		return _rejected("remaining_owner_preflight_attempt_v2_claim_sha256_invalid")
	...
	if not (claim_variant is Dictionary) or not (admission_variant is Dictionary):
		return _rejected("remaining_owner_preflight_attempt_v2_admission_invalid")
	...
	if not _has_exact_fields(claim, CLAIM_FIELDS) \
			...
			or str(claim.get("claim_id", "")) != "RemainingOwnerClosedDataPreflightAttemptClaimV2" \
		...
		return _rejected("remaining_owner_preflight_attempt_v2_claim_invalid")
	if not _has_exact_fields(admission, ADMISSION_FIELDS) \
			or int(admission.get("schema_version", 0)) != 1 \
			or str(admission.get("admission_id", "")) != "RemainingOwnerClosedDataPreflightChildAdmissionV2" \
	...
	if DirAccess.rename_absolute(admission_path, consumed_path) != OK:
		return _rejected("remaining_owner_preflight_attempt_v2_admission_already_consumed")
	return {
		"accepted": true,
		"reason_code": "remaining_owner_preflight_attempt_v2_admission_consumed",
