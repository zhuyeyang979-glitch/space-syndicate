extends RefCounted
class_name CardResolutionExecutionOwnerReplayScenarioIdentityV1

const WIRE := preload("res://scripts/semantic/semantic_wire_v1.gd")
const AUTHORIZATION_PATH := "res://scripts/tools/card_resolution_execution_owner_replay_authorization_v1.json"
const REPLAY_ROOT_RELATIVE_PATH := "codex/cold_restore_v3/non-official-alpha04c-card-resolution-execution-replay-v1"
const CONTRACT_ID := "CardResolutionExecutionOwnerReplayScenarioIdentityV1"
const AUTHORIZATION_ID := "alpha04c-card-resolution-execution-save-v4-replay-v1"
const RUN_ID_PREFIX := "alpha04c-card-resolution-execution-replay-v1"
const RUN_ID := "alpha04c-card-resolution-execution-replay-v1-attempt-1"
...
static func authorization() -> Dictionary:
	...
	if not _has_exact_fields(value, AUTHORIZATION_FIELDS) \
			...
			or str(value.get("contract_id", "")) != "CardResolutionExecutionOwnerReplayAuthorizationV1" \
...
static func validation_report(value: Variant, expected_repository_head := "") -> Dictionary:
	var auth := authorization()
	if auth.is_empty():
		return _rejected("execution_replay_authorization_invalid")
	if not (value is Dictionary):
		return _rejected("execution_replay_scenario_identity_not_dictionary")
	...
	if not _has_exact_fields(identity, IDENTITY_FIELDS) \
		...
		return _rejected("execution_replay_scenario_identity_invalid")
	return {"valid": true, "reason_code": "execution_replay_scenario_identity_valid"}
...
static func _identity_scalar_types_are_exact(identity: Dictionary) -> bool:
	for field in [
		...
		"execution_checkpoint_strategy", "identity_fingerprint",
	]:
	...
	for field in [
		...
		"transition_state_wire_version", "execution_registry_state_version",
	]:
