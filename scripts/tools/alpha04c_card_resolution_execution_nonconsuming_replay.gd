...
const REPLAY_IDENTITY := preload("res://scripts/tools/card_resolution_execution_owner_replay_scenario_identity_v1.gd")
...
func _run() -> void:
	...
	if authorization.is_empty():
		_finish(result, output_path, "execution_replay_authorization_invalid")
		return
	var expected_output_path := _normalize_absolute_path(ProjectSettings.globalize_path(
		"res://reports/handoffs/alpha04c_card_resolution_execution_save_v4_replay_v1.json"
	))
	if output_path.is_empty() or output_path.contains("current_run.save") \
		...
		_finish(result, "", "execution_replay_evidence_path_invalid")
		return
	if not _lower_hex(repository_head, 40, 40):
		_finish(result, output_path, "execution_replay_repository_head_invalid")
		return
	...
	if not bool(admission.get("accepted", false)):
		_finish(result, output_path, str(admission.get("reason_code", "execution_replay_admission_invalid")))
		return
	...
	if not bool(context.get("ready", false)):
		main.queue_free()
		await process_frame
		_finish(result, output_path, "execution_replay_production_composition_unavailable")
		return
	...
	if not bool(started.get("applied", false)):
		main.queue_free()
		await process_frame
		_finish(result, output_path, str(started.get("reason_code", "execution_replay_session_start_failed")))
		return
	...
	result["execution_replay_scenario_identity_attested"] = bool(identity_report.get("valid", false))
	...
	if not bool(identity_report.get("valid", false)):
		main.queue_free()
		await process_frame
		_finish(result, output_path, str(identity_report.get("reason_code", "execution_replay_scenario_identity_invalid")))
		return
	...
	result["execution_replay_registry_binding_attested"] = binding_attested
	...
	if not binding_attested:
		main.queue_free()
		await process_frame
		_finish(result, output_path, "execution_replay_registry_binding_invalid")
		return
	...
	result.merge({
		...
		"execution_restore_parity": restore_parity,
		...
		"execution_capture_mutation_count": capture_mutation_count,
	...
	var green: bool = bool(result.get("scenario_identity_attested", false)) \
			and binding_attested and save_closed and envelope_roundtrip and restore_parity \
			and timer_bits_parity and cadence_bits_parity and transaction_parity and pending_parity and lineage_parity \
	...
	result["execution_replay_scenario_identity_green"] = bool(result.get("scenario_identity_attested", false))
	result["execution_replay_registry_binding_green"] = binding_attested
	result["execution_save_v4_replay_green"] = green
	result["success"] = green
	result["status"] = "GREEN" if green else "BLOCKED"
	result["reason_code"] = "execution_nonconsuming_replay_green" if green else "execution_nonconsuming_replay_failed"
	main.queue_free()
	await process_frame
...
func _start_fixed_session(context: Dictionary, replay_run_id: String) -> Dictionary:
	...
	if draft == null or transaction == null or session == null or rng == null:
		return {"applied": false, "reason_code": "execution_replay_session_start_dependency_missing"}
	...
	if int(setup.get("challenge_depth", -1)) != REPLAY_IDENTITY.CHALLENGE_DEPTH \
		...
		return {"applied": false, "reason_code": "execution_replay_fixed_setup_mismatch"}
	...
	return {
		...
		"reason_code": receipt.reason_code if receipt != null else "execution_replay_session_start_receipt_missing",
...
func _build_scenario_identity(context: Dictionary, started: Dictionary, repository_head: String, replay_run_id: String) -> Dictionary:
...
func _target_binding(contract: Dictionary) -> Dictionary:
	...
	return (rows[TARGET_OWNER_INDEX] as Dictionary).duplicate(true) \
			if TARGET_OWNER_INDEX >= 0 and TARGET_OWNER_INDEX < rows.size() and rows[TARGET_OWNER_INDEX] is Dictionary else {}
...
func _consume_replay_admission(repository_head: String, authorization: Dictionary) -> Dictionary:
	...
	if replay_root.is_empty() or claim_path != expected_claim or admission_path != expected_admission \
		...
		return {"accepted": false, "reason_code": "execution_replay_admission_path_invalid"}
	...
	if claim_text.is_empty() or claim_text.sha256_text().to_lower() != claim_sha256:
		return {"accepted": false, "reason_code": "execution_replay_claim_sha256_invalid"}
	...
	if not (claim_variant is Dictionary) or not (admission_variant is Dictionary):
		return {"accepted": false, "reason_code": "execution_replay_admission_invalid"}
	...
	if not _has_exact_fields(claim, REPLAY_CLAIM_FIELDS) \
			...
			or str(claim.get("claim_id", "")) != "CardResolutionExecutionOwnerReplayAttemptClaimV1" \
		...
		return {"accepted": false, "reason_code": "execution_replay_claim_invalid"}
	if not _has_exact_fields(admission, REPLAY_ADMISSION_FIELDS) \
			or int(admission.get("schema_version", 0)) != 1 \
			or str(admission.get("admission_id", "")) != "CardResolutionExecutionOwnerReplayChildAdmissionV1" \
	...
	if DirAccess.rename_absolute(admission_path, consumed_path) != OK:
		return {"accepted": false, "reason_code": "execution_replay_admission_already_consumed"}
	return {"accepted": true, "reason_code": "execution_replay_admission_consumed", "claim_sha256": claim_sha256}
...
func _normalize_absolute_path(value: String) -> String:
	return "" if value.is_empty() or not value.is_absolute_path() else value.replace("\\", "/").simplify_path().trim_suffix("/")
...
func _print_result(result: Dictionary) -> void:
	print("ALPHA04C_CARD_RESOLUTION_EXECUTION_NONCONSUMING_REPLAY_V1|%s" % JSON.stringify(result))
