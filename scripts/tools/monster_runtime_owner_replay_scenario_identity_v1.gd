extends RefCounted
class_name MonsterRuntimeOwnerReplayScenarioIdentityV1

const WIRE := preload("res://scripts/semantic/semantic_wire_v1.gd")
const AUTHORIZATION_PATH := "res://scripts/tools/monster_runtime_owner_replay_authorization_v1.json"
const REPLAY_ROOT_RELATIVE_PATH := "codex/cold_restore_v3/non-official-alpha04c-monster-runtime-replay-v1"
const CONTRACT_ID := "MonsterRuntimeOwnerReplayScenarioIdentityV1"
const AUTHORIZATION_ID := "alpha04c-monster-runtime-save-v2-replay-v1"
const RUN_ID_PREFIX := "alpha04c-monster-runtime-replay-v1"
const RUN_ID := "alpha04c-monster-runtime-replay-v1-attempt-1"
...
const OWNER_INDEX := 9
const SECTION_ID := "monsters"
const OWNER_ID := "monster_runtime"
...
const IDENTITY_FIELDS := [
	...
	"monster_registry_state_version",
	"monster_checkpoint_strategy",
...
static func authorization() -> Dictionary:
	...
	if not _has_exact_fields(value, AUTHORIZATION_FIELDS) \
			...
			or str(value.get("contract_id", "")) != "MonsterRuntimeOwnerReplayAuthorizationV1" \
...
static func build(source: Dictionary) -> Dictionary:
	...
	var unsealed := {
		...
		"monster_save_schema_version": source.get("monster_save_schema_version", -1),
		"monster_registry_state_version": source.get("monster_registry_state_version", -1),
		"monster_checkpoint_strategy": source.get("monster_checkpoint_strategy", ""),
	}
...
static func validation_report(value: Variant, expected_repository_head: String = "") -> Dictionary:
	var auth := authorization()
	if auth.is_empty():
		return _rejected("monster_replay_authorization_invalid")
	if not (value is Dictionary):
		return _rejected("monster_replay_scenario_identity_not_dictionary")
	...
	if not _has_exact_fields(identity, IDENTITY_FIELDS) \
			...
			or int(identity.get("monster_save_schema_version", -1)) != SAVE_SCHEMA_VERSION \
			or int(identity.get("monster_registry_state_version", -1)) != REGISTRY_STATE_VERSION \
			or str(identity.get("monster_checkpoint_strategy", "")) != CHECKPOINT_STRATEGY \
			or str(identity.get("identity_fingerprint", "")) != WIRE.fingerprint(identity, "identity_fingerprint"):
		return _rejected("monster_replay_scenario_identity_invalid")
	return {"valid": true, "reason_code": "monster_replay_scenario_identity_valid"}
...
static func _identity_scalar_types_are_exact(identity: Dictionary) -> bool:
	for field in [
		...
		"owner_id", "monster_checkpoint_strategy", "identity_fingerprint",
	]:
		if not (identity.get(field) is String):
			return false
	for field in [
		"schema_version", "challenge_depth", "run_seed", "local_player_count",
		"ai_player_count", "owner_index", "monster_save_schema_version",
		"monster_registry_state_version",
	]:
