extends Control
class_name FirstMissionSpineBench

const GAME_SCREEN_SCENE_PATH := "res://scenes/ui/GameScreen.tscn"
const FIRST_MISSION_FIXTURE_SCRIPT_PATH := "res://scripts/tools/first_mission_spine_fixtures.gd"
const OUTPUT_DIR := "user://space_syndicate_design_qa/first_mission_spine/"
const MANIFEST_PATH := OUTPUT_DIR + "manifest.json"
const REPORT_PATH := OUTPUT_DIR + "report.md"
const SCREENSHOT_PATH := "user://space_syndicate_design_qa/first_mission_spine_sprint_1.png"
...
@onready var status_label: Label = %FirstMissionSpineStatusLabel
@onready var summary_label: Label = %FirstMissionSpineSummaryLabel
@onready var preview_viewport: SubViewport = %FirstMissionSpinePreviewViewport
...
func _ready() -> void:
	...
	if auto_run_on_ready:
		call_deferred("_run_mission_suite_and_maybe_quit")
...
func mission_steps() -> Array:
	...
	if fixtures == null or not fixtures.has_method("mission_steps"):
		return []
	var value: Variant = fixtures.call("mission_steps")
	return value if value is Array else []


func build_mission_manifest_preview() -> Dictionary:
	var records: Array = []
	...
	return {
		"version": "first-mission-spine-v1",
...
func run_mission_suite() -> void:
	await _run_mission_suite_internal()


func _run_mission_suite_and_maybe_quit() -> void:
	var exit_code := await _run_mission_suite_internal()
...
func _run_mission_suite_internal() -> int:
	_failures.clear()
	_set_status("Preparing First Mission Spine bench...")
	if not _prepare_output_dir():
		return _finish_mission_suite([])
	...
	if screen == null:
		...
		return _finish_mission_suite([])
	screen.name = "FirstMissionSpineGameScreen"
	...
	return _finish_mission_suite(records)
...
func _run_step(viewport: SubViewport, screen: Control, step_data: Dictionary, emitted_action_ids: Array[String], emitted_end_turns: Array[String], overlay_action_ids: Array[String]) -> Dictionary:
	...
	_set_status("Running first mission step: %s..." % step_id)
	...
	var overlay_checked := _overlay_node_present(screen)
	var privacy_checked := _privacy_surface_safe(screen)
	var coach_checked := _mission_coach_visible(screen)
	var mission_feedback_checked := true
	match step_id:
		"boot_first_mission":
			right_inspector_checked = _core_surface_present(screen) and _mission_surface_present(screen)
			player_feedback_checked = _player_board_mentions_mission(screen)
			coach_checked = _mission_coach_visible(screen)
		"inspect_first_card":
		...
		"read_public_track":
			...
			player_feedback_checked = _player_feedback_contains(screen, expected_action_id, "action")
			privacy_checked = _public_surface_safe(screen)
		...
		"end_turn":
			...
			right_inspector_checked = _mission_surface_present(screen)
			player_feedback_checked = _player_feedback_contains(screen, "end_turn", "action")
		"mission_step_feedback":
			clicked_action_id = expected_action_id
			await _click_mission_primary_action(viewport, screen)
			emitted_action_id = _latest_since(emitted_action_ids, before_action_count)
			mission_feedback_checked = _mission_complete_feedback_visible(screen)
			player_feedback_checked = _player_feedback_contains(screen, expected_action_id, "action")
			right_inspector_checked = _mission_surface_present(screen)
		_:
			_failures.append("unknown first mission step: %s" % step_id)
	var emit_ok := true
	match step_id:
		"execute_first_action", "read_public_track", "temporary_decision_if_present", "mission_step_feedback":
			emit_ok = emitted_action_id == expected_action_id
		"end_turn":
			emit_ok = emitted_action_id == "end_turn_requested"
		_:
			emit_ok = true
	var passed := right_inspector_checked and player_feedback_checked and overlay_checked and privacy_checked and coach_checked and mission_feedback_checked and emit_ok
	if not passed:
		_failures.append("%s failed: fixture=%s clicked=%s emitted=%s expected=%s inspector=%s feedback=%s overlay=%s privacy=%s coach=%s mission_feedback=%s" % [
			step_id,
			fixture_id,
			clicked_action_id,
			emitted_action_id,
	...
	var record := {
		...
		"mission_feedback_checked": mission_feedback_checked,
		"passed": passed,
...
func _preview_manifest_record(step_data: Dictionary) -> Dictionary:
	return {
		...
		"coach_checked": false,
		"mission_feedback_checked": false,
		"passed": false,
		"notes": str(step_data.get("notes", "")),
	}
...
func _click_card_action(viewport: SubViewport, screen: Control, fixture: Dictionary, card_id: String, disabled: bool, emitted_action_ids: Array[String]) -> bool:
	...
	if not disabled and emitted_action_ids.size() == before_count and not button.disabled:
		...
		await _pump_frames(4)
	if disabled:
		return button.disabled and emitted_action_ids.size() == before_count
...
func _click_mission_primary_action(viewport: SubViewport, screen: Control) -> void:
	var button := screen.find_child("ScenarioCoachPrimaryButton", true, false) as Button
	if button == null:
		button = screen.find_child("CoachPrimaryButton", true, false) as Button
	if button == null:
		_failures.append("mission primary button not found")
		return
	await _click_control(viewport, button)
	if not button.disabled:
		button.emit_signal("pressed")
...
func _core_surface_present(screen: Control) -> bool:
	for node_name in ["TopBar", "PlayerBoard", "HandRack", "RightInspector", "PublicTrack", "PlanetBoard", "OverlayLayer", "FirstRunCoach", "ScenarioCoach"]:
		if screen.find_child(node_name, true, false) == null:
			return false
	return true


func _mission_surface_present(screen: Control) -> bool:
	var text := _node_tree_text(screen)
	return text.contains("第一局") and text.contains("第一目标") and text.contains("手牌")


func _mission_coach_visible(screen: Control) -> bool:
	var first_text := _node_tree_text(screen.find_child("FirstRunCoach", true, false))
	var scenario_text := _node_tree_text(screen.find_child("ScenarioCoach", true, false))
	return (first_text + "\n" + scenario_text).contains("第一") and scenario_text.contains("任务")


func _player_board_mentions_mission(screen: Control) -> bool:
	var text := _node_tree_text(screen.find_child("PlayerBoard", true, false))
	return text.contains("第一局") and text.contains("目标")
...
func _temporary_decision_overlay_visible(screen: Control) -> bool:
	...
	return panel != null and panel.visible and text.contains("选择第一局合约目标")
...
func _mission_complete_feedback_visible(screen: Control) -> bool:
	var feedback := _runtime_feedback(screen)
	var board_feedback := _player_board_feedback(screen)
	var text := str(feedback.get("label", "")) + "\n" + str(feedback.get("detail", "")) + "\n" + str(board_feedback.get("label", "")) + "\n" + str(board_feedback.get("detail", ""))
	return text.contains("第一目标已完成")
...
func _public_surface_safe(screen: Control) -> bool:
...
func _fixtures_instance() -> RefCounted:
	if _fixtures != null:
		return _fixtures
	var script := load(FIRST_MISSION_FIXTURE_SCRIPT_PATH)
	if script == null:
		return null
...
func _active_viewport() -> SubViewport:
	...
	viewport.name = "FirstMissionSpinePreviewViewport"
...
func _build_markdown_report(manifest: Dictionary) -> String:
	...
	lines.append("# First Mission Spine QA")
	lines.append("")
	...
	lines.append("")
	lines.append("| Step | Fixture | Surface | Clicked | Emitted | Inspector | Feedback | Overlay | Privacy | Coach | Mission Feedback | Passed |")
	lines.append("| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |")
	for record_variant in manifest.get("records", []):
		...
		lines.append("| %s | %s | %s | `%s` | `%s` | %s | %s | %s | %s | %s | %s | %s |" % [
			...
			str(record.get("coach_checked", false)),
			str(record.get("mission_feedback_checked", false)),
			str(record.get("passed", false)),
		])
	return "\n".join(lines) + "\n"


func _finish_mission_suite(_records: Array) -> int:
	if _failures.is_empty():
		var message := "First Mission Spine QA complete. manifest=%s report=%s screenshot=%s" % [MANIFEST_PATH, REPORT_PATH, SCREENSHOT_PATH]
		print(message)
		_set_status(message)
		return 0
	var failure_text := "First Mission Spine QA failed:\n- %s" % "\n- ".join(_failures)
