...
$AuthorizationId = "alpha04c-ai-runtime-save-v3-checkpoint-v2-replay-v1"
$RunId = "alpha04c-ai-runtime-replay-v1-attempt-1"
$Pr77Baseline = "351ee15100dea6f4185a511ee360ca005e1cdfa5"
...
$evidenceFull = Join-Path $projectFull "reports/handoffs/alpha04c_ai_runtime_owner_replay_v1.json"
$parentFull = Join-Path $projectFull "reports/handoffs/alpha04c_ai_runtime_owner_replay_v1_parent_attestation.json"
$authorizationPath = Join-Path $projectFull "scripts/tools/ai_runtime_owner_replay_authorization_v1.json"
$characterizationPath = Join-Path $projectFull "reports/handoffs/alpha04c_ai_runtime_full_state_pre_v3_characterization.json"
$matrixPath = Join-Path $projectFull "docs/save/alpha04c_ai_save_field_authority_matrix.json"

Assert-Condition (Test-Path -LiteralPath $GodotPath -PathType Leaf) "godot_console_missing"
Assert-Condition (Test-Path -LiteralPath $authorizationPath -PathType Leaf) "ai_runtime_replay_authorization_missing"
Assert-Condition (-not (Test-Path -LiteralPath $evidenceFull) -and -not (Test-Path -LiteralPath $parentFull)) "ai_runtime_replay_evidence_already_exists"
...
Assert-Condition (
    (Test-ExactPropertySet -Value $authorization -Expected @(
        ...
        -and [string]$authorization.contract_id -ceq "AiRuntimeOwnerReplayAuthorizationV1" `
        -and [string]$authorization.authorization_id -ceq $AuthorizationId `
        -and [string]$authorization.run_id_prefix -ceq "alpha04c-ai-runtime-replay-v1" `
...
) "ai_runtime_replay_authorization_invalid"
...
Assert-Condition ($LASTEXITCODE -eq 0 -and $diffCheck.Count -eq 0) "ai_runtime_replay_git_diff_check_failed"
Assert-Condition ($dirtyPaths.Count -eq 0) "ai_runtime_replay_tree_not_clean"
Assert-Condition ($branch -cmatch "^codex/alpha04c-ai-runtime-save-v3-[0-9a-f]{7,12}$") "ai_runtime_replay_branch_invalid"
$remoteHead = (& git -C $projectFull rev-parse "refs/remotes/origin/$branch" 2>$null).Trim().ToLowerInvariant()
Assert-Condition ($LASTEXITCODE -eq 0 -and $remoteHead -ceq $repositoryHead) "ai_runtime_replay_remote_checkpoint_mismatch"
& git -C $projectFull merge-base --is-ancestor $Pr77Baseline $repositoryHead
Assert-Condition ($LASTEXITCODE -eq 0) "pr77_ai_repair_baseline_not_ancestor"
...
$remainingAttemptRoot = Join-Path $gitCommon "codex/cold_restore_v3/non-official-alpha04c-remaining-owner-preflight-index16-18-attempt-v3"
...
Assert-Condition (-not (Test-Path -LiteralPath $v8Root)) "v8_root_created_before_ai_runtime_replay"
Assert-Condition (-not (Test-Path -LiteralPath $remainingAttemptRoot)) "remaining_owner_attempt_v3_created_before_ai_runtime_replay"
...
$replayRoot = Join-Path $gitCommon "codex/cold_restore_v3/non-official-alpha04c-ai-runtime-replay-v1"
...
Assert-Condition (-not (Test-Path -LiteralPath $replayRoot)) "ai_runtime_replay_attempt_already_claimed"
$claim = [ordered]@{
    schema_version = 1
    claim_id = "AiRuntimeOwnerReplayAttemptClaimV1"
...
$admission = [ordered]@{
    schema_version = 1
    admission_id = "AiRuntimeOwnerReplayChildAdmissionV1"
...
$tempRoot = Join-Path $tempBase ("space-syndicate-alpha04c-ai-runtime-replay-v1-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    foreach ($argument in @(
        ...
        "res://scripts/tools/alpha04c_ai_runtime_nonconsuming_replay.gd",
    ...
    Assert-Condition ($process.Start()) "ai_runtime_replay_godot_launch_failed"
    $stdoutTask = $process.StandardOutput.ReadToEndAsync()
    $stderrTask = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(180000)) {
        $process.Kill($true)
        $process.WaitForExit()
        throw "ai_runtime_replay_timeout"
    }
    $stdout = $stdoutTask.GetAwaiter().GetResult()
    $stderr = $stderrTask.GetAwaiter().GetResult()
    $childExitCode = $process.ExitCode
    Assert-Condition (Test-Path -LiteralPath $evidenceFull -PathType Leaf) "ai_runtime_replay_child_evidence_missing"
    ...
    $childGreen = $childExitCode -eq 0 `
        ...
        -and [bool]$childResult.ai_runtime_replay_scenario_identity_green `
        -and [bool]$childResult.ai_runtime_replay_registry_binding_green `
        -and [bool]$childResult.runtime_loop_nondefault_ai_state_green `
        -and [int]$childResult.runtime_loop_ai_action_count -ge 1 `
        -and [int]$childResult.runtime_state_request_sequence -gt 0 `
        -and [bool]$childResult.ai_runtime_payload_closed `
        -and [bool]$childResult.ai_runtime_save_v3_replay_green `
        -and [bool]$childResult.ai_runtime_checkpoint_v2_replay_green `
        -and [bool]$childResult.ai_runtime_restore_parity `
        -and [bool]$childResult.ai_runtime_exact_once_green `
        -and [int]$childResult.ai_runtime_capture_mutation_count -eq 0 `
        -and [int]$childResult.duplicate_ai_action_submission_count -eq 0 `
        -and [int]$childResult.duplicate_ai_business_cost_debit_count -eq 0 `
        -and [int]$childResult.duplicate_ai_decision_sample_count -eq 0 `
        -and [int]$childResult.duplicate_ai_learning_update_count -eq 0 `
        ...
        -and -not [bool]$childResult.runtime_memory_injected `
        -and -not [bool]$childResult.runtime_action_injected `
    ...
    $parent = [ordered]@{
        ...
        v7_quota_ledger_sha256_before = $v7LedgerBefore
        v7_quota_ledger_sha256_after = $v7LedgerAfter
        ...
        remaining_owner_attempt_v3_root_created = [bool](Test-Path -LiteralPath $remainingAttemptRoot)
        ...
        parent_attestation_green = [bool]($childGreen -and $immutableV7 `
            -and $saveArtifacts.Count -eq 0 `
            -and -not (Test-Path -LiteralPath $remainingAttemptRoot) `
    ...
    Assert-Condition ([bool]$parent.parent_attestation_green) "ai_runtime_replay_parent_attestation_failed"
    [Console]::Out.WriteLine("ALPHA04C_AI_RUNTIME_REPLAY_PARENT|" + (ConvertTo-Json -InputObject $parent -Compress -Depth 8))
}
finally {
    if ($null -ne $process) {
        try {
    ...
    if ($resolvedTempRoot.StartsWith($tempBase, [StringComparison]::OrdinalIgnoreCase) `
            -and (Split-Path -Leaf $resolvedTempRoot).StartsWith("space-syndicate-alpha04c-ai-runtime-replay-v1-", [StringComparison]::Ordinal)) {
