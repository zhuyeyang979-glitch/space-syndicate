...
$evidenceFull = Join-Path $projectFull "reports/handoffs/alpha04c_v7_card_inventory_save_v4_checkpoint_v2_replay_v2.json"
$parentFull = Join-Path $projectFull "reports/handoffs/alpha04c_v7_card_inventory_save_v4_checkpoint_v2_replay_v2_parent_attestation.json"
$authorizationPath = Join-Path $projectFull "scripts/tools/card_inventory_owner_replay_authorization_v1.json"
$priorChildPath = Join-Path $projectFull "reports/handoffs/alpha04c_v7_card_inventory_save_v4_checkpoint_v2_replay.json"
$priorParentPath = Join-Path $projectFull "reports/handoffs/alpha04c_v7_card_inventory_save_v4_checkpoint_v2_parent_attestation.json"

Assert-Condition (Test-Path -LiteralPath $GodotPath -PathType Leaf) "godot_console_missing"
Assert-Condition (Test-Path -LiteralPath $authorizationPath -PathType Leaf) "replay_v2_authorization_missing"
Assert-Condition (-not (Test-Path -LiteralPath $evidenceFull) -and -not (Test-Path -LiteralPath $parentFull)) "replay_v2_evidence_already_exists"
Assert-Condition (Test-Path -LiteralPath $priorChildPath -PathType Leaf) "prior_replay_child_evidence_missing"
Assert-Condition (Test-Path -LiteralPath $priorParentPath -PathType Leaf) "prior_replay_parent_evidence_missing"
...
Assert-Condition (
    (Test-ExactPropertySet -Value $authorization -Expected @(
        ...
        -and [string]$authorization.contract_id -ceq "CardInventoryOwnerReplayAuthorizationV1" `
        -and [string]$authorization.authorization_id -ceq "alpha04c-v7-card-inventory-save-v4-checkpoint-v2-replay-v2-scenario-identity" `
        -and [string]$authorization.run_id_prefix -ceq "alpha04c-v7-card-inventory-replay-v2-scenario-identity" `
        -and [string]$authorization.run_id -ceq "alpha04c-v7-card-inventory-replay-v2-scenario-identity-attempt-2" `
        ...
        -and [string]$authorization.run_id -clike "$([string]$authorization.run_id_prefix)-*"
) "replay_v2_authorization_invalid"
Assert-Condition (
    -not [bool]$priorChild.success `
        -and [string]$priorChild.replay_run_id -ceq "alpha04c-v7-card-inventory-save-v4-checkpoint-v2-replay" `
        -and [string]$priorChild.reason_code -ceq "diagnostic_identity_ruleset_id_mismatch" `
        -and -not [bool]$priorParent.parent_attestation_green `
        -and [bool]$priorParent.immutable_v7_evidence_preserved `
        -and [int]$priorParent.replay_diagnostic_count_delta -eq 0 `
        -and [int]$priorParent.replay_quota_claim_count -eq 0
) "prior_replay_attempt_attestation_invalid"
...
Assert-Condition ($LASTEXITCODE -eq 0 -and $repositoryHead -cmatch "^[0-9a-f]{40}$") "repository_head_unavailable"
...
Assert-Condition ($LASTEXITCODE -eq 0 -and $diffCheck.Count -eq 0) "replay_v2_git_diff_check_failed"
Assert-Condition ($dirtyPaths.Count -eq 0) "replay_v2_tree_not_clean"
Assert-Condition (-not [string]::IsNullOrWhiteSpace($branch)) "replay_v2_branch_missing"
$remoteHead = (& git -C $projectFull rev-parse "refs/remotes/origin/$branch" 2>$null).Trim().ToLowerInvariant()
Assert-Condition ($LASTEXITCODE -eq 0 -and $remoteHead -ceq $repositoryHead) "replay_v2_remote_checkpoint_mismatch"
...
$replayRoot = Join-Path $gitCommon "codex/cold_restore_v3/non-official-alpha04c-card-inventory-replay-v2-scenario-identity"
...
Assert-Condition (-not (Test-Path -LiteralPath $replayRoot)) "replay_v2_attempt_already_claimed"
$claim = [ordered]@{
    schema_version = 1
    claim_id = "CardInventoryOwnerReplayAttemptClaimV1"
...
$admission = [ordered]@{
    schema_version = 1
    admission_id = "CardInventoryOwnerReplayChildAdmissionV1"
...
$tempRoot = Join-Path $tempBase ("space-syndicate-alpha04c-card-inventory-replay-v2-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    foreach ($argument in @(
        ...
        "res://scripts/tools/alpha04c_v7_card_inventory_nonconsuming_replay.gd",
    ...
    Assert-Condition ($process.Start()) "godot_replay_v2_launch_failed"
    $stdoutTask = $process.StandardOutput.ReadToEndAsync()
    $stderrTask = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(120000)) {
        $process.Kill($true)
        $process.WaitForExit()
        throw "godot_replay_v2_timeout"
    }
    $stdout = $stdoutTask.GetAwaiter().GetResult()
    $stderr = $stderrTask.GetAwaiter().GetResult()
    $childExitCode = $process.ExitCode
    Assert-Condition (Test-Path -LiteralPath $evidenceFull -PathType Leaf) "replay_v2_child_evidence_missing"
    ...
    $childGreen = $childExitCode -eq 0 `
        ...
        -and [string]$childResult.replay_run_id -ceq [string]$authorization.run_id `
        -and [string]$childResult.replay_authorization_id -ceq [string]$authorization.authorization_id `
        -and [bool]$childResult.replay_scenario_identity_attested `
        -and [bool]$childResult.scenario_identity_attested `
        -and [bool]$childResult.registry_binding_attested `
        -and -not [bool]$childResult.highest_target_ruleset_used_as_runtime_identity `
        -and [bool]$childResult.v7_card_inventory_save_v4_replay_green `
        -and [bool]$childResult.v7_card_inventory_checkpoint_v2_replay_green `
        -and [bool]$childResult.v7_card_inventory_restore_parity `
        -and [bool]$childResult.v7_card_inventory_payload_closed `
        -and [int]$childResult.v7_card_inventory_capture_mutation_count -eq 0 `
    ...
    $parent = [ordered]@{
        schema_version = 1
        replay_run_id = [string]$authorization.run_id
        replay_authorization_id = [string]$authorization.authorization_id
        repository_head = $repositoryHead
        ...
        parent_attestation_green = [bool]($childGreen -and $immutable -and $taskOwnedProcessCountAfter -eq 0)
        private_payload_redacted = $true
    }
    Write-JsonFile -Path $parentFull -Value $parent
    Assert-Condition ([bool]$parent.parent_attestation_green) "v7_card_inventory_replay_v2_parent_attestation_failed"
    [Console]::Out.WriteLine("ALPHA04C_V7_CARD_INVENTORY_REPLAY_V2_PARENT|" + (ConvertTo-Json -InputObject $parent -Compress -Depth 8))
}
finally {
    if ($null -ne $process) {
        try {
    ...
    if ($resolvedTempRoot.StartsWith($tempBase, [StringComparison]::OrdinalIgnoreCase) `
            -and (Split-Path -Leaf $resolvedTempRoot).StartsWith("space-syndicate-alpha04c-card-inventory-replay-v2-", [StringComparison]::Ordinal)) {
