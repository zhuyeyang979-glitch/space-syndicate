...
const CITY_DEVELOPMENT_SCRIPT_PATH := "res://tests/legacy_v05/runtime/city_development_runtime_controller_v05.gd"
...
func _evaluate_case(case_id: String) -> Dictionary:
	...
	match case_id:
		...
		"monster_and_wager_rules": return _record(case_id, int(monster.get("battle_limit_seconds", 0)) == 60 and int(monster.get("upgrade_delay_extension_seconds", 0)) == 60 and int(monster.get("wager_seconds", 0)) == 15 and int(monster.get("wager_standard_rate_bp", 0)) == 500, true, "Sixty-second battle, +60 upgrade delay, fifteen-second wager, standard 5% ante.")
