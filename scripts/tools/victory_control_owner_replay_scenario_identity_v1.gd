extends RefCounted
class_name VictoryControlOwnerReplayScenarioIdentityV1

const WIRE := preload("res://scripts/semantic/semantic_wire_v1.gd")
const SCALAR := preload("res://scripts/runtime/closed_save_scalar_codec_v1.gd")

const AUTHORIZATION_PATH := "res://scripts/tools/victory_control_owner_replay_authorization_v1.json"
const CHARACTERIZATION_PATH := "res://reports/handoffs/alpha04c_victory_control_full_state_pre_v3_characterization.json"
const AUTHORITY_MATRIX_PATH := "res://docs/save/alpha04c_victory_control_field_authority_matrix.json"
const REPLAY_ROOT_RELATIVE_PATH := "codex/cold_restore_v3/non-official-alpha04c-victory-control-replay-v1"
const CONTRACT_ID := "VictoryControlOwnerReplayScenarioIdentityV1"
const AUTHORIZATION_ID := "alpha04c-victory-control-save-v3-replay-v1"
const RUN_ID_PREFIX := "alpha04c-victory-control-replay-v1"
const RUN_ID := "alpha04c-victory-control-replay-v1-attempt-1"
...
const OWNER_INDEX := 17
const SECTION_ID := "victory_control"
const OWNER_ID := "victory_control"
const SAVE_SCHEMA_VERSION := 3
const REGISTRY_STATE_VERSION := 2
const CHECKPOINT_STRATEGY := "registry_managed_checkpoint"
const TIMER_BOUNDARY_EPSILON_MICROS := 1
const QUALIFICATION_DURATION_SECONDS := 10
const PUBLIC_AUDIT_DURATION_SECONDS := 120
...
const IDENTITY_FIELDS := [
	...
	"section_id",
	"owner_id",
	"victory_save_schema_version",
	"victory_registry_state_version",
	"victory_checkpoint_strategy",
	"clock_domain_ruleset_id",
	"qualification_duration_seconds",
	"public_audit_duration_seconds",
	"timer_boundary_epsilon_micros",
	"f64_codec_id",
...
static func authorization() -> Dictionary:
	...
	if not _has_exact_fields(value, AUTHORIZATION_FIELDS) \
			...
			or str(value.get("contract_id", "")) != "VictoryControlOwnerReplayAuthorizationV1" \
...
static func build(source: Dictionary) -> Dictionary:
	...
	var unsealed := {
		...
		"victory_save_schema_version": source.get("victory_save_schema_version", -1),
		"victory_registry_state_version": source.get("victory_registry_state_version", -1),
		"victory_checkpoint_strategy": source.get("victory_checkpoint_strategy", ""),
		"clock_domain_ruleset_id": source.get("clock_domain_ruleset_id", ""),
		"qualification_duration_seconds": source.get("qualification_duration_seconds", -1),
		"public_audit_duration_seconds": source.get("public_audit_duration_seconds", -1),
		"timer_boundary_epsilon_micros": TIMER_BOUNDARY_EPSILON_MICROS,
		"f64_codec_id": SCALAR.F64_CODEC_ID,
		"strict_semantic_wire_required": true,
		"runtime_state_source": "legal_typed_world_facts_and_advance_world_effective",
	}
...
static func validation_report(value: Variant, expected_repository_head := "") -> Dictionary:
	var auth := authorization()
	if auth.is_empty():
		return _rejected("victory_replay_authorization_invalid")
	if not (value is Dictionary):
		return _rejected("victory_replay_scenario_identity_not_dictionary")
	...
	if not _has_exact_fields(identity, IDENTITY_FIELDS) \
			...
			or int(identity.get("victory_save_schema_version")) != SAVE_SCHEMA_VERSION \
			or int(identity.get("victory_registry_state_version")) != REGISTRY_STATE_VERSION \
			or str(identity.get("victory_checkpoint_strategy")) != CHECKPOINT_STRATEGY \
			or str(identity.get("clock_domain_ruleset_id")) != PRODUCTION_RUNTIME_RULESET_ID \
			or int(identity.get("qualification_duration_seconds")) != QUALIFICATION_DURATION_SECONDS \
			or int(identity.get("public_audit_duration_seconds")) != PUBLIC_AUDIT_DURATION_SECONDS \
			or int(identity.get("timer_boundary_epsilon_micros")) != TIMER_BOUNDARY_EPSILON_MICROS \
			or str(identity.get("f64_codec_id")) != SCALAR.F64_CODEC_ID \
			or not bool(identity.get("strict_semantic_wire_required")) \
			or str(identity.get("runtime_state_source")) != "legal_typed_world_facts_and_advance_world_effective" \
			or str(identity.get("identity_fingerprint")) != WIRE.fingerprint(identity, "identity_fingerprint"):
		return _rejected("victory_replay_scenario_identity_invalid")
	return {"valid": true, "reason_code": "victory_replay_scenario_identity_valid"}
...
static func _identity_scalar_types_are_exact(identity: Dictionary) -> bool:
	for field in [
		...
		"owner_id", "victory_checkpoint_strategy", "clock_domain_ruleset_id",
		"f64_codec_id", "runtime_state_source", "identity_fingerprint",
	]:
		if not (identity.get(field) is String):
			return false
	for field in [
		"schema_version", "challenge_depth", "run_seed", "local_player_count",
		"ai_player_count", "owner_index", "victory_save_schema_version",
		"victory_registry_state_version", "qualification_duration_seconds",
		"public_audit_duration_seconds", "timer_boundary_epsilon_micros",
	]:
