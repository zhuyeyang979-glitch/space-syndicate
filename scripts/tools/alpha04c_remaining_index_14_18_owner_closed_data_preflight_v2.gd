...
const NEW_FIRST_OWNER_INDEX := 14
const NEW_LAST_OWNER_INDEX := 18
const NEW_EXPECTED_OWNER_COUNT := 5
const QUALIFIED_PRIOR_OWNER_COUNT := 6
const NEW_OUTPUT_PATH := "res://reports/handoffs/alpha04c_remaining_owner_preflight_index14_18_attempt_v2.json"
...
func _run() -> void:
	...
	if _normalize_absolute_path(output_path) != expected_output or FileAccess.file_exists(expected_output):
		_finish_attempt(result, "remaining_index_14_18_preflight_v2_evidence_path_invalid", 1)
		return
	if not _lower_hex(repository_head, 40) or ATTEMPT.authorization().is_empty():
		_finish_attempt(result, "remaining_index_14_18_preflight_v2_authorization_invalid", 1)
		return
	...
	if not bool(admission.get("accepted", false)):
		_finish_attempt(result, str(admission.get("reason_code", "remaining_index_14_18_preflight_v2_admission_invalid")), 1)
		return
	...
	if not bool(context.get("ready", false)):
		main.queue_free()
		await process_frame
		_finish_attempt(result, "remaining_index_14_18_preflight_v2_production_composition_unavailable", 1)
		return
	...
	if not bool(started.get("applied", false)):
		main.queue_free()
		await process_frame
		_finish_attempt(result, str(started.get("reason_code", "remaining_index_14_18_preflight_v2_session_start_failed")), 1)
		return
	...
	if not scenario_attested:
		main.queue_free()
		await process_frame
		_finish_attempt(result, "remaining_index_14_18_preflight_v2_scenario_identity_mismatch", 1)
		return
	...
	if not registry_attested:
		main.queue_free()
		await process_frame
		_finish_attempt(result, str(registry_report.get("reason_code", "remaining_index_14_18_preflight_v2_registry_invalid")), 1)
		return
	...
	result.merge({
		...
		"success": success,
		"reason_code": "remaining_index_14_18_preflight_v2_green" if success else first_failure_reason,
...
func _base_result(repository_head: String) -> Dictionary:
	return {
		...
		"preflight_id": "alpha04c_remaining_index_14_18_owner_closed_data_preflight_v2",
...
func _print_result(result: Dictionary) -> void:
	print("ALPHA04C_REMAINING_INDEX_14_18_OWNER_PREFLIGHT_V2|%s" % JSON.stringify(result))
