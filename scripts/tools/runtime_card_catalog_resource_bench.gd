...
const EXPECTED_CARD_COUNT := 230
const EXPECTED_FAMILY_COUNT := 113
const EXPECTED_PACK_COUNT := 10
const EXPECTED_POOL_COUNT := 116
const EXPECTED_UPGRADEABLE_COUNT := 70
const EXPECTED_KIND_COUNT := 48
...
const LIVE_CASE_IDS := [
	"resource_scripts_load", "family_asset_count_113", "embedded_rank_count_230",
	...
	"requirement_policy_stays_external", "all_48_kinds_validate", "pure_data_definition",
...
func run_resource_suite() -> void:
	...
	print("RuntimeCardCatalogResourceBench assets: 113 families; 230 authored ranks; 10 packs; 1 catalog")
...
func _historical_pass(case_id: String) -> bool:
	...
	match case_id:
		...
		"intel_card_shape":
			return _kinds_have_cards(["intel_city_reveal", "card_history_public_review", "card_history_subscription"])
...
func _live_pass(case_id: String) -> bool:
	...
	match case_id:
		...
		"family_asset_count_113": return _tres_count(FAMILY_DIR) == EXPECTED_FAMILY_COUNT
		"embedded_rank_count_230": return _embedded_rank_count() == EXPECTED_CARD_COUNT
		...
		"all_48_kinds_validate": return bool(report.get("valid", false)) and int(report.get("kind_count", 0)) == EXPECTED_KIND_COUNT
...
func _update_ui(manifest: Dictionary) -> void:
	summary_label.text = "230 cards | 113 families | 10 packs | 48 kinds | %d/%d passed" % [int(manifest.get("passed_count", 0)), CASE_COUNT]
...
func _markdown_report(manifest: Dictionary) -> String:
	var lines: Array[String] = [
		...
		"- Assets: 113 family Resources, 230 authored rank subresources, 10 pack Resources, 1 catalog Resource",
