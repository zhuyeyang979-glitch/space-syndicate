...
const CASE_IDS := [
	...
	"catalog_has_113_families",
	"catalog_has_230_authored_cards",
	"output_is_user_scoped",
	"authoritative_catalog_validates",
	"all_48_kinds_remain_registered",
...
func _case_pass(case_id: String) -> bool:
	match case_id:
		...
		"catalog_has_113_families": return int(_index.get("family_count", 0)) == 113
		"catalog_has_230_authored_cards": return int(_index.get("card_count", 0)) == 230
		...
		"all_48_kinds_remain_registered": return int(_catalog.validation_report().get("kind_count", 0)) == 48
...
func _update_ui(manifest: Dictionary) -> void:
	summary_label.text = "230 cards | 113 families | 10 packs | %d/%d workflow checks" % [int(manifest.get("passed_count", 0)), CASE_IDS.size()]
