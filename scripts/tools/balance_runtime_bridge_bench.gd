...
const ROUTE_BASELINE := {
	"city_growth": {"display_name": "城市成长", "goal": "建设、升级生产/需求/交通，把稳定GDP变成终局现金。", "play_pattern": "先建稳定城市，再补生产、需求和交通，把GDP按秒滚成现金。", "counterplay": "断商路、做空GDP、诱导怪兽踩城，或用订单改走需求。", "ai_plan_hint": "领先时继续修路/保险；落后时只在安全高GDP区域扩张。", "strategy_labels": ["城市成长"], "required_for_ai_baseline": true, "sort_order": 0},
	"contract_route": {"display_name": "订单供需", "goal": "用自动结算的条件订单与供货改变商品网络，让真实商路和市场容量产生收益。", "play_pattern": "先识别可达市场、路线方式和距离条件，再提交能由正式经济网络兑现的订单或供货。", "counterplay": "扩充或破坏路线容量、改变供需结构、抢占仓储，或让牌面条件在结算时失效。", "ai_plan_hint": "优先选择公开网络事实可满足、预计净现金高且容量充足的自动订单或供货。", "strategy_labels": ["条件式供需"], "required_for_ai_baseline": true, "sort_order": 1},
