...
$AttemptId = "alpha04c-remaining-owner-preflight-index16-18-attempt-v3"
$ExpectedOwnerIds = @("bankruptcy_neutral_estate", "victory_control", "game_session")
...
$evidenceFull = Join-Path $projectFull "reports/handoffs/alpha04c_remaining_owner_preflight_index16_18_attempt_v3.json"
$parentFull = Join-Path $projectFull "reports/handoffs/alpha04c_remaining_owner_preflight_index16_18_attempt_v3_parent_attestation.json"
$authorizationPath = Join-Path $projectFull "scripts/tools/remaining_owner_closed_data_preflight_attempt_v3_authorization.json"
$priorAttemptChildPath = Join-Path $projectFull "reports/handoffs/alpha04c_remaining_owner_preflight_index14_18_attempt_v2.json"
$priorAttemptParentPath = Join-Path $projectFull "reports/handoffs/alpha04c_remaining_owner_preflight_index14_18_attempt_v2_parent_attestation.json"
$aiReplayChildPath = Join-Path $projectFull "reports/handoffs/alpha04c_ai_runtime_owner_replay_v1.json"
$aiReplayParentPath = Join-Path $projectFull "reports/handoffs/alpha04c_ai_runtime_owner_replay_v1_parent_attestation.json"

Assert-Condition (Test-Path -LiteralPath $GodotPath -PathType Leaf) "godot_console_missing"
Assert-Condition (Test-Path -LiteralPath $authorizationPath -PathType Leaf) "remaining_owner_preflight_attempt_v3_authorization_missing"
Assert-Condition (-not (Test-Path -LiteralPath $evidenceFull) -and -not (Test-Path -LiteralPath $parentFull)) "remaining_owner_preflight_attempt_v3_evidence_already_exists"
...
Assert-Condition (
    (Test-ExactPropertySet -Value $authorization -Expected @(
        ...
        "targeted_owner_capture_diagnostic_count_after", "prior_attempt_v2_child_sha256",
        "prior_attempt_v2_parent_sha256", "ai_replay_child_sha256",
        "ai_replay_parent_sha256", "ai_replay_evidence_commit"
    )) `
        -and (Test-ExactJsonInt64 -Value $authorization.schema_version -Expected 1) `
        -and [string]$authorization.contract_id -ceq "RemainingOwnerClosedDataPreflightAttemptV3Authorization" `
        -and [string]$authorization.attempt_id -ceq $AttemptId `
        -and (Test-ExactJsonInt64 -Value $authorization.start_index -Expected 16) `
        -and (Test-ExactJsonInt64 -Value $authorization.end_index -Expected 18) `
        -and (Test-ExactJsonInt64 -Value $authorization.qualified_prior_owner_count -Expected 8) `
...
) "remaining_owner_preflight_attempt_v3_authorization_invalid"

foreach ($path in @($priorAttemptChildPath, $priorAttemptParentPath, $aiReplayChildPath, $aiReplayParentPath)) {
    Assert-Condition (Test-Path -LiteralPath $path -PathType Leaf) "remaining_owner_preflight_prerequisite_evidence_missing"
}
Assert-Condition ((Get-FileHash -LiteralPath $priorAttemptChildPath -Algorithm SHA256).Hash.ToLowerInvariant() -ceq [string]$authorization.prior_attempt_v2_child_sha256) "prior_attempt_v2_child_hash_mismatch"
Assert-Condition ((Get-FileHash -LiteralPath $priorAttemptParentPath -Algorithm SHA256).Hash.ToLowerInvariant() -ceq [string]$authorization.prior_attempt_v2_parent_sha256) "prior_attempt_v2_parent_hash_mismatch"
Assert-Condition ((Get-FileHash -LiteralPath $aiReplayChildPath -Algorithm SHA256).Hash.ToLowerInvariant() -ceq [string]$authorization.ai_replay_child_sha256) "ai_runtime_replay_child_hash_mismatch"
Assert-Condition ((Get-FileHash -LiteralPath $aiReplayParentPath -Algorithm SHA256).Hash.ToLowerInvariant() -ceq [string]$authorization.ai_replay_parent_sha256) "ai_runtime_replay_parent_hash_mismatch"
...
Assert-Condition (
    -not [bool]$priorChild.success `
        -and [int]$priorChild.total_remaining_owner_preflight_count -eq 8 `
        -and [int]$priorChild.total_remaining_owner_preflight_green_count -eq 7 `
        -and [int]$priorChild.first_remaining_owner_failure_index -eq 15 `
        -and [string]$priorChild.first_remaining_owner_failure_id -ceq "ai_runtime" `
        -and [bool]$priorParent.parent_attestation_green
) "prior_attempt_v2_evidence_invalid"

$aiReplayChild = Get-Content -LiteralPath $aiReplayChildPath -Raw | ConvertFrom-Json
$aiReplayParent = Get-Content -LiteralPath $aiReplayParentPath -Raw | ConvertFrom-Json
Assert-Condition (
    [bool]$aiReplayChild.success `
        -and [bool]$aiReplayChild.ai_runtime_save_v3_replay_green `
        -and [bool]$aiReplayChild.ai_runtime_checkpoint_v2_replay_green `
        -and [bool]$aiReplayChild.ai_runtime_payload_closed `
        -and [bool]$aiReplayChild.ai_runtime_restore_parity `
        -and [bool]$aiReplayChild.ai_runtime_exact_once_green `
        -and [int]$aiReplayChild.targeted_owner_capture_diagnostic_count_after -eq 7 `
        -and [bool]$aiReplayParent.parent_attestation_green
) "ai_runtime_replay_v1_not_green"
...
Assert-Condition ($LASTEXITCODE -eq 0 -and $diffCheck.Count -eq 0) "remaining_owner_preflight_attempt_v3_diff_check_failed"
Assert-Condition ($repositoryHead -cmatch "^[0-9a-f]{40}$" -and $dirtyPaths.Count -eq 0) "remaining_owner_preflight_attempt_v3_tree_not_clean"
Assert-Condition ($branch -ceq "codex/alpha04c-save-resume-cold-restore-5b8601b") "remaining_owner_preflight_attempt_v3_branch_invalid"
$remoteHead = (& git -C $projectFull rev-parse "refs/remotes/origin/$branch" 2>$null).Trim().ToLowerInvariant()
Assert-Condition ($LASTEXITCODE -eq 0 -and $remoteHead -ceq $repositoryHead) "remaining_owner_preflight_attempt_v3_remote_checkpoint_mismatch"
& git -C $projectFull merge-base --is-ancestor ([string]$authorization.ai_replay_evidence_commit) $repositoryHead
Assert-Condition ($LASTEXITCODE -eq 0) "ai_runtime_replay_evidence_not_ancestor"
...
$attemptRoot = Join-Path $gitCommon "codex/cold_restore_v3/non-official-alpha04c-remaining-owner-preflight-index16-18-attempt-v3"
...
Assert-Condition (-not (Test-Path -LiteralPath $attemptRoot)) "remaining_owner_preflight_attempt_v3_already_claimed"

$startedAt = [DateTimeOffset]::UtcNow.ToString("o")
$claim = [ordered]@{
    schema_version = 1
    claim_id = "RemainingOwnerClosedDataPreflightAttemptClaimV3"
...
$admission = [ordered]@{
    schema_version = 1
    admission_id = "RemainingOwnerClosedDataPreflightChildAdmissionV3"
...
$tempRoot = Join-Path $tempBase ("space-syndicate-alpha04c-remaining-index-16-18-preflight-v3-" + [Guid]::NewGuid().ToString("N"))
...
try {
    ...
    foreach ($argument in @(
        ...
        "res://scripts/tools/alpha04c_remaining_index_16_18_owner_closed_data_preflight_v3.gd",
    ...
    Assert-Condition ($process.Start()) "remaining_owner_preflight_attempt_v3_launch_failed"
    $stdoutTask = $process.StandardOutput.ReadToEndAsync()
    $stderrTask = $process.StandardError.ReadToEndAsync()
    if (-not $process.WaitForExit(120000)) {
        $process.Kill($true)
        $process.WaitForExit()
        throw "remaining_owner_preflight_attempt_v3_timeout"
    }
    $stdout = $stdoutTask.GetAwaiter().GetResult()
    $stderr = $stderrTask.GetAwaiter().GetResult()
    $childExitCode = $process.ExitCode
    Assert-Condition (Test-Path -LiteralPath $evidenceFull -PathType Leaf) "remaining_owner_preflight_attempt_v3_child_evidence_missing"
    Assert-Condition ((Test-Path -LiteralPath $consumedPath -PathType Leaf) -and -not (Test-Path -LiteralPath $admissionPath)) "remaining_owner_preflight_attempt_v3_admission_not_consumed"
    ...
    for ($index = 0; $index -lt $ownerResults.Count; $index += 1) {
        ...
        $rowsValid = $rowsValid `
            ...
            -and [int]$row.owner_index -eq (16 + $index) `
            -and [string]$row.owner_id -ceq $ExpectedOwnerIds[$index] `
    ...
    $successOutcome = $childExitCode -eq 0 `
        ...
        -and $ownerResults.Count -eq 3 `
        -and [int]$child.new_remaining_owner_preflight_green_count -eq 3 `
    ...
    $blockedOutcome = $childExitCode -eq 2 `
        ...
        -and [int]$child.first_remaining_owner_failure_index -ge 16 `
        -and [int]$child.first_remaining_owner_failure_index -le 18 `
        -and $ownerResults.Count -eq ([int]$child.first_remaining_owner_failure_index - 15)
    ...
    $aiReplayImmutable = (Get-FileHash -LiteralPath $aiReplayChildPath -Algorithm SHA256).Hash.ToLowerInvariant() -ceq [string]$authorization.ai_replay_child_sha256 `
        -and (Get-FileHash -LiteralPath $aiReplayParentPath -Algorithm SHA256).Hash.ToLowerInvariant() -ceq [string]$authorization.ai_replay_parent_sha256
    ...
    $outcomeValid = [string]$child.attempt_id -ceq $AttemptId `
        ...
        -and [bool]$child.ai_runtime_replay_v1_green `
    ...
    $parentGreen = $outcomeValid -and $v7Immutable -and $aiReplayImmutable `
        -and $v8Absent -and $saveArtifacts.Count -eq 0 -and $taskOwnedProcessCountAfter -eq 0
    ...
    $parentCore = [ordered]@{
        ...
        immutable_ai_replay_evidence_preserved = [bool]$aiReplayImmutable
    ...
    $ledger = [ordered]@{
        schema_version = 1
        contract_id = "RemainingOwnerClosedDataPreflightAttemptV3"
        attempt_id = $AttemptId
        frozen_code_head = $repositoryHead
        start_index = 16
        end_index = 18
    ...
    Assert-Condition $parentGreen "remaining_owner_preflight_attempt_v3_parent_attestation_failed"
    [Console]::Out.WriteLine("ALPHA04C_REMAINING_INDEX_16_18_OWNER_PREFLIGHT_V3_PARENT|" + (ConvertTo-Json -InputObject $parent -Compress -Depth 12))
    exit $childExitCode
}
finally {
    if ($null -ne $process) {
    ...
    if ($resolvedTempRoot.StartsWith($tempBase, [StringComparison]::OrdinalIgnoreCase) `
            -and (Split-Path -Leaf $resolvedTempRoot).StartsWith("space-syndicate-alpha04c-remaining-index-16-18-preflight-v3-", [StringComparison]::Ordinal)) {
