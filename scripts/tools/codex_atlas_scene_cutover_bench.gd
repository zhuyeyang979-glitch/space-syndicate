...
const QA_SAVE_PATH := "user://test_runs/codex_atlas_scene_cutover.save"
...
func run_cutover_suite() -> void:
	...
	if not await _ensure_main():
		...
		await _dispose_main()
		push_error("CodexAtlasSceneCutoverBench failed: formal session startup")
		get_tree().quit(1)
		return
	for case_id_variant: Variant in cutover_cases():
...
func _ensure_main() -> bool:
	var start_result := await SESSION_START_DRIVER.start_default_session(get_tree(), QA_SAVE_PATH, "codex-atlas-scene-cutover")
...
func _open_bestiary_browser() -> void:
	_open_compendium_domain("monster")


func _open_product_browser() -> void:
	_open_compendium_domain("product")


func _open_compendium_domain(domain: String) -> bool:
	if not _open_compendium_hub():
		return false
	var flow := _main.get_node_or_null("RuntimeServices/CompendiumApplicationFlowController") as CompendiumApplicationFlowController if _main != null else null
	return flow != null and flow.handle_surface_action("hub_action", {"action_id": domain})
