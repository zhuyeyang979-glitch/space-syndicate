extends RefCounted
class_name RemainingOwnerClosedDataPreflightAttemptV3

const AUTHORIZATION_PATH := "res://scripts/tools/remaining_owner_closed_data_preflight_attempt_v3_authorization.json"
const PRIOR_ATTEMPT_CHILD_PATH := "res://reports/handoffs/alpha04c_remaining_owner_preflight_index14_18_attempt_v2.json"
const PRIOR_ATTEMPT_PARENT_PATH := "res://reports/handoffs/alpha04c_remaining_owner_preflight_index14_18_attempt_v2_parent_attestation.json"
const AI_REPLAY_CHILD_PATH := "res://reports/handoffs/alpha04c_ai_runtime_owner_replay_v1.json"
const AI_REPLAY_PARENT_PATH := "res://reports/handoffs/alpha04c_ai_runtime_owner_replay_v1_parent_attestation.json"
const ATTEMPT_ROOT_RELATIVE_PATH := "codex/cold_restore_v3/non-official-alpha04c-remaining-owner-preflight-index16-18-attempt-v3"
const CONTRACT_ID := "RemainingOwnerClosedDataPreflightAttemptV3"
const ATTEMPT_ID := "alpha04c-remaining-owner-preflight-index16-18-attempt-v3"
...
const AUTHORIZATION_FIELDS := [
	...
	"prior_attempt_v2_child_sha256",
	"prior_attempt_v2_parent_sha256",
	"ai_replay_child_sha256",
	"ai_replay_parent_sha256",
	"ai_replay_evidence_commit",
]
const CLAIM_FIELDS := [
	"schema_version",
	"claim_id",
...
static func authorization() -> Dictionary:
	...
	if not _has_exact_fields(value, AUTHORIZATION_FIELDS) \
			...
			or str(value.get("contract_id", "")) != "RemainingOwnerClosedDataPreflightAttemptV3Authorization" \
		...
		return {}
	for field in [
		"prior_attempt_v2_child_sha256", "prior_attempt_v2_parent_sha256",
		"ai_replay_child_sha256", "ai_replay_parent_sha256",
	]:
		if not _lower_hex(str(value.get(field, "")), 64):
			return {}
	if not _lower_hex(str(value.get("ai_replay_evidence_commit", "")), 40) \
			or FileAccess.get_sha256(PRIOR_ATTEMPT_CHILD_PATH).to_lower() != str(value.get("prior_attempt_v2_child_sha256", "")) \
			or FileAccess.get_sha256(PRIOR_ATTEMPT_PARENT_PATH).to_lower() != str(value.get("prior_attempt_v2_parent_sha256", "")) \
			or FileAccess.get_sha256(AI_REPLAY_CHILD_PATH).to_lower() != str(value.get("ai_replay_child_sha256", "")) \
			or FileAccess.get_sha256(AI_REPLAY_PARENT_PATH).to_lower() != str(value.get("ai_replay_parent_sha256", "")):
		return {}
...
static func consume_child_admission(
	...
	if attempt_root.is_empty() or claim_path != expected_claim \
		...
		return _rejected("remaining_owner_preflight_attempt_v3_admission_path_invalid")
	...
	if claim_text.is_empty() or claim_text.sha256_text().to_lower() != claim_sha256:
		return _rejected("remaining_owner_preflight_attempt_v3_claim_sha256_invalid")
	...
	if not (claim_variant is Dictionary) or not (admission_variant is Dictionary):
		return _rejected("remaining_owner_preflight_attempt_v3_admission_invalid")
	...
	if not _has_exact_fields(claim, CLAIM_FIELDS) \
			...
			or str(claim.get("claim_id", "")) != "RemainingOwnerClosedDataPreflightAttemptClaimV3" \
		...
		return _rejected("remaining_owner_preflight_attempt_v3_claim_invalid")
	if not _has_exact_fields(admission, ADMISSION_FIELDS) \
			or int(admission.get("schema_version", 0)) != 1 \
			or str(admission.get("admission_id", "")) != "RemainingOwnerClosedDataPreflightChildAdmissionV3" \
	...
	if DirAccess.rename_absolute(admission_path, consumed_path) != OK:
		return _rejected("remaining_owner_preflight_attempt_v3_admission_already_consumed")
	return {
		"accepted": true,
		"reason_code": "remaining_owner_preflight_attempt_v3_admission_consumed",
