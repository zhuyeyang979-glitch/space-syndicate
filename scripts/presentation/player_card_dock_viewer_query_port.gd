...
var _composed_bundle_count := 0
var _standalone_compose_count := 0
...
func snapshot_for_viewer(viewer_index: int, expected_authorization_revision: int = -1) -> Dictionary:
	...
	_standalone_compose_count += 1
	return _snapshot_from_composed_cards(
		viewer_index,
...
func snapshot_for_viewer_with_composed_cards(
	viewer_index: int,
...
func _snapshot_from_composed_cards(
	viewer_index: int,
	expected_authorization_revision: int,
	hand_sources: Array,
	hand_viewmodels: Array
	...
	if hand_sources.size() != hand_viewmodels.size():
		return _reject("player-card-dock-composed-bundle-mismatch")
...
func debug_snapshot() -> Dictionary:
	return {
		...
		"composed_bundle_count": _composed_bundle_count,
		"standalone_compose_count": _standalone_compose_count,
