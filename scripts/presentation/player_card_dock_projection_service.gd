...
func compose_shared_v06(
	...
	var source_slots: Array = source_by_slot.keys()
	var viewmodel_slots: Array = viewmodel_by_slot.keys()
	slots.sort()
	source_slots.sort()
	viewmodel_slots.sort()
	if private_by_slot.size() != private_hand.size() \
			...
			or slots != source_slots \
			or slots != viewmodel_slots:
		return {}
...
func _normal_card(
	...
	return {
		...
		"slot_id": str(OFFER.target_ids(offer).get("hand_slot_id", "")),
		...
		"disabled_reason_text": str(viewmodel.get(
			"block_reason",
			viewmodel.get("why", offer.get("disabled_reason_id", "当前不可用"))
		)),
...
func _bound_action(
	...
	return {
		...
		"source_display_name": str(skill.get(
			"source_display_name",
			("怪兽 %d" if source_kind == "monster" else "军队 %d") % source_uid
		)),
		"display_name": str(viewmodel.get("name", skill.get("display_name", skill.get("name", card_semantic_id)))),
		"illustration_key": str(viewmodel.get("illustration_key", "")),
