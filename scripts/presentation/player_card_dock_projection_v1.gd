...
static func validation_report(value: Variant) -> Dictionary:
	...
	var projection_revision := int(projection.get("source_revision", -1))
	...
	for value_variant in normal_cards:
		...
		if int((value_variant as Dictionary).get("source_revision", -1)) != projection_revision:
			return _invalid("player_card_dock_normal_card_projection_revision_mismatch")
	...
	for value_variant in commodity_cards:
		...
		if not reason.is_empty():
			return _invalid(reason)
		if int((value_variant as Dictionary).get("source_revision", -1)) != projection_revision:
			return _invalid("player_card_dock_commodity_card_projection_revision_mismatch")
	...
	for value_variant in bound_actions:
		...
		if not reason.is_empty():
			return _invalid(reason)
		if int((value_variant as Dictionary).get("source_revision", -1)) != projection_revision:
			return _invalid("player_card_dock_bound_action_projection_revision_mismatch")
...
static func _normal_card_error(value: Variant) -> String:
	...
	if not WIRE.exact_fields(card, NORMAL_CARD_FIELDS) \
			...
			or not WIRE.is_stable_id(card.get("slot_id")) \
			...
			or not (card.get("disabled_reason_text") is String) \
...
static func _bound_action_error(value: Variant) -> String:
	...
	if not WIRE.exact_fields(action, BOUND_ACTION_FIELDS) \
			...
			or not (action.get("source_display_name") is String) \
