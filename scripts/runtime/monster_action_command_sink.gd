...
func is_ready() -> bool:
	return _authority != null and _monster != null and bool(_authority.debug_snapshot().get("ready", false))
...
func apply_command(command: Dictionary, _envelope: Dictionary = {}) -> Dictionary:
	...
	if _applied_command_ids.has(command_id):
		if str(command.get("action_kind", "")) == "battle_opening_attack" \
			and _monster.monster_battle_opening_attack_command_matches(command):
			return {
				"handled": true,
				"accepted": true,
				"idempotent_replay": true,
				"reason": "monster_battle_opening_attack_already_applied",
				"actor_uid": int(command.get("actor_uid", -1)),
				"action_index": int(command.get("action_index", -1)),
			}
		# A loaded pre-command owner snapshot is authoritative. Its saved
		# opening flag deliberately reopens this bounded sink cache entry.
		if str(command.get("action_kind", "")) == "battle_opening_attack":
			_applied_command_ids.erase(command_id)
		else:
	...
	var battle_action := str(command.get("action_kind", "")) == "battle_opening_attack"
	var target_uid := int(command.get("target_monster_uid", -1))
	var wager_id := int(command.get("wager_id", -1))
	var before := _monster.simulation_battle_action_mutation_snapshot(actor_uid, target_uid, wager_id) \
		if battle_action else _monster.simulation_mutation_snapshot_by_uid(actor_uid)
	if before.is_empty():
	...
	var after := _monster.simulation_battle_action_mutation_snapshot(actor_uid, target_uid, wager_id) \
		if battle_action else _monster.simulation_mutation_snapshot_by_uid(actor_uid)
	if after.is_empty():
	...
	var audit := _authority.record_mutation(mutation_command, before, after, {
		"domain": "monster",
		"mutation_kind": "monster_battle_opening_attack" if battle_action else "autonomous_action",
		"target_key": "monster-battle:%d:%d:%d" % [wager_id, actor_uid, target_uid] if battle_action else "monster:%d" % actor_uid,
		"outcome": "applied",
