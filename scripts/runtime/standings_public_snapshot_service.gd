...
func _summary_text(source: Dictionary) -> String:
	...
	var lines := [
		...
		"未进入审计名单的对手资产继续保密；只有胜利系统明确标记的名单席位才公开准确现金账本。",
	]
...
func _scoreboard_snapshot(source: Dictionary, seats: Array, overview_cards: Array) -> Dictionary:
	...
	return {
		...
		"kpis": [
			...
			{"title": "审计名单", "value": "%d席" % int((_victory_snapshot(source).get("audit_roster", []) as Array).size()), "meta": _state_label(str(_victory_snapshot(source).get("state", "idle"))), "accent": Color("#fb923c"), "tooltip": "名单公开胜利进度；准确现金还需要胜利系统的双重公开标记。"},
		...
		"hint": "审计名单公开胜利进度；准确现金只在胜利系统明确授权时显示。",
	}
...
func _chip_snapshots(source: Dictionary) -> Array:
	...
	return [
		...
		{"text": "名单明牌", "accent": Color("#c4b5fd"), "tooltip": "名单公开胜利进度；准确现金需要权威公开标记，私人手牌和情报始终隐藏。"},
	]
...
func _seat_snapshots(source: Dictionary, seats: Array) -> Array:
	...
	for seat_index in range(seats.size()):
		...
		var authorized_cash: Variant = _authorized_audit_cash_cents(source, audit, player_index)
		var cash_is_public := authorized_cash != null
		var chips := []
		if eliminated:
			...
			if cash_is_public:
				chips.append({"text": "账本¥%.2f" % (float(int(authorized_cash)) / 100.0), "accent": Color("#fef3c7"), "tooltip": "胜利系统明确授权公开的准确现金总账。"})
			elif can_view_private:
		...
		result.append({
			...
			"chips": chips,
			"meta": "审计账本公开" if cash_is_public else ("审计进度公开" if publicly_audited else ("已淘汰" if eliminated else "经济资产保密")),
...
func _audit_entry(source: Dictionary, player_index: int) -> Dictionary:
	...
	for entry_variant in _victory_snapshot(source).get("audit_entries", []):
		...
		if typeof(entry.get("player_index")) != TYPE_INT or int(entry.get("player_index", -1)) != player_index:
			continue
		if matched.is_empty():
			matched = entry.duplicate(true)
		elif matched != entry:
			return {}
	return matched


func _authorized_audit_cash_cents(source: Dictionary, audit: Dictionary, player_index: int) -> Variant:
	var victory := _victory_snapshot(source)
	if str(victory.get("cash_visibility", "")) != "public_audit":
		return null
	var revealed := false
	var revealed_variant: Variant = victory.get("audit_revealed_player_indices", [])
	if revealed_variant is Array:
		for index_variant in revealed_variant:
			if typeof(index_variant) == TYPE_INT and int(index_variant) == player_index:
				revealed = true
				break
	if not revealed:
		return null
	if typeof(audit.get("player_index")) != TYPE_INT or int(audit.get("player_index", -1)) != player_index:
		return null
	if str(audit.get("cash_visibility", "")) != "public_audit":
		return null
	if typeof(audit.get("cash_ledger_cents")) != TYPE_INT:
		return null
	return int(audit.get("cash_ledger_cents"))
