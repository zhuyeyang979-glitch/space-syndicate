@tool
extends Node
class_name MilitaryMonsterDamageCommandSink
...
func apply_command(command: Dictionary, envelope: Dictionary = {}) -> Dictionary:
	if not is_ready():
		return _reject("military_monster_damage_dependencies_unavailable")
	var command_id := str(command.get("command_id", "")).strip_edges()
	if command_id.is_empty():
		return _reject("military_monster_damage_command_id_missing")
	if _applied_command_ids.has(command_id):
		return _reject("military_monster_damage_duplicate_command")
	...
	if not bool(authorization.get("authorized", false)):
		return _reject(str(authorization.get("reason", "military_monster_damage_unauthorized")))
	var target_uid := int(command.get("target_monster_uid", -1))
	var before := _monster.simulation_mutation_snapshot_by_uid(target_uid)
	if before.is_empty():
		return _reject("military_monster_damage_target_unavailable")
	var mutation := _monster.apply_external_damage_by_uid(command)
	if not bool(mutation.get("accepted", false)):
		return _reject(str(mutation.get("reason", "military_monster_damage_target_rejected")))
	var after := _monster.simulation_mutation_snapshot_by_uid(target_uid)
	if after.is_empty():
		return _reject("military_monster_damage_postimage_unavailable")
	var mutation_command := command.duplicate(true)
	mutation_command["command_type"] = String(RuntimeCommandEnvelope.TYPE_MILITARY_MONSTER_DAMAGE)
	var audit := _authority.record_mutation(mutation_command, before, after, {
		"domain": "monster",
		"mutation_kind": "external_damage",
		"target_key": "monster:%d" % target_uid,
		"outcome": "applied",
		"applied_damage": int(mutation.get("applied_damage", 0)),
	})
	if not bool(audit.get("recorded", false)):
		return _reject(str(audit.get("reason", "military_monster_damage_audit_rejected")))
	_applied_command_ids[command_id] = true
	while _applied_command_ids.size() > 64:
