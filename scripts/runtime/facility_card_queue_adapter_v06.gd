...
func configure(
	...
	_configured = _dependencies_ready() and bool(submission_binding.get("bound", false))
	_last_reason_code = "configured" if _configured else str(submission_binding.get(
		"reason_code",
		"facility_queue_dependencies_missing"
	))
	return {
...
func submit(capability: RefCounted, request: Dictionary) -> Dictionary:
	...
	if not bool(escrow_commit.get("committed", false)):
		return _reject(str(escrow_commit.get("reason_code", "facility_card_escrow_commit_failed")))
	...
	if not bool(mana_commit.get("authorized", false)):
		_card_state.release_facility_card_escrow(
			escrow_id,
			escrow_fingerprint,
			"asset_reservation_commit_failed"
		)
		return _reject(str(mana_commit.get("reason", "asset_reservation_commit_failed")))
	...
	if binding.is_empty():
		...
		_rollback_submission_owners(escrow_id, escrow_fingerprint, reservation_id, reason_code)
		return _reject(reason_code)
	...
	if not bool(queue_commit.get("committed", false)):
		_rollback_submission_owners(escrow_id, escrow_fingerprint, reservation_id, "queue_commit_failed")
		return _reject(str(queue_commit.get("reason", "facility_queue_commit_failed")))
	_queued_count += 1
...
func resolve(entry: Dictionary) -> Dictionary:
	...
	if not bool(escrow_finalize.get("finalized", false)):
		var compensation := _rollback_resolution_owners(
			...
			reservation_ref,
			bool(reservation_ref.get("required", false)),
			"facility_card_escrow_finalize_failed"
		)
		return _resolution_failure("facility_card_escrow_finalize_failed", compensation)
	...
	if not bool(finalization.get("finalized", false)):
		...
		var compensation := _rollback_after_escrow_finalization(
			effect_receipt,
			escrow_finalize,
			current.get("card", {}) as Dictionary,
			reservation_ref,
...
func _rollback_after_escrow_finalization(
	effect_receipt: Dictionary,
	escrow_finalize: Dictionary,
	card_record: Dictionary,
	reservation_ref: Dictionary,
	reason_code: String
) -> Dictionary:
	var safe_reason := _safe_reason(reason_code)
	var card_compensation := _card_state.compensate_finalized_facility_card_escrow(
		str(escrow_finalize.get("escrow_id", "")),
		str(escrow_finalize.get("receipt_fingerprint", "")),
		card_record,
		safe_reason
	)
	if not bool(card_compensation.get("compensated", false)):
		return {
			"settled": false,
			"reason_code": safe_reason,
			"card_compensation": card_compensation.duplicate(true),
		}
	var facility_rollback := _core.rollback_queued_facility_card(effect_receipt)
	var asset_settlement := _settle_asset_release(reservation_ref, safe_reason)
	var settled := bool(card_compensation.get("released", false)) \
		and bool(facility_rollback.get("rolled_back", false)) \
		and bool(asset_settlement.get("settled", false))
	if settled:
		_rollback_count += 1
	return {
		"settled": settled,
		...
		"card_compensation": card_compensation.duplicate(true),
		"facility_rollback": facility_rollback.duplicate(true),
		"asset_settlement": asset_settlement.duplicate(true),
	}
...
func _rollback_submission_owners(
	...
	if not reservation_id.is_empty() and not _mana.reservation_snapshot(reservation_id).is_empty():
		_mana.release_reservation(reservation_id, reason_code)
	var escrow_snapshot := _card_state.facility_card_escrow_snapshot(escrow_id)
	if bool(escrow_snapshot.get("found", false)):
		var escrow: Dictionary = escrow_snapshot.get("escrow", {}) as Dictionary
		_card_state.release_facility_card_escrow(
			escrow_id,
			str(escrow.get("escrow_fingerprint", escrow_fingerprint)),
			reason_code
		)
	_rollback_count += 1
