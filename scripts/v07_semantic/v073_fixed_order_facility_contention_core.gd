extends RefCounted
class_name V073FixedOrderFacilityContentionCore
...
const CORE_AUTHORITY_ID := "v073.facility_contention.core_authority.v1"
const AI_OBSERVATION_ID := "v073.facility_contention.ai_observation.v1"
const PLAYER_PROJECTION_ID := "v073.facility_contention.player_projection.v1"
const FACILITY_ACTION_INTENT_ID := "v073.facility_contention.intent.v1"
const RECEIPT_ID := "v073.facility_contention.authoritative_receipt.v1"
const SAVE_STATE_ID := "v073.facility_contention.save_state.v1"
const PUBLIC_PROJECTION_ID := "v073.facility_contention.public_projection.v1"
...
const FACILITY_TYPES := ["factory", "market"]
const INDUSTRIES := [
	"life",
	"energy",
	"industry",
...
static func build_new_action(
	...
	if _slot_error(slot) != "" or slot.get("occupancy") != "empty":
		return {}
...
static func build_upgrade_action(
	...
	if _slot_error(slot) != "" or slot.get("occupancy") != "occupied" \
...
static func _apply_successful_action(state: Dictionary, action: Dictionary) -> void:
	...
	match str(action.get("facility_action_mode", "")):
		...
		"UPGRADE_OWN":
			slot["facility_generation"] = int(slot.get("facility_generation", 0)) + 1
			slot["rank"] = int(slot.get("rank", 0)) + 1
		"REPAIR_OWN":
			slot["facility_generation"] = int(slot.get("facility_generation", 0)) + 1
...
static func _state_error(value: Variant) -> String:
	...
	if state.get("status") != expected_status \
			or int(state.get("revision", 0)) != cursor + 1:
