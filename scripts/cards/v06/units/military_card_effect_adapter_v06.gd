extends RefCounted
class_name MilitaryCardEffectAdapterV06

const PORT_SCRIPT := preload("res://scripts/cards/v06/units/military_card_owner_port_v06.gd")
...
func capability_matrix() -> Dictionary:
	return _port.call("capability_matrix") if _port != null else {"atomic_mutation_ready": false, "capability_reason": "military_owner_port_unconfigured"}


func checkpoint_status() -> Dictionary:
	return _port.call("checkpoint_status") if _port != null else {"can_checkpoint": false, "reason_code": "military_owner_port_unconfigured", "inflight_count": -1}


func _unconfigured(source: Dictionary, stage: String) -> Dictionary:
	return UnitCardRuntimeSchemaV06.failure_receipt(source, "military_owner_port_unconfigured", "军队效果尚未接入。", "请选择其他卡牌。", {"stage": stage})
...
func _normalize_card_flow_intent(intent: Dictionary) -> Dictionary:
	...
	var action_kind := effect_kind if effect_kind == "deploy_or_upgrade_military" else str(payload.get("action_kind", target.get("action_kind", "")))
