...
The final visual review also caught a post-integration issue that the original outer-panel audit could not see: detailed district labels overlapped after the globe blended into the local projection. District labels now use deterministic collision-aware placement in every projection mode, with non-selected labels compacted while space is tight or the camera is moving. The collision audit rechecks the local projection at 1366x768, 1600x960, and 1920x1080; all district-label and interactive-control intersection counts are zero. Hidden or completed Coach Marks no longer contribute stale pointer-occlusion counters.
