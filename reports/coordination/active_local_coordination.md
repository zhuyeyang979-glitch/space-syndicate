...
- Region Codex is now scene-owned and viewer-invariant. `RegionCodexPublicSourceService` consumes only strict RegionInfrastructure public facts, Monster owner's nonnumeric attraction projection, Weather summary, Route load, and the existing formatter; Main's three Region source/wrapper helpers are physically deleted for a further 97-line/3-function reduction.
- C's former Region privacy red gate is now `18/18` green: selected player, city guesses, exact cash, hand/discard, hidden city/monster owner, and AI private plan do not change source or final snapshot; sanitized public aftermath changes only the public clue path. The three legitimate private Intel helper callers remain isolated.
- Current Region milestone gates are green: SourceService `13/13`, Codex public cutover `20/20`, Codex hard cutover `20/20`, Main composition PASS, smoke check-only, and an 8775 headed `PASS 13/13` real Surface render. No cloud push has occurred.
