...
以后每一轮开发默认都要留下三样东西，除非本轮只是纯调查：

1. 硬标准：用玩家能感知的结果描述“做到什么程度才算好”，不要只写“优化 UI / 增强策略 / 美化卡牌”。
2. 可复用骨架：能抽成 helper、viewmodel、balance script、snapshot、scene component 或数据契约的，就不要做一次性补丁。
3. 验收要求：至少补一个自动测试、视觉合同、审计报告或明确的人工检查截图路径。

临时灰盒也可以提交，但必须写清楚它当前没有达到哪条硬标准，以及下一轮应该用什么验收门槛把它锁住。
