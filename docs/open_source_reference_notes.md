...
- Card art baseline: the repository currently uses the attributed Night Patrol UI frame/sigil pack plus procedural GDScript motifs. Future GitHub art references must be logged here with license, copied files, and the exact visual job they solve before being made player-facing.
