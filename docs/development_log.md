...
- 主游戏玩家面板新增“开局轻引导（可关闭）”：前 1-2 分钟用短清单提醒首召怪兽、建城、从怪兽补给范围购牌、匿名出牌、查看经济总览；关闭状态会随本局存档保存。
- 终局结算新增“终局总结”，同时写入日志和局势排名页，解释赢家、钱从哪里来、关键卡牌、怪兽影响、AI 主要路线、关键城市、存活城市/毁坏区域。
...
- 新增最大席位烟测：临时开启 8 席 / 7 AI / 深度V 星球，验证 7 个 AI 都能获得角色与起始怪兽牌、首召怪兽、自动建城、购牌、至少产生后续出牌/匿名商业/经营收入/决策样本，并能恢复原局面。
- 终局倒计时测试现在也验证终局总结会出现在日志和排名文本中，并覆盖关键卡牌、怪兽影响和 AI 路线字段。
- 主面板测试验证短目标提示、可关闭开局清单、经济总览快捷入口和存档恢复，同时复杂经济/角色细节仍不回流到主面板。
