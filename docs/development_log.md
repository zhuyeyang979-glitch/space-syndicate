...
- 地图采用球面世界模型：
  ...
  - 左右、上下边界在球面意义上连续。
- 每局按 Roguelike 挑战深度 I-VI 随机生成星球：浅层星球较小，约 8-13 个区域；深层星球逐步扩大到几十个区域，同时提高通关现金目标。
...
- 已建立 Godot headless 烟测：
  - 加载主场景
  - 新建 4 席、3 AI 的 PVE 运行，并验证 3-8 总席位、2-7 AI 设置与 Roguelike 深度星球规模/现金目标
  - 验证角色卡、起始怪兽卡、资源/赠牌/升兽收益、情报能力与远程补给范围
  - 验证怪兽召唤、升级、持续时间、自动行动
  - 验证球面地图、城市、商品、商路、区域伤害
  - 验证卡牌购买、升级、打出、竞价、匿名归属猜测
  - 验证 AI 会评分手牌、匿名出牌、参与同时批次竞价、自动回应合约、做城市业主推理/卡牌归属押注，并记录候选与收益样本
  - 验证合约牌展示后独立签约窗口
...
1. 继续把 AI 行为扩展到怪兽诱导路线、更长期的经济规划、跨周期商品策略与更稳的“最终钱最多”reward 训练。
