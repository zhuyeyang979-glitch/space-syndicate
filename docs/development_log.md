...
- 把首局动作链从“看懂经济”继续推进到“知道接下来怎么玩”：
  ...
  - Runtime first-run Coach 不再在“出牌+看牌轨”后直接折叠；真实首局会继续引导到经济总览和路线选择。旧 demo/教程仍可通过 `auto_fold_when_track_seen` 保持折叠行为。
- 硬标准：
...
- 让战役入口更像桌游电子版的“开始路径”，而不是只给章节列表：
  - `CampaignMenuSnapshot` 新增 `path_steps`，用数据描述“开桌 → 练四步 → 完整局”。
