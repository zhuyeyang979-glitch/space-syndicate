...
- 已建立 Godot headless 烟测：
  ...
  - 验证 AI 会评分手牌、匿名出牌、参与同时批次竞价、自动回应合约、做城市业主推理/卡牌归属押注、规划怪兽诱导目标、维护经济焦点商品、切换 grow/defend/disrupt 策略意图、跨建城/卡牌/合约/商业动作推进商品路线计划，并记录候选与收益样本
  - 验证合约牌展示后独立签约窗口
...
1. 在已有多周期商品路线之上继续加入连续诱导/合约组合，并用“最终钱最多”的 reward 回填来训练路线切换与阶段选择。
