...
- 主菜单逻辑已整理为清晰分支：开局准备、继续/保存、局势排名、经济总览、新手引导、游戏规则、统一图鉴。
- “选择怪兽”不再是主菜单分支；新游戏开始前进入开局准备，每名玩家选择外星角色卡。
- 图鉴统一包含：
  - 角色图鉴
  - 怪兽图鉴
  - 卡牌图鉴
...
1. 拆分 `scripts/main.gd`，把规则模型、卡牌数据、UI、地图投影、经济系统分模块维护。
2. 做一版新手引导，把当前“游戏规则”菜单内容改造成逐步教程。
3. 继续把合约牌和城市经营卡做成更强的推理线索，例如按商品品类、城市规模、海洋商路或怪兽资源偏好触发不同签约奖惩。
4. 继续调怪兽行动概率与资源偏好，让怪兽争夺商品资源的路线更容易被玩家推理。
5. 为卡牌与怪兽替换正式美术，保留当前程序美术作为占位 fallback。
6. 为长局经济波动增加专门测试，验证商品供需、城市竞争、商路破坏与最终金钱结算的平衡。
