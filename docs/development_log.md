...
- AI 记忆中的训练样本从简单记录扩展为可训练结构：
  ...
  - 下一次经营周期结算后回填现金收益和估算结算收益，形成训练/调参的 reward 字段，并已用于上面的在线学习加成。
...
- 已建立 Godot headless 烟测：
  ...
  - 验证 AI 会评分手牌、匿名出牌、参与同时批次竞价、自动回应合约、做城市业主推理/卡牌归属押注、规划怪兽诱导目标、维护经济焦点商品、切换 grow/defend/disrupt 策略意图、跨建城/卡牌/合约/商业动作推进商品路线计划，按现金 reward 进行席位隔离的在线学习，并记录候选与收益样本
  - 验证合约牌展示后独立签约窗口
...
1. 在已有在线学习之上继续加入连续诱导/合约组合，并把 reward 从单周期收益扩展到更长局、终局现金和不同 Roguelike 星球尺寸下的稳定表现。
