...
| 首局可读性 | 4 | `tests/commercial_playability_gate_test.gd` now enters real `RuntimeGameScreen` at 1280×720 / 1600×960 / 1920×1080, checks thin timeline, visible hand, prominent planet, single CTA, privacy, local-human-bound first-run Coach actions, 5-second strong focus pulses after Coach CTAs, runtime keyboard/controller focus order with dynamic card-rack insertion, first-purchase teaching-card gate, and a full first-loop chain: starter summon, first city, rack, first regional purchase, first non-starter card play, public-track inspection, and economy-overview inspection. | Add headed human review captures for the full first 10 minutes and controller-device QA. |
