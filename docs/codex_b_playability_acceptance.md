...
7. 剧本行动日志只显示公开记录和当前玩家私密记录。
8. Campaign 关卡完成必须来自真实 scenario signal，而不是 UI 按钮直接通关。
9. 截图/布局测试必须覆盖 Scenario/Campaign 关键页面。
10. 每轮收尾必须运行 Codex B 必跑测试，并在推 main 前确认没有覆盖 Codex A 的最新 main。
