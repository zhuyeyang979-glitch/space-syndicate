...
This candidate is the first observation-only human baseline for the fully connected V0.7.3 production ruleset. It supports New Game only. Save and Continue remain visible but disabled, and V0.6 save files are neither loaded nor modified.
...
- Release tag: `alpha-0.5c-v073-human-playtest-1`
...
5. Use the contextual Coach Marks as needed, or choose `Skip all` and continue unaided.
...
Run A is valid only when the manifest records seed `900626424`, four players, ruleset `v0.7.3`, and profile `v073_human_baseline_01`.
...
Run B must use the same frozen balance profile as Run A. The random seed is written to `manifest.json`.
