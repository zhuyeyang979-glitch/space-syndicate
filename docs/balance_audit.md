...
Current automated runtime guards: the eight-seat smoke run now calls `_ai_live_route_balance_report()`, `_ai_product_route_bridge_report()`, and `_ai_profile_strategy_identity_report()` after AI opening, city building, buying, playing, business actions, cashflow, and primary-route exercises. These reports are internal-only. The first checks live route diversity, money progress, primary-route hits, and multiple action kinds. The second checks that hidden AI samples actually bridge product ecology, focus products, route-plan products, route stages, development routes, and policy families such as buying, playing, anonymous commerce, contracts, futures, weather, direct interaction, monster pressure, military, or intel. The third checks that each simulated AI personality expresses its own route and action-family identity instead of collapsing into one generic opponent. Together they close the gap between “card fields say the route exists” and “AI actually connects products, cards, routes, personality, and economy in a simulated run.”
...
- Covered now: AI personality identity is audited by `_ai_profile_strategy_identity_report()` inside the eight-seat smoke run. The test verifies six simulated personality profiles, identity-ready samples for each, at least five primary routes, at least four expected action families, and at least three signature action families. This is a guard against all AI profiles degenerating into one generic policy while keeping hidden AI scoring out of player pages.
