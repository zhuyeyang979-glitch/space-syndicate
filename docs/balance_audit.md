...
7. Covered now: AI route viability has an internal bridge audit. `_ai_route_viability_report()` combines static card pressure, real eight-seat AI decision samples, and personality-route support to check that at least five core routes are currently "pursuable goals" instead of just labels in the codex.
8. Role cards now have a separate role-budget audit. The current role pool is broad enough, but the next tuning pass should watch high-leverage public passives:
...
Current automated runtime guards: the eight-seat smoke run now calls `_ai_live_route_balance_report()`, `_ai_route_viability_report()`, `_ai_product_route_bridge_report()`, and `_ai_profile_strategy_identity_report()` after AI opening, city building, buying, playing, business actions, cashflow, and primary-route exercises. It also exercises explicit leader/trailer/endgame scenarios for `victory_race_*` scoring. These reports are internal-only. The first checks live route diversity, money progress, primary-route hits, and multiple action kinds. The new route-viability bridge checks that static card pressure plus real decision samples plus profile support produce at least five pursuable core routes. The product bridge checks that hidden AI samples actually connect product ecology, focus products, route-plan products, route stages, development routes, and policy families such as buying, playing, anonymous commerce, contracts, futures, weather, direct interaction, monster pressure, military, or intel. The identity report checks that each simulated AI personality expresses its own route and action-family identity instead of collapsing into one generic opponent, and now also verifies that the real scoring layer produced `profile_signature_bonus` samples for every simulated profile. The phase-policy check additionally verifies that a trailing AI can enter `break_countdown` pressure against a leader and a leading AI can enter `protect_lead` defense. Together they close the gap between “card fields say the route exists” and “AI actually connects products, cards, routes, personality, finale pressure, and economy in a simulated run.”
...
- Covered now: AI route viability is audited by `_ai_route_viability_report()` inside the same eight-seat smoke run. It does not require every route to dominate every generated planet; it requires at least five core routes to have field-driven value, readable gates/clues, real or profile-supported AI access, and enough pressure to count as a credible path toward the money goal.
