...
- Keyword chips: use symbols for repeated rules (`¥`, `◇商品流`, `◆怪兽`, `◎玩家`, `⇄合约`, `一次/固定`) and expose the same legend in `RulesQuickReferenceBoard` as a dedicated card-symbol rail, not as prose hidden inside a paragraph.
