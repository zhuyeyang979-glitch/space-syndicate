...
The public `_capture_run_state()` and `_apply_run_state()` wrappers have been physically retired from production `main.gd`. Tests and tools must use the owning controller, `GameSessionRuntimeController`, or the v0.6 save-envelope gates; restoring those public wrapper names is forbidden. The two flat-domain adapters above remain migration-only while the active legacy run-save payload still needs them, and they must shrink whenever an owner section becomes authoritative rather than becoming a second save system.

The legacy flat key `district_card_purchase_snapshot` remains migration-only. `DistrictPurchaseRuntimeController` captures the current purchase session and its locked quote; restore accepts the same bound quote only while `now_world_us < expires_at_world_us`, otherwise it fails closed without live repricing. The retired 12-second qualification window, regional access tiers, owner discounts, and price-floor behavior must not be restored through this compatibility path.
...
## SS05-04: Victory Control And Public Audit Hard Cutover
