...
Status: **PRE-IMPLEMENTATION REVIEW — PRIVACY BLOCKERS IDENTIFIED**
...
The current Main-owned presentation chain cannot be moved unchanged into a
scene-owned `TablePresentationSourceOwner`.
...
If either condition cannot be met without a new Main callback or a second
world owner, the production cutover must fail closed.
...
`TABLE_PRESENTATION_SOURCE_TARGET_CUTOVER_BLOCKED`, not a Main fallback or a
