...
- `CardArtView` 负责共享卡面美术：星空底、多来源中央 sprite 插画、类型纹样、等级标记和当前临时卡面皮肤。
- Night Patrol backplate/frame/strip/sigil 仍是当前生产基线，在新的 Card/Table/Menu skin 通过完整切换门之前必须保持可见；它已被列为计划退役层，不能继续扩展成新的长期依赖。
...
- 卡框和机械 HUD 首选参考 `Mechanized Magic` CC0；按钮、面板、滑块和弹窗首选 `Kenney UI Pack - Sci-Fi` CC0。
- 资源、牌堆、竞价、回合、教程和状态语义优先使用 `Kenney Board Game Icons/Info` CC0，并通过稳定 semantic icon id 访问，不允许 UI 脚本继续增加第三方硬编码路径。
- `Game-icons.net` 只补充 CC0 图标库无法表达的语义；每个保留图标必须登记原作者、源链接和 CC BY 3.0。
- 新皮肤必须保留 Space Syndicate 自己的路线色、类型层级、中央星球、匿名牌轨、资源筹码和右侧检查器，不得把任何参考包原样当成品牌身份。
- `SCIFI UI` 只用于灰盒切片参考；`Wenrexa White UI Kit` 优先作为 QA/高对比界面；两者不能与主皮肤无选择拼贴。
- Night Patrol 卡框层只有在手牌、牌架、结算、Inspector、Codex、Authoring 和截图门全部切换，且 `rg` 为零引用后才可删除。
- 完整采用、测试和退役顺序见 `docs/card_table_menu_reference_adoption_plan.md`。
...
3. **费用与门槛**：费用、区域 GDP 份额门槛或“免门槛”必须作为 chip 出现。
