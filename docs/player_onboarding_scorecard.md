...
| 一步一目标 | 4 | ScenarioCoach 单主 CTA；首局 CTA 现在会自动落到推荐区域 | 卡住状态需要更多自动定位 |
| 错误反馈 | 4 | Scenario 阶段已要求 `stuck_hint` / `focus_target`；Coach 求助会显示短卡住提示；运行桌面现在有独立 `FocusGuideLayer` 光框；普通首局 `FirstRunCoach` 也会输出桌面焦点；首局 CTA 缺选区时会自动选推荐区 | 下一轮把连续失败直接转成自动打开对应页面/抽屉 |
...
2. 自动定位：首局 CTA 已支持推荐区域兜底；下一步处理连续失败后自动打开对应区域/牌架/详情。
