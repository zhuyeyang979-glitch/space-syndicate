...
| 错误反馈 | 4 | Scenario 阶段已要求 `stuck_hint` / `focus_target`；Coach 求助会显示短卡住提示；运行桌面现在有独立 `FocusGuideLayer` 光框；剧本“定位”会打开真实目标；普通首局 `FirstRunCoach` 也会输出桌面焦点；首局 CTA 缺选区时会自动选推荐区，买牌选错区时会恢复到合法牌架并同步星球视角 | 下一轮让连续失败自动闪烁并推荐最短操作 |
...
2. 自动定位：首局 CTA 已支持推荐区域兜底、买牌合法牌架恢复和星球视角聚焦；剧本“定位”已能打开牌轨/牌架/经济/情报等真实目标；下一步处理连续失败后的闪烁节奏和最短操作推荐。
