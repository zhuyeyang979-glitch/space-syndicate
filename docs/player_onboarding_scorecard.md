...
| 新手战役入口 | 4 | CampaignMenu/ProgressMap；CampaignMenu 使用“开桌→练流程→完整局”三步路径轨 | 真人复验是否能直接理解从战役进入试玩 |
| 关卡目标清晰度 | 4 | CampaignBriefing 三张摘要卡 + Coach；剧本“定位”现在会真实打开牌轨/牌架/经济/情报等目标；强卡住态会显示最短操作 | 真人复验摘要卡是否足够支撑直接开始 |
| 一步一目标 | 4 | ScenarioCoach 单主 CTA；首局 CTA 现在会自动落到推荐区域；买牌 CTA 会自动寻找合法牌架，并把星球旋转到目标区；星球旁试玩罗盘显示已完成/当前/待办和下一步短句，并已覆盖点区→首召→建城→买牌→出牌→牌轨→经济→路线；FirstRunCoach 现在是星球左侧窄卡，不会在看过牌轨后提前折叠，会继续带到经济与路线；重复卡住仍只保留一个“定位下一步”主 CTA | 真人复验首局 10 分钟 |
...
1. 运行牌桌细焦点：分区焦点链已覆盖顶部、牌轨、星球、详情、手牌、行动、竞价；牌轨、手牌和区域牌架卡已经支持确认键；星球地图焦点下方向键会切区并旋转到目标区域，确认键可打开牌架；星球旁试玩罗盘和 FirstRunCoach 都已覆盖到路线选择，且默认折叠点延后到路线选择之后；FirstRunCoach 已从横幅改为左侧窄卡，下一步做真人手柄型号复验。
